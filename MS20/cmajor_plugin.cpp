...
// Auto-generated Cmajor code for patch 'MS20'
...
//==============================================================================
/// Auto-generated C++ class for the 'MS20' processor
///
...
namespace performer
{
    struct MS20
    {
        ...
        static constexpr std::string_view name = "MS20";
        ...
        //==============================================================================
        static constexpr uint32_t numInputEndpoints  = 31;
        ...
        static constexpr uint32_t getEndpointHandleForName (std::string_view endpointName)
        {
            ...
            if (endpointName == "mgRate")    return static_cast<uint32_t> (EndpointHandles::mgRate);
            if (endpointName == "mgWave")    return static_cast<uint32_t> (EndpointHandles::mgWave);
            if (endpointName == "mgPitch")   return static_cast<uint32_t> (EndpointHandles::mgPitch);
            if (endpointName == "mgFilt")    return static_cast<uint32_t> (EndpointHandles::mgFilt);
            ...
            if (endpointName == "o2Pitch")   return static_cast<uint32_t> (EndpointHandles::o2Pitch);
            ...
            if (endpointName == "hpfCut")    return static_cast<uint32_t> (EndpointHandles::hpfCut);
            if (endpointName == "hpfPeak")   return static_cast<uint32_t> (EndpointHandles::hpfPeak);
            if (endpointName == "lpfCut")    return static_cast<uint32_t> (EndpointHandles::lpfCut);
            if (endpointName == "lpfPeak")   return static_cast<uint32_t> (EndpointHandles::lpfPeak);
            if (endpointName == "egFilt")    return static_cast<uint32_t> (EndpointHandles::egFilt);
            if (endpointName == "e1A")       return static_cast<uint32_t> (EndpointHandles::e1A);
            if (endpointName == "e1R")       return static_cast<uint32_t> (EndpointHandles::e1R);
            if (endpointName == "e1Pitch")   return static_cast<uint32_t> (EndpointHandles::e1Pitch);
            ...
            if (endpointName == "porta")     return static_cast<uint32_t> (EndpointHandles::porta);
        ...
        static constexpr EndpointInfo inputEndpoints[] =
        {
            ...
            { 2,   "mgRate",   EndpointType::event },
            { 3,   "mgWave",   EndpointType::event },
            { 4,   "mgPitch",  EndpointType::event },
            { 5,   "mgFilt",   EndpointType::event },
            { 6,   "o1Wave",   EndpointType::event },
            { 7,   "o1PW",     EndpointType::event },
            { 8,   "o2Wave",   EndpointType::event },
            { 9,   "o2Pitch",  EndpointType::event },
            { 10,  "o2PW",     EndpointType::event },
            { 11,  "sync",     EndpointType::event },
            { 12,  "mix1",     EndpointType::event },
            { 13,  "mix2",     EndpointType::event },
            { 14,  "noise",    EndpointType::event },
            { 15,  "ring",     EndpointType::event },
            { 16,  "drive",    EndpointType::event },
            { 17,  "hpfCut",   EndpointType::event },
            { 18,  "hpfPeak",  EndpointType::event },
            { 19,  "lpfCut",   EndpointType::event },
            { 20,  "lpfPeak",  EndpointType::event },
            { 21,  "egFilt",   EndpointType::event },
            { 22,  "e1A",      EndpointType::event },
            { 23,  "e1R",      EndpointType::event },
            { 24,  "e1Pitch",  EndpointType::event },
            ...
            { 30,  "tune",     EndpointType::event },
            { 31,  "porta",    EndpointType::event }
        };

        static constexpr EndpointInfo outputEndpoints[] =
        {
            { 32,  "audioOut",  EndpointType::stream },
            { 33,  "noteOut",   EndpointType::event  }
        };
        ...
        struct _MS20_State
        {
            int32_t _sessionID = {};
            double _frequency = {};
            Array<_MS20Voice_1_State, 1> voices;
        ...
        struct MS20_eventValue_noteOut
        {
            int32_t frame = {};
            int32_t type = {};
        ...
        struct MS20_State
        {
            ...
            Array<MS20_eventValue_noteOut, 32> noteOut;
        };
        ...
        {
            if (endpointHandle == 32) { std::memcpy (reinterpret_cast<char*> (dest), std::addressof (cmajIO.audioOut), 8 * numFramesToCopy); std::memset (reinterpret_cast<char*> (std::addressof (cmajIO.audioOut)), 0, 8 * numFramesToCopy); return; }
        ...
        {
            if (endpointHandle == 33) return (uint32_t) cmajState.noteOut_eventCount;
        ...
        {
            if (endpointHandle == 33) { cmajState.noteOut_eventCount = 0; return; }
        }

        uint32_t getOutputEventType (EndpointHandle endpointHandle, uint32_t index)
        {
            if (endpointHandle == 33) return (uint32_t) cmajState.noteOut[(IndexType) index].type;
            throw std::runtime_error ("Unknown event endpointHandle:" + std::to_string (endpointHandle));
            return {};
        }
        ...
        static uint32_t getOutputEventDataSize (EndpointHandle endpointHandle, uint32_t typeIndex)
        {
            ...
            if (endpointHandle == 33 && typeIndex == 0) return 8;
            throw std::runtime_error ("Unknown event endpointHandle:" + std::to_string (endpointHandle));
            return 0;
        }
        ...
        void addEvent_mgRate (float event)
        {
            _sendEvent_mgRate (cmajState, event);
        }

        void addEvent_mgWave (float event)
        {
            _sendEvent_mgWave (cmajState, event);
        }

        void addEvent_mgPitch (float event)
        {
            _sendEvent_mgPitch (cmajState, event);
        }

        void addEvent_mgFilt (float event)
        {
            _sendEvent_mgFilt (cmajState, event);
        }
        ...
        void addEvent_o2Pitch (float event)
        {
            _sendEvent_o2Pitch (cmajState, event);
        }
        ...
        void addEvent_hpfCut (float event)
        {
            _sendEvent_hpfCut (cmajState, event);
        }

        void addEvent_hpfPeak (float event)
        {
            _sendEvent_hpfPeak (cmajState, event);
        }

        void addEvent_lpfCut (float event)
        {
            _sendEvent_lpfCut (cmajState, event);
        }

        void addEvent_lpfPeak (float event)
        {
            _sendEvent_lpfPeak (cmajState, event);
        }

        void addEvent_egFilt (float event)
        {
            _sendEvent_egFilt (cmajState, event);
        }

        void addEvent_e1A (float event)
        {
            _sendEvent_e1A (cmajState, event);
        }

        void addEvent_e1R (float event)
        {
            _sendEvent_e1R (cmajState, event);
        }

        void addEvent_e1Pitch (float event)
        {
            _sendEvent_e1Pitch (cmajState, event);
        }
        ...
        void addEvent_porta (float event)
        {
            _sendEvent_porta (cmajState, event);
        }
        ...
        MS20_State cmajState = {};
        MS20_IO cmajIO = {};

        //==============================================================================
        void _sendEvent_midiIn (MS20_State& _state, std_midi_Message value) noexcept
        {
            _MS20__midiIn (_state._state, value);
        }

        void _MS20__midiIn (_MS20_State& _state, std_midi_Message value) noexcept
        {
        ...
        {
            _MS20___forwardEvent (state_upcast_struct_MS20_State_struct_vJwBVc(_state), value);
            _MS20___forwardEvent_5 (state_upcast_struct_MS20_State_struct_vJwBVc(_state), value);
        }

        void _MS20___forwardEvent (_MS20_State& _state, std_notes_NoteOn value) noexcept
        {
        ...
        {
            _MS20___forwardEvent_3 (state_upcast_struct_MS20_State_struct_F3s1ce(_state), value);
        }

        void _MS20___forwardEvent_3 (_MS20_State& _state, plugincorp_NoteEvent value) noexcept
        {
            _MS20___writeEvent_noteOut (_state, value);
        }

        void _MS20___writeEvent_noteOut (_MS20_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
            MS20___writeEvent_noteOut (state_upcast_struct_MS20_State_struct__XWNTve(_state), value);
        }

        void MS20___writeEvent_noteOut (MS20_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
            MS20___writeEvent_noteOut_0 (_state, value);
        }

        void MS20___writeEvent_noteOut_0 (MS20_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
        ...
        void _MS20___forwardEvent_5 (_MS20_State& _state, std_notes_NoteOn value) noexcept
        {
        ...
        {
            _MS20___forwardEvent_12 (state_upcast_struct_MS20_State_struct_pskEPe(_state), index, value);
        }

        void _MS20___forwardEvent_12 (_MS20_State& _state, int32_t index, std_notes_NoteOff value) noexcept
        {
            MS20Voice__eventsIn (_state.voices[index], value);
        }

        void MS20Voice__eventsIn (_MS20Voice_1_State& _state, std_notes_NoteOff e) noexcept
        {
        ...
        {
            _MS20___forwardEvent_11 (state_upcast_struct_MS20_State_struct_pskEPe(_state), index, value);
        }

        void _MS20___forwardEvent_11 (_MS20_State& _state, int32_t index, std_notes_NoteOn value) noexcept
        {
            MS20Voice__eventsIn_0 (_state.voices[index], value);
        }

        void MS20Voice__eventsIn_0 (_MS20Voice_1_State& _state, std_notes_NoteOn e) noexcept
        {
            float  sr;
            float  gt;
        ...
        {
            _MS20___forwardEvent_2 (state_upcast_struct_MS20_State_struct_vJwBVc(_state), value);
            _MS20___forwardEvent_6 (state_upcast_struct_MS20_State_struct_vJwBVc(_state), value);
        }

        void _MS20___forwardEvent_2 (_MS20_State& _state, std_notes_NoteOff value) noexcept
        {
        ...
        void _MS20___forwardEvent_6 (_MS20_State& _state, std_notes_NoteOff value) noexcept
        {
        ...
        {
            _MS20___forwardEvent_7 (state_upcast_struct_MS20_State_struct_vJwBVc(_state), value);
        }

        void _MS20___forwardEvent_7 (_MS20_State& _state, std_notes_PitchBend value) noexcept
        {
        ...
        {
            _MS20___forwardEvent_9 (state_upcast_struct_MS20_State_struct_vJwBVc(_state), value);
        }

        void _MS20___forwardEvent_9 (_MS20_State& _state, std_notes_Pressure value) noexcept
        {
        ...
        {
            _MS20___forwardEvent_8 (state_upcast_struct_MS20_State_struct_vJwBVc(_state), value);
        }

        void _MS20___forwardEvent_8 (_MS20_State& _state, std_notes_Slide value) noexcept
        {
        ...
        {
            _MS20___forwardEvent_10 (state_upcast_struct_MS20_State_struct_vJwBVc(_state), value);
        }

        void _MS20___forwardEvent_10 (_MS20_State& _state, std_notes_Control value) noexcept
        {
        ...
        void _sendEvent_mgRate (MS20_State& _state, float value) noexcept
        {
            _MS20__mgRate (_state._state, value);
        }

        void _MS20__mgRate (_MS20_State& _state, float value) noexcept
        {
            ParamsProcessor__mgRate (_state.paramsProcessor, value);
        }

        void ParamsProcessor__mgRate (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.mgRate = v;
        ...
        {
            _MS20___forwardEvent_4 (state_upcast_struct_MS20_State_struct_XuVgme(_state), value);
        }

        void _MS20___forwardEvent_4 (_MS20_State& _state, plugincorp_Params value) noexcept
        {
            MS20Voice__paramsIn (_state.voices[int32_t {0}], value);
        }

        void MS20Voice__paramsIn (_MS20Voice_1_State& _state, plugincorp_Params p) noexcept
        {
            _state.params = p;
        }

        void _sendEvent_mgWave (MS20_State& _state, float value) noexcept
        {
            _MS20__mgWave (_state._state, value);
        }

        void _MS20__mgWave (_MS20_State& _state, float value) noexcept
        {
            ParamsProcessor__mgWave (_state.paramsProcessor, value);
        }

        void ParamsProcessor__mgWave (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.mgWave = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_mgPitch (MS20_State& _state, float value) noexcept
        {
            _MS20__mgPitch (_state._state, value);
        }

        void _MS20__mgPitch (_MS20_State& _state, float value) noexcept
        {
            ParamsProcessor__mgPitch (_state.paramsProcessor, value);
        }

        void ParamsProcessor__mgPitch (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.mgPitch = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_mgFilt (MS20_State& _state, float value) noexcept
        {
            _MS20__mgFilt (_state._state, value);
        }

        void _MS20__mgFilt (_MS20_State& _state, float value) noexcept
        {
            ParamsProcessor__mgFilt (_state.paramsProcessor, value);
        }

        void ParamsProcessor__mgFilt (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.mgFilt = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_o1Wave (MS20_State& _state, float value) noexcept
        {
            _MS20__o1Wave (_state._state, value);
        }

        void _MS20__o1Wave (_MS20_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_o1PW (MS20_State& _state, float value) noexcept
        {
            _MS20__o1PW (_state._state, value);
        }

        void _MS20__o1PW (_MS20_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_o2Wave (MS20_State& _state, float value) noexcept
        {
            _MS20__o2Wave (_state._state, value);
        }

        void _MS20__o2Wave (_MS20_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_o2Pitch (MS20_State& _state, float value) noexcept
        {
            _MS20__o2Pitch (_state._state, value);
        }

        void _MS20__o2Pitch (_MS20_State& _state, float value) noexcept
        {
            ParamsProcessor__o2Pitch (_state.paramsProcessor, value);
        }

        void ParamsProcessor__o2Pitch (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.o2Pitch = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_o2PW (MS20_State& _state, float value) noexcept
        {
            _MS20__o2PW (_state._state, value);
        }

        void _MS20__o2PW (_MS20_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_sync (MS20_State& _state, float value) noexcept
        {
            _MS20__sync (_state._state, value);
        }

        void _MS20__sync (_MS20_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_mix1 (MS20_State& _state, float value) noexcept
        {
            _MS20__mix1 (_state._state, value);
        }

        void _MS20__mix1 (_MS20_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_mix2 (MS20_State& _state, float value) noexcept
        {
            _MS20__mix2 (_state._state, value);
        }

        void _MS20__mix2 (_MS20_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_noise (MS20_State& _state, float value) noexcept
        {
            _MS20__noise (_state._state, value);
        }

        void _MS20__noise (_MS20_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_ring (MS20_State& _state, float value) noexcept
        {
            _MS20__ring (_state._state, value);
        }

        void _MS20__ring (_MS20_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_drive (MS20_State& _state, float value) noexcept
        {
            _MS20__drive (_state._state, value);
        }

        void _MS20__drive (_MS20_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_hpfCut (MS20_State& _state, float value) noexcept
        {
            _MS20__hpfCut (_state._state, value);
        }

        void _MS20__hpfCut (_MS20_State& _state, float value) noexcept
        {
            ParamsProcessor__hpfCut (_state.paramsProcessor, value);
        }

        void ParamsProcessor__hpfCut (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.hpfCut = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_hpfPeak (MS20_State& _state, float value) noexcept
        {
            _MS20__hpfPeak (_state._state, value);
        }

        void _MS20__hpfPeak (_MS20_State& _state, float value) noexcept
        {
            ParamsProcessor__hpfPeak (_state.paramsProcessor, value);
        }

        void ParamsProcessor__hpfPeak (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.hpfPeak = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_lpfCut (MS20_State& _state, float value) noexcept
        {
            _MS20__lpfCut (_state._state, value);
        }

        void _MS20__lpfCut (_MS20_State& _state, float value) noexcept
        {
            ParamsProcessor__lpfCut (_state.paramsProcessor, value);
        }

        void ParamsProcessor__lpfCut (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.lpfCut = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_lpfPeak (MS20_State& _state, float value) noexcept
        {
            _MS20__lpfPeak (_state._state, value);
        }

        void _MS20__lpfPeak (_MS20_State& _state, float value) noexcept
        {
            ParamsProcessor__lpfPeak (_state.paramsProcessor, value);
        }

        void ParamsProcessor__lpfPeak (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.lpfPeak = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_egFilt (MS20_State& _state, float value) noexcept
        {
            _MS20__egFilt (_state._state, value);
        }

        void _MS20__egFilt (_MS20_State& _state, float value) noexcept
        {
            ParamsProcessor__egFilt (_state.paramsProcessor, value);
        }

        void ParamsProcessor__egFilt (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.egFilt = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_e1A (MS20_State& _state, float value) noexcept
        {
            _MS20__e1A (_state._state, value);
        }

        void _MS20__e1A (_MS20_State& _state, float value) noexcept
        {
            ParamsProcessor__e1A (_state.paramsProcessor, value);
        }

        void ParamsProcessor__e1A (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.e1A = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_e1R (MS20_State& _state, float value) noexcept
        {
            _MS20__e1R (_state._state, value);
        }

        void _MS20__e1R (_MS20_State& _state, float value) noexcept
        {
            ParamsProcessor__e1R (_state.paramsProcessor, value);
        }

        void ParamsProcessor__e1R (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.e1R = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_e1Pitch (MS20_State& _state, float value) noexcept
        {
            _MS20__e1Pitch (_state._state, value);
        }

        void _MS20__e1Pitch (_MS20_State& _state, float value) noexcept
        {
            ParamsProcessor__e1Pitch (_state.paramsProcessor, value);
        }

        void ParamsProcessor__e1Pitch (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.e1Pitch = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_aA (MS20_State& _state, float value) noexcept
        {
            _MS20__aA (_state._state, value);
        }

        void _MS20__aA (_MS20_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_aD (MS20_State& _state, float value) noexcept
        {
            _MS20__aD (_state._state, value);
        }

        void _MS20__aD (_MS20_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_aS (MS20_State& _state, float value) noexcept
        {
            _MS20__aS (_state._state, value);
        }

        void _MS20__aS (_MS20_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_aR (MS20_State& _state, float value) noexcept
        {
            _MS20__aR (_state._state, value);
        }

        void _MS20__aR (_MS20_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_volume (MS20_State& _state, float value) noexcept
        {
            _MS20__volume (_state._state, value);
        }

        void _MS20__volume (_MS20_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_tune (MS20_State& _state, float value) noexcept
        {
            _MS20__tune (_state._state, value);
        }

        void _MS20__tune (_MS20_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_porta (MS20_State& _state, float value) noexcept
        {
            _MS20__porta (_state._state, value);
        }

        void _MS20__porta (_MS20_State& _state, float value) noexcept
        {
            ParamsProcessor__porta (_state.paramsProcessor, value);
        }

        void ParamsProcessor__porta (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.porta = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _initialise (MS20_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            _MS20___initialise (_state._state, processorID, sessionID, frequency);
        }

        void _MS20___initialise (_MS20_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
            _state.voices[int32_t {0}]._instanceIndex = int32_t {0};
            MS20Voice___initialise (_state.voices[int32_t {0}], processorID, sessionID, frequency);
        ...
        void MS20Voice___initialise (_MS20Voice_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
        ...
        void ParamsProcessor___initialise (_ParamsProcessor_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
            _state.params = plugincorp_Params { 0.3f, 0.0f, 0.0f, 0.0f, 0.34f, 0.5f, 0.0f, 0.5f, 0.5f, 0.0f, 0.8f, 0.6f, 0.0f, 0.0f, 0.3f, 0.0f, 0.2f, 0.55f, 0.3f, 0.4f, 0.02f, 0.4f, 0.0f, 0.02f, 0.45f, 0.7f, 0.25f, 0.8f, 0.5f, 0.0f };
        }
        ...
        void _advance (MS20_State& _state, MS20_IO& _io, int32_t _frames) noexcept
        {
            _MS20_IO  ioCopy;
        ...
        void main (_MS20_State& _state, _MS20_IO& _io) noexcept
        {
            Array<_MS20Voice_1_IO, 1>  voices_io;
            ...
            voices_io = Array<_MS20Voice_1_IO, 1> {};
            ...
            MS20Voice__main (_state.voices[int32_t {0}], voices_io[int32_t {0}]);
            {
            }
        ...
        void MS20Voice__main (_MS20Voice_1_State& _state, _MS20Voice_1_IO& _io) noexcept
        {
            float  out;
            float  mgHz;
            float  mTri;
            ...
            _branch0_0:
            {
                for (;;)
                {
                    ...
                    if (_state.active)
                    {
                        mgHz = 0.05f + ((_state.params.mgRate * _state.params.mgRate) * 28.0f);
                        _state.mgPh = (_state.mgPh + (mgHz * _state.invSr));
                        if (_state.mgPh >= 1.0f)
                        {
                            _state.mgPh = (_state.mgPh - 1.0f);
                        }
                        mTri = (_state.mgPh < 0.5f) ? ((4.0f * _state.mgPh) - 1.0f) : (3.0f - (4.0f * _state.mgPh));
                        mPul = (_state.mgPh < 0.5f) ? 1.0f : -1.0f;
                        mg = (mTri * (1.0f - _state.params.mgWave)) + (mPul * _state.params.mgWave);
                        e1a = 1.0f / (intrinsics::max (0.0005f, 0.0006f + ((_state.params.e1A * _state.params.e1A) * 3.0f)) * _state.sr);
                        e1r = 1.0f / (intrinsics::max (0.0005f, 0.003f + ((_state.params.e1R * _state.params.e1R) * 7.0f)) * _state.sr);
                        ...
                        e2a = 1.0f / (intrinsics::max (0.0005f, 0.0006f + ((_state.params.aA * _state.params.aA) * 3.0f)) * _state.sr);
                        e2d = 1.0f / (intrinsics::max (0.0005f, 0.003f + ((_state.params.aD * _state.params.aD) * 7.0f)) * _state.sr);
                        e2r = 1.0f / (intrinsics::max (0.0005f, 0.003f + ((_state.params.aR * _state.params.aR) * 7.0f)) * _state.sr);
                        ...
                        pitch = _state.glideFreq;
                        pitch = (pitch * intrinsics::pow (2.0f, (_state.params.tune - 0.5f) * 0.16f));
                        pitch = (pitch * intrinsics::pow (2.0f, (mg * _state.params.mgPitch) * 0.5f));
                        pitch = (pitch * intrinsics::pow (2.0f, (_state.e1 * (_state.params.e1Pitch - 0.0f)) * 2.0f));
                        pw1 = _state.params.o1PW;
                        pw2 = _state.params.o2PW;
                        k1 = static_cast<int32_t> (intrinsics::clamp (intrinsics::floor (_state.params.o1Wave * 3.0f), 0.0f, 2.0f));
                        z2 = static_cast<int32_t> (intrinsics::clamp (intrinsics::floor (_state.params.o2Wave * 3.0f), 0.0f, 2.0f));
                        inc1 = intrinsics::clamp (pitch, 0.0f, _state.sr * 0.49f) * _state.invSr;
                        prev1 = _state.ph1;
                        ...
                        o1 = MS20Voice__wave (_state.ph1, inc1, k1, pw1);
                        f2 = pitch * intrinsics::pow (2.0f, (_state.params.o2Pitch - 0.5f) * 4.0f);
                        ...
                        if (z2 == int32_t {0})
                        {
                            o2 = MS20Voice__wave (_state.ph2, inc2, int32_t {1}, pw2);
                        }
                        else
                        {
                            if (z2 == int32_t {1})
                            {
                                o2 = MS20Voice__wave (_state.ph2, inc2, int32_t {2}, 0.5f);
                            }
                            else
                            {
                                o2 = MS20Voice__wave (_state.ph2, inc2, int32_t {2}, pw2);
                            }
                        }
                        nz = MS20Voice__rnd (_state);
                        sig = ((o1 * _state.params.mix1) + (o2 * _state.params.mix2)) + ((nz * _state.params.noise) * 0.7f);
                        sig = (sig + ((o1 * o2) * _state.params.ring));
                        sig = intrinsics::tanh (sig * (0.8f + (_state.params.drive * 3.2f)));
                        yb = (sig - _state.dcx) + (0.9985f * _state.dcy);
                        _state.dcx = sig;
                        _state.dcy = yb;
                        sig = yb;
                        egc = _state.params.egFilt * _state.e2;
                        mgc = _state.params.mgFilt * mg;
                        hpHz = intrinsics::clamp ((20.0f * intrinsics::exp (_state.params.hpfCut * 6.6f)) * intrinsics::pow (2.0f, mgc * 2.0f), 20.0f, _state.sr * 0.45f);
                        lpHz = intrinsics::clamp ((30.0f * intrinsics::exp (_state.params.lpfCut * 6.2f)) * intrinsics::pow (2.0f, (egc + mgc) * 3.0f), 20.0f, _state.sr * 0.45f);
                        xin = intrinsics::tanh (sig * (1.0f + _state.params.drive));
                        gH = intrinsics::tan ((3.1415927f * hpHz) * _state.invSr);
                        kH = intrinsics::clamp (2.0f - (1.97f * _state.params.hpfPeak), 0.025f, 2.0f);
                        a1H = 1.0f / (1.0f + (gH * (gH + kH)));
                        a2H = gH * a1H;
                        a3H = gH * a2H;
                        v3H = xin - _state.h2;
                        v1H = (a1H * _state.h1) + (a2H * v3H);
                        v2H = (_state.h2 + (a2H * _state.h1)) + (a3H * v3H);
                        _state.h1 = ((2.0f * v1H) - _state.h1);
                        _state.h2 = ((2.0f * v2H) - _state.h2);
                        hp = (xin - (kH * v1H)) - v2H;
                        xinL = intrinsics::tanh (hp * 1.1f);
                        gL = intrinsics::tan ((3.1415927f * lpHz) * _state.invSr);
                        kL = intrinsics::clamp (2.0f - (1.97f * _state.params.lpfPeak), 0.025f, 2.0f);
                        a1L = 1.0f / (1.0f + (gL * (gL + kL)));
                        a2L = gL * a1L;
                        a3L = gL * a2L;
                        v3L = xinL - _state.l2;
                        v1L = (a1L * _state.l1) + (a2L * v3L);
                        v2L = (_state.l2 + (a2L * _state.l1)) + (a3L * v3L);
                        _state.l1 = ((2.0f * v1L) - _state.l1);
                        _state.l2 = ((2.0f * v2L) - _state.l2);
                        lp = v2L;
                        out = intrinsics::tanh (((lp * _state.e2) * (0.4f + (0.6f * _state.vel))) * (0.5f + (_state.params.volume * 1.2f)));
        ...
        float MS20Voice__wave (float ph, float inc, int32_t kind, float pw) noexcept
        {
            float  saw;
            float  p2;
            float  saw2;
        ...
        float MS20Voice__rnd (_MS20Voice_1_State& _state) noexcept
        {
            float  s;
        ...
        static _MS20_State& state_upcast_struct_MS20_State_struct_XuVgme (_ParamsProcessor_1_State& child) { return *reinterpret_cast<_MS20_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_MS20_State, paramsProcessor))); }
        static _MS20_State& state_upcast_struct_MS20_State_struct_pskEPe (std_voices_std_voices_VoiceAllocator_specialised_Sitcze_1_State& child) { return *reinterpret_cast<_MS20_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_MS20_State, voiceAllocator))); }
        static MS20_State& state_upcast_struct_MS20_State_struct__XWNTve (_MS20_State& child) { return *reinterpret_cast<MS20_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (MS20_State, _state))); }
        static _MS20_State& state_upcast_struct_MS20_State_struct_F3s1ce (std_midi_MPEConverter_1_State& child) { return *reinterpret_cast<_MS20_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_MS20_State, noteTap))); }
        static _MS20_State& state_upcast_struct_MS20_State_struct_vJwBVc (std_midi_MPEConverter_1_State& child) { return *reinterpret_cast<_MS20_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_MS20_State, mpe))); }
...
struct MS20
{
    using PerformerClass = performer::MS20;
    static constexpr auto filename = "MS20.cmajorpatch";
    ...
    static constexpr const char* cmaj_api_cmajpatchconnection_js =
        ...
        "    }\n"
        "}\n";
    static constexpr const char* MS20_cmajorpatch =
        "{\n"
        "  \"CmajorVersion\": 1,\n"
        "  \"ID\": \"com.plugincorp.jwms20\",\n"
        "  \"version\": \"0.1\",\n"
        "  \"name\": \"JW-20M\",\n"
        "  \"description\": \"TOI JW-20M - screaming semi-modular mono by plugin corp\",\n"
        "  \"category\": \"instrument\",\n"
        "  \"manufacturer\": \"TOI\",\n"
        "  \"isInstrument\": true,\n"
    ...
    static constexpr std::array files =
    {
        ...
        File { "MS20.cmajorpatch", std::string_view (MS20_cmajorpatch, 371) },
...
// native faceplate (replaces the generic Cmajor UI). See MS20Native.h.
#include "MS20Native.h"

juce::AudioProcessor* JUCE_CALLTYPE createPluginFilter()
{
    return new Ms20Plugin (std::make_shared<cmaj::Patch>());
}
