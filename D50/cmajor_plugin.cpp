...
// Auto-generated Cmajor code for patch 'JWD50'
...
//==============================================================================
/// Auto-generated C++ class for the 'JWD50' processor
///
...
namespace performer
{
    struct JWD50
    {
        ...
        static constexpr std::string_view name = "JWD50";
        ...
        //==============================================================================
        static constexpr uint32_t numInputEndpoints  = 34;
        ...
        static constexpr uint32_t getEndpointHandleForName (std::string_view endpointName)
        {
            ...
            if (endpointName == "structure")    return static_cast<uint32_t> (EndpointHandles::structure);
            if (endpointName == "attackType")   return static_cast<uint32_t> (EndpointHandles::attackType);
            if (endpointName == "attackTime")   return static_cast<uint32_t> (EndpointHandles::attackTime);
            if (endpointName == "attackLevel")  return static_cast<uint32_t> (EndpointHandles::attackLevel);
            if (endpointName == "susWave")      return static_cast<uint32_t> (EndpointHandles::susWave);
            if (endpointName == "susTune")      return static_cast<uint32_t> (EndpointHandles::susTune);
            if (endpointName == "susFine")      return static_cast<uint32_t> (EndpointHandles::susFine);
            if (endpointName == "susLevel")     return static_cast<uint32_t> (EndpointHandles::susLevel);
            ...
            if (endpointName == "tvfEnv")       return static_cast<uint32_t> (EndpointHandles::tvfEnv);
            if (endpointName == "tvfKbd")       return static_cast<uint32_t> (EndpointHandles::tvfKbd);
            if (endpointName == "fT1")          return static_cast<uint32_t> (EndpointHandles::fT1);
            if (endpointName == "fT2")          return static_cast<uint32_t> (EndpointHandles::fT2);
            if (endpointName == "fLb")          return static_cast<uint32_t> (EndpointHandles::fLb);
            if (endpointName == "fT3")          return static_cast<uint32_t> (EndpointHandles::fT3);
            if (endpointName == "fLs")          return static_cast<uint32_t> (EndpointHandles::fLs);
            if (endpointName == "fR")           return static_cast<uint32_t> (EndpointHandles::fR);
            if (endpointName == "aT1")          return static_cast<uint32_t> (EndpointHandles::aT1);
            if (endpointName == "aT2")          return static_cast<uint32_t> (EndpointHandles::aT2);
            if (endpointName == "aLb")          return static_cast<uint32_t> (EndpointHandles::aLb);
            if (endpointName == "aT3")          return static_cast<uint32_t> (EndpointHandles::aT3);
            if (endpointName == "aLs")          return static_cast<uint32_t> (EndpointHandles::aLs);
            ...
            if (endpointName == "lfoDepth")     return static_cast<uint32_t> (EndpointHandles::lfoDepth);
            if (endpointName == "grain")        return static_cast<uint32_t> (EndpointHandles::grain);
            ...
            if (endpointName == "pEnv")         return static_cast<uint32_t> (EndpointHandles::pEnv);
            if (endpointName == "dual")         return static_cast<uint32_t> (EndpointHandles::dual);
        ...
        static constexpr EndpointInfo inputEndpoints[] =
        {
            ...
            { 2,   "structure",    EndpointType::event },
            { 3,   "attackType",   EndpointType::event },
            { 4,   "attackTime",   EndpointType::event },
            { 5,   "attackLevel",  EndpointType::event },
            { 6,   "susWave",      EndpointType::event },
            { 7,   "susTune",      EndpointType::event },
            { 8,   "susFine",      EndpointType::event },
            { 9,   "susLevel",     EndpointType::event },
            { 10,  "cutoff",       EndpointType::event },
            { 11,  "reso",         EndpointType::event },
            { 12,  "tvfEnv",       EndpointType::event },
            { 13,  "tvfKbd",       EndpointType::event },
            { 14,  "fT1",          EndpointType::event },
            { 15,  "fT2",          EndpointType::event },
            { 16,  "fLb",          EndpointType::event },
            { 17,  "fT3",          EndpointType::event },
            { 18,  "fLs",          EndpointType::event },
            { 19,  "fR",           EndpointType::event },
            { 20,  "aT1",          EndpointType::event },
            { 21,  "aT2",          EndpointType::event },
            { 22,  "aLb",          EndpointType::event },
            { 23,  "aT3",          EndpointType::event },
            { 24,  "aLs",          EndpointType::event },
            { 25,  "aR",           EndpointType::event },
            { 26,  "lfoRate",      EndpointType::event },
            { 27,  "lfoDepth",     EndpointType::event },
            { 28,  "grain",        EndpointType::event },
            { 29,  "chorus",       EndpointType::event },
            { 30,  "reverb",       EndpointType::event },
            { 31,  "volume",       EndpointType::event },
            { 32,  "tune",         EndpointType::event },
            { 33,  "pEnv",         EndpointType::event },
            { 34,  "dual",         EndpointType::event }
        };

        static constexpr EndpointInfo outputEndpoints[] =
        {
            { 35,  "audioOut",  EndpointType::stream },
            { 36,  "noteOut",   EndpointType::event  }
        };
        ...
        struct _D50Voice_1_State
        {
            plugincorp_Params params;
            Array<float, 32768> atk;
            bool built = {};
            bool active = {};
            float freq = {};
            float vel = {};
        ...
        struct _D50Chorus_1_State
        {
            Array<float, 8192> bufL;
            Array<float, 8192> bufR;
            int32_t w = {};
            float p1 = {};
            float p2 = {};
            float p3 = {};
        ...
        {
            _D50Chorus_1_State chorus;
            _D50Reverb_1_State reverb;
        ...
        struct _JWD50_State
        {
            int32_t _sessionID = {};
            double _frequency = {};
            Array<_D50Voice_1_State, 6> voices;
        ...
        struct JWD50_eventValue_noteOut
        {
            int32_t frame = {};
            int32_t type = {};
        ...
        struct JWD50_State
        {
            ...
            Array<JWD50_eventValue_noteOut, 32> noteOut;
        };
        ...
        {
            if (endpointHandle == 35) { std::memcpy (reinterpret_cast<char*> (dest), std::addressof (cmajIO.audioOut), 8 * numFramesToCopy); std::memset (reinterpret_cast<char*> (std::addressof (cmajIO.audioOut)), 0, 8 * numFramesToCopy); return; }
        ...
        {
            if (endpointHandle == 36) return (uint32_t) cmajState.noteOut_eventCount;
        ...
        {
            if (endpointHandle == 36) { cmajState.noteOut_eventCount = 0; return; }
        }

        uint32_t getOutputEventType (EndpointHandle endpointHandle, uint32_t index)
        {
            if (endpointHandle == 36) return (uint32_t) cmajState.noteOut[(IndexType) index].type;
            throw std::runtime_error ("Unknown event endpointHandle:" + std::to_string (endpointHandle));
            return {};
        }
        ...
        static uint32_t getOutputEventDataSize (EndpointHandle endpointHandle, uint32_t typeIndex)
        {
            ...
            if (endpointHandle == 36 && typeIndex == 0) return 8;
            throw std::runtime_error ("Unknown event endpointHandle:" + std::to_string (endpointHandle));
            return 0;
        }
        ...
        void addEvent_structure (float event)
        {
            _sendEvent_structure (cmajState, event);
        }

        void addEvent_attackType (float event)
        {
            _sendEvent_attackType (cmajState, event);
        }

        void addEvent_attackTime (float event)
        {
            _sendEvent_attackTime (cmajState, event);
        }

        void addEvent_attackLevel (float event)
        {
            _sendEvent_attackLevel (cmajState, event);
        }

        void addEvent_susWave (float event)
        {
            _sendEvent_susWave (cmajState, event);
        }

        void addEvent_susTune (float event)
        {
            _sendEvent_susTune (cmajState, event);
        }

        void addEvent_susFine (float event)
        {
            _sendEvent_susFine (cmajState, event);
        }

        void addEvent_susLevel (float event)
        {
            _sendEvent_susLevel (cmajState, event);
        }
        ...
        void addEvent_tvfEnv (float event)
        {
            _sendEvent_tvfEnv (cmajState, event);
        }

        void addEvent_tvfKbd (float event)
        {
            _sendEvent_tvfKbd (cmajState, event);
        }

        void addEvent_fT1 (float event)
        {
            _sendEvent_fT1 (cmajState, event);
        }

        void addEvent_fT2 (float event)
        {
            _sendEvent_fT2 (cmajState, event);
        }

        void addEvent_fLb (float event)
        {
            _sendEvent_fLb (cmajState, event);
        }

        void addEvent_fT3 (float event)
        {
            _sendEvent_fT3 (cmajState, event);
        }

        void addEvent_fLs (float event)
        {
            _sendEvent_fLs (cmajState, event);
        }

        void addEvent_fR (float event)
        {
            _sendEvent_fR (cmajState, event);
        }

        void addEvent_aT1 (float event)
        {
            _sendEvent_aT1 (cmajState, event);
        }

        void addEvent_aT2 (float event)
        {
            _sendEvent_aT2 (cmajState, event);
        }

        void addEvent_aLb (float event)
        {
            _sendEvent_aLb (cmajState, event);
        }

        void addEvent_aT3 (float event)
        {
            _sendEvent_aT3 (cmajState, event);
        }

        void addEvent_aLs (float event)
        {
            _sendEvent_aLs (cmajState, event);
        }
        ...
        void addEvent_lfoDepth (float event)
        {
            _sendEvent_lfoDepth (cmajState, event);
        }

        void addEvent_grain (float event)
        {
            _sendEvent_grain (cmajState, event);
        }
        ...
        void addEvent_pEnv (float event)
        {
            _sendEvent_pEnv (cmajState, event);
        }

        void addEvent_dual (float event)
        {
            _sendEvent_dual (cmajState, event);
        }
        ...
        void addEvent (EndpointHandle endpointHandle, uint32_t typeIndex, const unsigned char* eventData)
        {
            ...
            if (endpointHandle == 2)
            {
                ...
                eventData += 4;
                return addEvent_structure (value);
            }
            ...
            if (endpointHandle == 3)
            {
                ...
                memcpy (&value, eventData, 4);
                eventData += 4;
                return addEvent_attackType (value);
            }
            ...
            if (endpointHandle == 4)
            {
                ...
                memcpy (&value, eventData, 4);
                eventData += 4;
                return addEvent_attackTime (value);
            }
            ...
            if (endpointHandle == 5)
            {
                ...
                memcpy (&value, eventData, 4);
                eventData += 4;
                return addEvent_attackLevel (value);
            }
            ...
            if (endpointHandle == 9)
            {
                ...
                memcpy (&value, eventData, 4);
                eventData += 4;
                return addEvent_susLevel (value);
            }
            ...
            if (endpointHandle == 27)
            {
                ...
                memcpy (&value, eventData, 4);
                eventData += 4;
                return addEvent_lfoDepth (value);
            }
        ...
        JWD50_State cmajState = {};
        JWD50_IO cmajIO = {};

        //==============================================================================
        void _sendEvent_midiIn (JWD50_State& _state, std_midi_Message value) noexcept
        {
            _JWD50__midiIn (_state._state, value);
        }

        void _JWD50__midiIn (_JWD50_State& _state, std_midi_Message value) noexcept
        {
        ...
        {
            _JWD50___forwardEvent (state_upcast_struct_JWD50_State_struc_NywlWb(_state), value);
            _JWD50___forwardEvent_6 (state_upcast_struct_JWD50_State_struc_NywlWb(_state), value);
        }

        void _JWD50___forwardEvent (_JWD50_State& _state, std_notes_NoteOn value) noexcept
        {
        ...
        {
            _JWD50___forwardEvent_3 (state_upcast_struct_JWD50_State_struc_Cjt4te(_state), value);
        }

        void _JWD50___forwardEvent_3 (_JWD50_State& _state, plugincorp_NoteEvent value) noexcept
        {
            _JWD50___writeEvent_noteOut (_state, value);
        }

        void _JWD50___writeEvent_noteOut (_JWD50_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
            JWD50___writeEvent_noteOut (state_upcast_struct_JWD50_State_struct_Zu4RG(_state), value);
        }

        void JWD50___writeEvent_noteOut (JWD50_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
            JWD50___writeEvent_noteOut_0 (_state, value);
        }

        void JWD50___writeEvent_noteOut_0 (JWD50_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
        ...
        void _JWD50___forwardEvent_6 (_JWD50_State& _state, std_notes_NoteOn value) noexcept
        {
        ...
        {
            _JWD50___forwardEvent_13 (state_upcast_struct_JWD50_State_struc_RaRSs(_state), index, value);
        }

        void _JWD50___forwardEvent_13 (_JWD50_State& _state, int32_t index, std_notes_NoteOff value) noexcept
        {
            D50Voice__eventsIn (_state.voices[index], value);
        }

        void D50Voice__eventsIn (_D50Voice_1_State& _state, std_notes_NoteOff e) noexcept
        {
        ...
        {
            _JWD50___forwardEvent_12 (state_upcast_struct_JWD50_State_struc_RaRSs(_state), index, value);
        }

        void _JWD50___forwardEvent_12 (_JWD50_State& _state, int32_t index, std_notes_NoteOn value) noexcept
        {
            D50Voice__eventsIn_0 (_state.voices[index], value);
        }

        void D50Voice__eventsIn_0 (_D50Voice_1_State& _state, std_notes_NoteOn e) noexcept
        {
        ...
        {
            _JWD50___forwardEvent_2 (state_upcast_struct_JWD50_State_struc_NywlWb(_state), value);
            _JWD50___forwardEvent_7 (state_upcast_struct_JWD50_State_struc_NywlWb(_state), value);
        }

        void _JWD50___forwardEvent_2 (_JWD50_State& _state, std_notes_NoteOff value) noexcept
        {
        ...
        void _JWD50___forwardEvent_7 (_JWD50_State& _state, std_notes_NoteOff value) noexcept
        {
        ...
        {
            _JWD50___forwardEvent_8 (state_upcast_struct_JWD50_State_struc_NywlWb(_state), value);
        }

        void _JWD50___forwardEvent_8 (_JWD50_State& _state, std_notes_PitchBend value) noexcept
        {
        ...
        {
            _JWD50___forwardEvent_10 (state_upcast_struct_JWD50_State_struc_NywlWb(_state), value);
        }

        void _JWD50___forwardEvent_10 (_JWD50_State& _state, std_notes_Pressure value) noexcept
        {
        ...
        {
            _JWD50___forwardEvent_9 (state_upcast_struct_JWD50_State_struc_NywlWb(_state), value);
        }

        void _JWD50___forwardEvent_9 (_JWD50_State& _state, std_notes_Slide value) noexcept
        {
        ...
        {
            _JWD50___forwardEvent_11 (state_upcast_struct_JWD50_State_struc_NywlWb(_state), value);
        }

        void _JWD50___forwardEvent_11 (_JWD50_State& _state, std_notes_Control value) noexcept
        {
        ...
        void _sendEvent_structure (JWD50_State& _state, float value) noexcept
        {
            _JWD50__structure (_state._state, value);
        }

        void _JWD50__structure (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__structure (_state.paramsProcessor, value);
        }

        void ParamsProcessor__structure (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.structure = v;
        ...
        {
            _JWD50___forwardEvent_4 (state_upcast_struct_JWD50_State_struc_ITdCWc(_state), value);
            _JWD50___forwardEvent_5 (state_upcast_struct_JWD50_State_struc_ITdCWc(_state), value);
        }

        void _JWD50___forwardEvent_4 (_JWD50_State& _state, plugincorp_Params value) noexcept
        {
            int32_t  i;

            {
                ...
                for (;;)
                {
                    ...
                    D50Voice__paramsIn (_state.voices[i], value);
                    {
                        ++i;
                    }
                }
        ...
        void D50Voice__paramsIn (_D50Voice_1_State& _state, plugincorp_Params p) noexcept
        {
            _state.params = p;
        }

        void _JWD50___forwardEvent_5 (_JWD50_State& _state, plugincorp_Params value) noexcept
        {
        ...
        void _sendEvent_attackType (JWD50_State& _state, float value) noexcept
        {
            _JWD50__attackType (_state._state, value);
        }

        void _JWD50__attackType (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__attackType (_state.paramsProcessor, value);
        }

        void ParamsProcessor__attackType (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.attackType = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_attackTime (JWD50_State& _state, float value) noexcept
        {
            _JWD50__attackTime (_state._state, value);
        }

        void _JWD50__attackTime (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__attackTime (_state.paramsProcessor, value);
        }

        void ParamsProcessor__attackTime (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.attackTime = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_attackLevel (JWD50_State& _state, float value) noexcept
        {
            _JWD50__attackLevel (_state._state, value);
        }

        void _JWD50__attackLevel (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__attackLevel (_state.paramsProcessor, value);
        }

        void ParamsProcessor__attackLevel (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.attackLevel = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_susWave (JWD50_State& _state, float value) noexcept
        {
            _JWD50__susWave (_state._state, value);
        }

        void _JWD50__susWave (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__susWave (_state.paramsProcessor, value);
        }

        void ParamsProcessor__susWave (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.susWave = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_susTune (JWD50_State& _state, float value) noexcept
        {
            _JWD50__susTune (_state._state, value);
        }

        void _JWD50__susTune (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__susTune (_state.paramsProcessor, value);
        }

        void ParamsProcessor__susTune (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.susTune = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_susFine (JWD50_State& _state, float value) noexcept
        {
            _JWD50__susFine (_state._state, value);
        }

        void _JWD50__susFine (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__susFine (_state.paramsProcessor, value);
        }

        void ParamsProcessor__susFine (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.susFine = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_susLevel (JWD50_State& _state, float value) noexcept
        {
            _JWD50__susLevel (_state._state, value);
        }

        void _JWD50__susLevel (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__susLevel (_state.paramsProcessor, value);
        }

        void ParamsProcessor__susLevel (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.susLevel = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_cutoff (JWD50_State& _state, float value) noexcept
        {
            _JWD50__cutoff (_state._state, value);
        }

        void _JWD50__cutoff (_JWD50_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_reso (JWD50_State& _state, float value) noexcept
        {
            _JWD50__reso (_state._state, value);
        }

        void _JWD50__reso (_JWD50_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_tvfEnv (JWD50_State& _state, float value) noexcept
        {
            _JWD50__tvfEnv (_state._state, value);
        }

        void _JWD50__tvfEnv (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__tvfEnv (_state.paramsProcessor, value);
        }

        void ParamsProcessor__tvfEnv (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.tvfEnv = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_tvfKbd (JWD50_State& _state, float value) noexcept
        {
            _JWD50__tvfKbd (_state._state, value);
        }

        void _JWD50__tvfKbd (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__tvfKbd (_state.paramsProcessor, value);
        }

        void ParamsProcessor__tvfKbd (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.tvfKbd = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_fT1 (JWD50_State& _state, float value) noexcept
        {
            _JWD50__fT1 (_state._state, value);
        }

        void _JWD50__fT1 (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__fT1 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__fT1 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.fT1 = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_fT2 (JWD50_State& _state, float value) noexcept
        {
            _JWD50__fT2 (_state._state, value);
        }

        void _JWD50__fT2 (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__fT2 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__fT2 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.fT2 = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_fLb (JWD50_State& _state, float value) noexcept
        {
            _JWD50__fLb (_state._state, value);
        }

        void _JWD50__fLb (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__fLb (_state.paramsProcessor, value);
        }

        void ParamsProcessor__fLb (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.fLb = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_fT3 (JWD50_State& _state, float value) noexcept
        {
            _JWD50__fT3 (_state._state, value);
        }

        void _JWD50__fT3 (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__fT3 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__fT3 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.fT3 = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_fLs (JWD50_State& _state, float value) noexcept
        {
            _JWD50__fLs (_state._state, value);
        }

        void _JWD50__fLs (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__fLs (_state.paramsProcessor, value);
        }

        void ParamsProcessor__fLs (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.fLs = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_fR (JWD50_State& _state, float value) noexcept
        {
            _JWD50__fR (_state._state, value);
        }

        void _JWD50__fR (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__fR (_state.paramsProcessor, value);
        }

        void ParamsProcessor__fR (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.fR = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_aT1 (JWD50_State& _state, float value) noexcept
        {
            _JWD50__aT1 (_state._state, value);
        }

        void _JWD50__aT1 (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__aT1 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__aT1 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.aT1 = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_aT2 (JWD50_State& _state, float value) noexcept
        {
            _JWD50__aT2 (_state._state, value);
        }

        void _JWD50__aT2 (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__aT2 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__aT2 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.aT2 = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_aLb (JWD50_State& _state, float value) noexcept
        {
            _JWD50__aLb (_state._state, value);
        }

        void _JWD50__aLb (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__aLb (_state.paramsProcessor, value);
        }

        void ParamsProcessor__aLb (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.aLb = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_aT3 (JWD50_State& _state, float value) noexcept
        {
            _JWD50__aT3 (_state._state, value);
        }

        void _JWD50__aT3 (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__aT3 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__aT3 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.aT3 = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_aLs (JWD50_State& _state, float value) noexcept
        {
            _JWD50__aLs (_state._state, value);
        }

        void _JWD50__aLs (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__aLs (_state.paramsProcessor, value);
        }

        void ParamsProcessor__aLs (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.aLs = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_aR (JWD50_State& _state, float value) noexcept
        {
            _JWD50__aR (_state._state, value);
        }

        void _JWD50__aR (_JWD50_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_lfoRate (JWD50_State& _state, float value) noexcept
        {
            _JWD50__lfoRate (_state._state, value);
        }

        void _JWD50__lfoRate (_JWD50_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_lfoDepth (JWD50_State& _state, float value) noexcept
        {
            _JWD50__lfoDepth (_state._state, value);
        }

        void _JWD50__lfoDepth (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__lfoDepth (_state.paramsProcessor, value);
        }

        void ParamsProcessor__lfoDepth (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.lfoDepth = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_grain (JWD50_State& _state, float value) noexcept
        {
            _JWD50__grain (_state._state, value);
        }

        void _JWD50__grain (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__grain (_state.paramsProcessor, value);
        }

        void ParamsProcessor__grain (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.grain = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_chorus (JWD50_State& _state, float value) noexcept
        {
            _JWD50__chorus (_state._state, value);
        }

        void _JWD50__chorus (_JWD50_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_reverb (JWD50_State& _state, float value) noexcept
        {
            _JWD50__reverb (_state._state, value);
        }

        void _JWD50__reverb (_JWD50_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_volume (JWD50_State& _state, float value) noexcept
        {
            _JWD50__volume (_state._state, value);
        }

        void _JWD50__volume (_JWD50_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_tune (JWD50_State& _state, float value) noexcept
        {
            _JWD50__tune (_state._state, value);
        }

        void _JWD50__tune (_JWD50_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_pEnv (JWD50_State& _state, float value) noexcept
        {
            _JWD50__pEnv (_state._state, value);
        }

        void _JWD50__pEnv (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__pEnv (_state.paramsProcessor, value);
        }

        void ParamsProcessor__pEnv (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.pEnv = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_dual (JWD50_State& _state, float value) noexcept
        {
            _JWD50__dual (_state._state, value);
        }

        void _JWD50__dual (_JWD50_State& _state, float value) noexcept
        {
            ParamsProcessor__dual (_state.paramsProcessor, value);
        }

        void ParamsProcessor__dual (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.dual = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _initialise (JWD50_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            _JWD50___initialise (_state._state, processorID, sessionID, frequency);
        }

        void _JWD50___initialise (_JWD50_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            int32_t  i;
            ...
            g__frequency = frequency;
            {
                ...
                for (;;)
                {
                    ...
                    D50Voice___initialise (_state.voices[i], processorID, sessionID, frequency);
                    {
                        ++i;
                    }
                }
        ...
        void D50Voice___initialise (_D50Voice_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
        ...
        void ParamsProcessor___initialise (_ParamsProcessor_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
            _state.params = plugincorp_Params { 0.0f, 0.43f, 0.4f, 0.6f, 0.2f, 0.5f, 0.52f, 0.8f, 0.6f, 0.2f, 0.4f, 0.4f, 0.3f, 0.5f, 0.6f, 0.5f, 0.5f, 0.4f, 0.18f, 0.5f, 0.8f, 0.5f, 0.7f, 0.5f, 0.3f, 0.1f, 0.3f, 0.6f, 0.5f, 0.8f, 0.5f, 0.5f, 0.0f };
        }
        ...
        {
            D50Chorus___initialise (_state.chorus, processorID, sessionID, frequency);
            D50Reverb___initialise (_state.reverb, processorID, sessionID, frequency);
            FXMixer___initialise (_state.mix, processorID, sessionID, frequency);
        }

        void D50Chorus___initialise (_D50Chorus_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
            _state.p1 = 0.0f;
        ...
        void D50Reverb___initialise (_D50Reverb_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
            _state.s1 = 0.0f;
        ...
        void FXMixer___initialise (_FXMixer_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
            _state.chorusAmt = 0.6f;
            _state.reverbAmt = 0.5f;
        }

        void _advance (JWD50_State& _state, JWD50_IO& _io, int32_t _frames) noexcept
        {
            _JWD50_IO  ioCopy;
        ...
        void main (_JWD50_State& _state, _JWD50_IO& _io) noexcept
        {
            Array<_D50Voice_1_IO, 6>  voices_io;
            ...
            voices_io = Array<_D50Voice_1_IO, 6> {};
            ...
            }
            {
                ...
                for (;;)
                {
                    ...
                    D50Voice__main (_state.voices[i], voices_io[i]);
                    {
                        ++i;
                    }
                }
        ...
        void D50Voice__main (_D50Voice_1_State& _state, _D50Voice_1_IO& _io) noexcept
        {
            float  outL;
            float  outR;
            float  masterTune;
            ...
            if (! _state.built)
            {
                D50Voice__buildAttackBank (_state);
                _state.built = true;
            }
            ...
            _branch0_0:
            {
                for (;;)
                {
                    ...
                    if (_state.active)
                    {
                        ...
                        lfoHz = 0.05f + ((_state.params.lfoRate * _state.params.lfoRate) * 9.0f);
                        _state.lfoPh = (_state.lfoPh + (lfoHz * _state.invSr));
                        if (_state.lfoPh >= 1.0f)
                        {
                            _state.lfoPh = (_state.lfoPh - 1.0f);
                        }
                        lfo = intrinsics::sin (_state.lfoPh * 6.2831855f);
                        aT1r = 1.0f / (intrinsics::max (0.0008f, 0.001f + ((_state.params.aT1 * _state.params.aT1) * 3.5f)) * _state.sr);
                        aT2r = 1.0f / (intrinsics::max (0.0008f, 0.004f + ((_state.params.aT2 * _state.params.aT2) * 6.0f)) * _state.sr);
                        aT3r = 1.0f / (intrinsics::max (0.0008f, 0.004f + ((_state.params.aT3 * _state.params.aT3) * 7.0f)) * _state.sr);
                        aRr = 1.0f / (intrinsics::max (0.0008f, 0.004f + ((_state.params.aR * _state.params.aR) * 8.0f)) * _state.sr);
                        if (_state.aStage == int32_t {0})
                        {
                            _state.ae = (_state.ae + aT1r);
                            if (_state.ae >= 1.0f)
                            {
                                _state.ae = 1.0f;
                                _state.aStage = int32_t {1};
                        ...
                        {
                            if (_state.aStage == int32_t {1})
                            {
                                _state.ae = (_state.ae - aT2r);
                                if (_state.ae <= _state.params.aLb)
                                {
                                    _state.ae = _state.params.aLb;
                                    _state.aStage = int32_t {2};
                                }
                            }
                            else
                            {
                                if (_state.aStage == int32_t {2})
                                {
                                    d = _state.params.aLs - _state.ae;
                                    _state.ae = (_state.ae + ((d > 0.0f) ? aT3r : (- aT3r)));
                                    if (intrinsics::abs (_state.params.aLs - _state.ae) <= aT3r)
                                    {
                                        _state.ae = _state.params.aLs;
                                        _state.aStage = int32_t {3};
                                    }
                                }
                                else
                                {
                                    if (_state.aStage == int32_t {3})
                                    {
                                        _state.ae = _state.params.aLs;
                                    }
                                    else
                                    {
                                        if (_state.aStage == int32_t {4})
                                        {
                                            _state.ae = (_state.ae - aRr);
                                            if (_state.ae <= 0.0f)
                                            {
                                                _state.ae = 0.0f;
                                                _state.aStage = int32_t {-1};
                        ...
                        fT1r = 1.0f / (intrinsics::max (0.0008f, 0.001f + ((_state.params.fT1 * _state.params.fT1) * 3.5f)) * _state.sr);
                        fT2r = 1.0f / (intrinsics::max (0.0008f, 0.004f + ((_state.params.fT2 * _state.params.fT2) * 6.0f)) * _state.sr);
                        fT3r = 1.0f / (intrinsics::max (0.0008f, 0.004f + ((_state.params.fT3 * _state.params.fT3) * 7.0f)) * _state.sr);
                        fRr = 1.0f / (intrinsics::max (0.0008f, 0.004f + ((_state.params.fR * _state.params.fR) * 8.0f)) * _state.sr);
                        if (_state.fStage == int32_t {0})
                        {
                            _state.fe = (_state.fe + fT1r);
                            if (_state.fe >= 1.0f)
                            {
                                _state.fe = 1.0f;
                                _state.fStage = int32_t {1};
                        ...
                        {
                            if (_state.fStage == int32_t {1})
                            {
                                _state.fe = (_state.fe - fT2r);
                                if (_state.fe <= _state.params.fLb)
                                {
                                    _state.fe = _state.params.fLb;
                                    _state.fStage = int32_t {2};
                                }
                            }
                            else
                            {
                                if (_state.fStage == int32_t {2})
                                {
                                    d_0 = _state.params.fLs - _state.fe;
                                    _state.fe = (_state.fe + ((d_0 > 0.0f) ? fT3r : (- fT3r)));
                                    if (intrinsics::abs (_state.params.fLs - _state.fe) <= fT3r)
                                    {
                                        _state.fe = _state.params.fLs;
                                        _state.fStage = int32_t {3};
                                    }
                                }
                                else
                                {
                                    if (_state.fStage == int32_t {3})
                                    {
                                        _state.fe = _state.params.fLs;
                                    }
                                    else
                                    {
                                        if (_state.fStage == int32_t {4})
                                        {
                                            _state.fe = (_state.fe - fRr);
                                            if (_state.fe <= 0.0f)
                                            {
                                                _state.fe = 0.0f;
                                                _state.fStage = int32_t {-1};
                        ...
                        octS = intrinsics::pow (2.0f, intrinsics::floor ((_state.params.susTune * 4.0f) + 0.5f) - 2.0f);
                        fineS = intrinsics::pow (2.0f, (_state.params.susFine - 0.5f) * 0.18f);
                        pitchMod = intrinsics::pow (2.0f, (lfo * _state.params.lfoDepth) * 0.04f);
                        pEnvOct = ((_state.params.pEnv - 0.5f) * 3.0f) * intrinsics::exp ((- _state.noteT) * 5.5f);
                        scoopMul = intrinsics::pow (2.0f, pEnvOct);
                        fS = intrinsics::clamp (((((_state.freq * masterTune) * octS) * fineS) * pitchMod) * scoopMul, 0.0f, _state.sr * 0.49f);
                        incS = fS * _state.invSr;
                        _state.susPh = (_state.susPh + incS);
                        if (_state.susPh >= 1.0f)
                        {
                            _state.susPh = (_state.susPh - 1.0f);
                        }
                        sus = D50Voice__susOsc (_state.susPh, incS, _state.params.susWave) * _state.params.susLevel;
                        if (_state.params.dual > 0.001f)
                        {
                            incS2 = intrinsics::clamp (fS * (1.0f + (_state.params.dual * 0.013f)), 0.0f, _state.sr * 0.49f) * _state.invSr;
                            _state.susPh2 = (_state.susPh2 + incS2);
                            if (_state.susPh2 >= 1.0f)
                            {
                                _state.susPh2 = (_state.susPh2 - 1.0f);
                            }
                            sus = (sus + (((D50Voice__susOsc (_state.susPh2, incS2, _state.params.susWave) * _state.params.susLevel) * _state.params.dual) * 0.9f));
                        }
                        att = 0.0f;
                        if (_state.atkOn)
                        {
                            ty = static_cast<int32_t> (intrinsics::floor ((_state.params.attackType * 7.0f) + 0.5f));
                            scoop = 1.0f + (0.35f * intrinsics::exp ((- _state.atkElapsed) / 0.018f));
                            pitchRatio = ((_state.freq / g_refHz) * scoop) * scoopMul;
                            att = (D50Voice__readAtk (_state, ty, _state.atkPos) * _state.params.attackLevel);
                            decayMul = intrinsics::exp ((- _state.atkElapsed) / intrinsics::max (0.02f, _state.params.attackTime * 0.6f));
                            att = (att * decayMul);
                            _state.atkPos = (_state.atkPos + pitchRatio);
                            _state.atkElapsed = (_state.atkElapsed + _state.invSr);
                            if ((_state.atkPos >= 4095.0f) ? true : (decayMul < 0.0008f))
                            {
                                _state.atkOn = false;
                            }
                        }
                        susA = sus * _state.ae;
                        st = static_cast<int32_t> (intrinsics::floor ((_state.params.structure * 2.0f) + 0.5f));
                        ring = (att * sus) * 2.0f;
                        filtIn = susA;
                        dryAtt = att;
                        tvfMod = 0.0f;
                        ...
                        else
                        {
                            if (st == int32_t {2})
                            {
                                tvfMod = (intrinsics::abs (att) * 1.6f);
                            }
                        }
                        ...
                        _state.dcy = yb;
                        filtIn = yb;
                        kbd = 1.0f + (_state.params.tvfKbd * ((_state.freq / g_refHz) - 1.0f));
                        envOct = ((_state.params.tvfEnv * _state.fe) * 6.0f) + tvfMod;
                        lfoOct = (lfo * _state.params.lfoDepth) * 1.5f;
                        fcHz = intrinsics::clamp ((40.0f * intrinsics::pow (2.0f, ((_state.params.cutoff * 8.0f) + envOct) + lfoOct)) * kbd, 30.0f, _state.sr * 0.45f);
                        gg = intrinsics::tan ((3.1415927f * fcHz) * _state.invSr);
                        kq = 1.0f / (0.5f + ((_state.params.reso * _state.params.reso) * 8.0f));
                        a1 = 1.0f / (1.0f + (gg * (gg + kq)));
                        a2 = gg * a1;
                        a3 = gg * a2;
                        v3 = filtIn - _state.ic2;
                        ...
                        lp = v2;
                        sig = lp + dryAtt;
                        if (_state.params.grain > 0.001f)
                        {
                            steps = 4096.0f - (_state.params.grain * 3960.0f);
                            crushed = intrinsics::floor ((sig * steps) + 0.5f) / steps;
                            srDiv = static_cast<int32_t> (1.0f + (_state.params.grain * 5.0f));
                            if (intrinsics::modulo (_state.rngc, srDiv) == int32_t {0})
                            {
                                _state.held = crushed;
                            }
                            ++_state.rngc;
                            sig = ((sig * (1.0f - _state.params.grain)) + (_state.held * _state.params.grain));
                        }
                        amp = (sig * (0.4f + (0.6f * _state.vel))) * (0.35f + (_state.params.volume * 1.0f));
                        o = intrinsics::tanh (amp * 1.15f);
                        outL = o;
                        outR = o;
                        ...
                        if ((_state.aStage == int32_t {-1}) ? (! _state.atkOn) : false)
                        {
                            _state.active = false;
                        }
                    }
        ...
        void D50Voice__buildAttackBank (_D50Voice_1_State& _state) noexcept
        {
            float  refHz;
            float  sr;
            float  dur;
                ...
                for (;;)
                {
                    ...
                    lpB = 0.0f;
                    {
                        ...
                        for (;;)
                        {
                            ...
                            n = nn;
                            frac = static_cast<float> (n) / 4096.0f;
                            secs = static_cast<float> (n) / sr;
                            ph = secs * refHz;
                            s = 0.0f;
                            if (t == int32_t {0})
                            {
                                ...
                                r2 = 5.4f;
                                r3 = 8.93f;
                                s = ((((intrinsics::sin ((6.2831855f * ph) * r0) * intrinsics::exp ((- secs) * 6.0f)) + ((0.6f * intrinsics::sin ((6.2831855f * ph) * r1)) * intrinsics::exp ((- secs) * 9.0f))) + ((0.4f * intrinsics::sin ((6.2831855f * ph) * r2)) * intrinsics::exp ((- secs) * 14.0f))) + ((0.25f * intrinsics::sin ((6.2831855f * ph) * r3)) * intrinsics::exp ((- secs) * 20.0f)));
                            }
                            else
                            {
                                if (t == int32_t {1})
                                {
                                    {
                                        ...
                                        for (;;)
                                        {
                                            ...
                                            s = (s + ((1.0f / intrinsics::sqrt (k)) * intrinsics::sin ((6.2831855f * ph) * k)));
                                            {
                                                ++kk;
                                            }
                                        }
                                    }
                                    s = (((s * 0.4f) + ((D50Voice__rnd (_state) * intrinsics::exp ((- secs) * 200.0f)) * 0.8f)) * intrinsics::exp ((- secs) * 34.0f));
                                }
                                else
                                {
                                    if (t == int32_t {2})
                                    {
                                        wnz = D50Voice__rnd (_state);
                                        lpA = (lpA + (0.3f * (wnz - lpA)));
                                        lpB = (lpB + (0.05f * (lpA - lpB)));
                                        band = lpA - lpB;
                                        onset = 1.0f - intrinsics::exp ((- secs) * 55.0f);
                                        s = (((band * onset) * intrinsics::exp ((- secs) * 9.0f)) * 3.4f);
                                    }
                                    else
                                    {
                                        if (t == int32_t {3})
                                        {
                                            {
                                                ...
                                                for (;;)
                                                {
                                                    ...
                                                    f1 = intrinsics::exp ((-0.5f * ((k_0 - 2.7f) / 1.1f)) * ((k_0 - 2.7f) / 1.1f));
                                                    f2 = intrinsics::exp ((-0.5f * ((k_0 - 4.2f) / 1.6f)) * ((k_0 - 4.2f) / 1.6f)) * 0.8f;
                                                    f3 = intrinsics::exp ((-0.5f * ((k_0 - 10.0f) / 3.2f)) * ((k_0 - 10.0f) / 3.2f)) * 0.4f;
                                                    s = (s + ((((f1 + f2) + f3) * (1.0f / intrinsics::sqrt (k_0))) * intrinsics::sin ((6.2831855f * ph) * k_0)));
                                                    {
                                                        ++kk_0;
                                                    }
                                                }
                                            }
                                            s = (s * ((1.0f - intrinsics::exp ((- secs) * 45.0f)) * intrinsics::exp ((- secs) * 7.0f)));
                                        }
                                        else
                                        {
                                            if (t == int32_t {4})
                                            {
                                                nz = ((nz * 0.5f) + (D50Voice__rnd (_state) * 0.5f));
                                                s = (((intrinsics::sin (6.2831855f * ph) * 0.5f) + nz) * intrinsics::exp ((- secs) * 60.0f));
                                            }
                                            else
                                            {
                                                if (t == int32_t {5})
                                                {
                                                    s = (((intrinsics::sin (6.2831855f * ph) * intrinsics::exp ((- secs) * 9.0f)) + ((0.5f * intrinsics::sin ((6.2831855f * ph) * 2.0f)) * intrinsics::exp ((- secs) * 16.0f))) + ((0.3f * intrinsics::sin ((6.2831855f * ph) * 3.01f)) * intrinsics::exp ((- secs) * 24.0f)));
                                                }
                                                else
                                                {
                                                    if (t == int32_t {6})
                                                    {
                                                        nz = ((nz * 0.9f) + (D50Voice__rnd (_state) * 0.1f));
                                                        tone = intrinsics::sin (6.2831855f * ph) + (0.4f * intrinsics::sin ((6.2831855f * ph) * 2.0f));
                                                        s = ((((tone * 0.6f) + (nz * 0.8f)) * (1.0f - intrinsics::exp ((- secs) * 30.0f))) * intrinsics::exp ((- secs) * 6.0f));
                                                    }
                                                    else
                                                    {
                                                        {
                                                            ...
                                                            for (;;)
                                                            {
                                                                ...
                                                                k_1 = static_cast<float> (kk_1 + int32_t {5});
                                                                s = (s + ((1.0f / k_1) * intrinsics::sin (((6.2831855f * ph) * k_1) * (1.0f + (0.002f * k_1)))));
                                                                {
                                                                    ++kk_1;
                                                                }
                                                            }
                                                        }
                                                        s = (s * (intrinsics::exp ((- secs) * 8.0f) * (0.6f + (0.4f * frac))));
                                                    }
                                                }
                                            }
                                        }
                            ...
                            _state.atk[std__intrinsics___wrap_32768 ((t * g_AL) + n)] = s;
                            a = intrinsics::abs (s);
                            if (a > maxv)
                            {
                                maxv = a;
                    ...
                    inv = 1.0f / maxv;
                    {
                        ...
                        for (;;)
                        {
                            if (nn_0 >= int32_t {4096})
                            {
                                break;
                            }
                            {
                                t_0 = std__intrinsics___wrap_32768 ((t * g_AL) + nn_0);
                                _state.atk[t_0] = (_state.atk[t_0] * inv);
                            }
                            {
                                ++nn_0;
                            }
        ...
        float D50Voice__rnd (_D50Voice_1_State& _state) noexcept
        {
            float  s;
        ...
        int32_t std__intrinsics___wrap_32768 (int32_t n) noexcept
        {
            return n & int32_t {32767};
        }
        ...
        float D50Voice__susOsc (float p, float inc, float wave) noexcept
        {
            float  saw;
            float  p2;
            float  saw2;
            ...
            saw3 = ((2.0f * p3) - 1.0f) - toi__dsp__polyBlep (p3, inc);
            pulse = saw - saw3;
            digital = ((0.6f * intrinsics::sin ((6.2831855f * p) * 5.0f)) + (0.4f * intrinsics::sin ((6.2831855f * p) * 9.0f))) + (0.3f * intrinsics::sin ((6.2831855f * p) * 13.0f));
            if (wave < 0.34f)
            {
                m = wave / 0.34f;
                return (saw * (1.0f - m)) + (sq * m);
            }
            else
            {
                if (wave < 0.67f)
                {
                    m_0 = (wave - 0.34f) / 0.33f;
                    return (sq * (1.0f - m_0)) + ((pulse * 0.7f) * m_0);
                }
                else
                {
                    m_1 = (wave - 0.67f) / 0.33f;
                    return ((pulse * 0.7f) * (1.0f - m_1)) + ((digital * 0.8f) * m_1);
                }
            }
        }
        ...
        float D50Voice__readAtk (_D50Voice_1_State& _state, int32_t ty, float pos) noexcept
        {
            int32_t  i0;
            float  fr;
            float  a;
            ...
            if ((pos < 0.0f) ? true : (pos >= 4095.0f))
            {
                return 0.0f;
            }
            i0 = static_cast<int32_t> (pos);
            fr = pos - static_cast<float> (i0);
            a = _state.atk[std__intrinsics___wrap_32768 ((ty * g_AL) + i0)];
            b = _state.atk[std__intrinsics___wrap_32768 (((ty * g_AL) + i0) + int32_t {1})];
        ...
        void main_4 (_FXBus_1_State& _state, _FXBus_1_IO& _io) noexcept
        {
            ...
            D50Chorus__main (_state.chorus, chorus_io);
            {
                reverb_io.audioIn = (reverb_io.audioIn + chorus_io.audioOut);
            }
            D50Reverb__main (_state.reverb, reverb_io);
            {
        ...
        void D50Chorus__main (_D50Chorus_1_State& _state, _FXBus_1_IO& _io) noexcept
        {
            float  sr;
            float  m1;
            float  m2;
            ...
            _state.i3 = (0.37f / sr);
            _state.i4 = (0.58f / sr);
            _state.baseD = (0.009f * sr);
            _state.dep = (0.0034f * sr);
            _branch0_0:
            {
                for (;;)
                {
                    ...
                    m4 = intrinsics::sin (_state.p4 * 6.2831855f);
                    wetL = (D50Chorus__rdL (_state, _state.baseD + (_state.dep * m1)) + D50Chorus__rdL (_state, (_state.baseD * 1.4f) + (_state.dep * m3))) * 0.5f;
                    wetR = (D50Chorus__rdR (_state, _state.baseD + (_state.dep * m2)) + D50Chorus__rdR (_state, (_state.baseD * 1.45f) - (_state.dep * m4))) * 0.5f;
                    _state.p1 = (_state.p1 + _state.i1);
                    if (_state.p1 >= 1.0f)
                    {
                        _state.p1 = (_state.p1 - 1.0f);
                    }
                    _state.p2 = (_state.p2 + _state.i2);
                    if (_state.p2 >= 1.0f)
                    {
                        _state.p2 = (_state.p2 - 1.0f);
                    }
                    _state.p3 = (_state.p3 + _state.i3);
                    if (_state.p3 >= 1.0f)
                    {
                        _state.p3 = (_state.p3 - 1.0f);
                    }
                    _state.p4 = (_state.p4 + _state.i4);
                    if (_state.p4 >= 1.0f)
                    {
                        _state.p4 = (_state.p4 - 1.0f);
                    }
                    std__intrinsics___wrapped_post_inc_8192 (_state.w);
                    _temp[int32_t {0}] = ((_io.audioIn[int32_t {0}] * 0.7f) + (wetL * 0.7f));
                    _temp[int32_t {1}] = ((_io.audioIn[int32_t {1}] * 0.7f) + (wetR * 0.7f));
        ...
        float D50Chorus__rdL (_D50Chorus_1_State& _state, float d) noexcept
        {
            float  rp;
            int32_t  i0;
            int32_t  i1;
            ...
            i0 = std__intrinsics___wrap_8192 (static_cast<int32_t> (rp));
            i1 = std__intrinsics___wrap_8192 (static_cast<int32_t> (rp) + int32_t {1});
            fr = rp - intrinsics::floor (rp);
            return (_state.bufL[i0] * (1.0f - fr)) + (_state.bufL[i1] * fr);
        }

        int32_t std__intrinsics___wrap_8192 (int32_t n) noexcept
        {
            return n & int32_t {8191};
        }

        float D50Chorus__rdR (_D50Chorus_1_State& _state, float d) noexcept
        {
            float  rp;
            int32_t  i0;
            int32_t  i1;
        ...
        int32_t std__intrinsics___wrapped_post_inc_8192 (int32_t& n) noexcept
        {
            int32_t  result;

            result = n;
            n = std__intrinsics___wrap_8192 (n + int32_t {1});
            return result;
        }

        void D50Reverb__main (_D50Reverb_1_State& _state, _FXBus_1_IO& _io) noexcept
        {
            float  inp;
            float  y1;
            float  y2;
            ...
            _branch0_0:
            {
                for (;;)
                {
                    inp = ((_io.audioIn[int32_t {0}] + _io.audioIn[int32_t {1}]) * 0.5f) * 0.028f;
                    y1 = _state.c1[_state.p1];
                    _state.s1 = ((y1 * 0.65999997f) + (_state.s1 * g_damp));
                    _state.c1[_state.p1] = (inp + (_state.s1 * g_fb));
                    std__intrinsics___wrapped_post_inc_1687 (_state.p1);
                    y2 = _state.c2[_state.p2];
                    _state.s2 = ((y2 * 0.65999997f) + (_state.s2 * g_damp));
                    _state.c2[_state.p2] = (inp + (_state.s2 * g_fb));
                    std__intrinsics___wrapped_post_inc_1601 (_state.p2);
                    y3 = _state.c3[_state.p3];
                    _state.s3 = ((y3 * 0.65999997f) + (_state.s3 * g_damp));
                    _state.c3[_state.p3] = (inp + (_state.s3 * g_fb));
                    std__intrinsics___wrapped_post_inc_2053 (_state.p3);
                    y4 = _state.c4[_state.p4];
                    _state.s4 = ((y4 * 0.65999997f) + (_state.s4 * g_damp));
                    _state.c4[_state.p4] = (inp + (_state.s4 * g_fb));
                    std__intrinsics___wrapped_post_inc_2251 (_state.p4);
                    x = ((y1 + y2) + y3) + y4;
                    ba = _state.apA[_state.qa];
                    _state.apA[_state.qa] = (x + (ba * 0.5f));
                    x = ((- x) + ba);
                    std__intrinsics___wrapped_post_inc_389 (_state.qa);
                    bb = _state.apB[_state.qb];
                    _state.apB[_state.qb] = (x + (bb * 0.5f));
                    x = ((- x) + bb);
                    std__intrinsics___wrapped_post_inc_307 (_state.qb);
                    _state.modd[_state.mp] = x;
                    _state.lfo = (_state.lfo + (0.5f / _state.sr));
                    if (_state.lfo >= 1.0f)
                    {
                        _state.lfo = (_state.lfo - 1.0f);
                    }
                    md = 9.0f + (7.0f * intrinsics::sin (_state.lfo * 6.2831855f));
                    rp = static_cast<float> (_state.mp) - md;
                    if (rp < 0.0f)
                    {
                        rp = (rp + 1024.0f);
                    }
                    i0 = std__intrinsics___wrap_1024 (static_cast<int32_t> (rp));
                    i1 = std__intrinsics___wrap_1024 (static_cast<int32_t> (rp) + int32_t {1});
                    fr = rp - intrinsics::floor (rp);
                    flg = (_state.modd[i0] * (1.0f - fr)) + (_state.modd[i1] * fr);
                    std__intrinsics___wrapped_post_inc_1024 (_state.mp);
                    _temp[int32_t {0}] = ((x + (flg * 0.45f)) * 0.42f);
                    _temp[int32_t {1}] = (((x * 0.92f) - (flg * 0.4f)) * 0.42f);
                    _io.audioOut = (_io.audioOut + _temp);
                    _state._resumeIndex = int32_t {1};
                    return;
                }
        ...
        int32_t std__intrinsics___wrapped_post_inc_1687 (int32_t& n) noexcept
        {
            int32_t  result;

            result = n;
            n = std__intrinsics___wrap_1687 (n + int32_t {1});
            return result;
        }

        int32_t std__intrinsics___wrap_1687 (int32_t n) noexcept
        {
            int32_t  x;

            x = intrinsics::modulo (n, int32_t {1687});
            return (x < int32_t {0}) ? (x + int32_t {1687}) : x;
        }

        int32_t std__intrinsics___wrapped_post_inc_1601 (int32_t& n) noexcept
        {
            int32_t  result;

            result = n;
            n = std__intrinsics___wrap_1601 (n + int32_t {1});
            return result;
        }

        int32_t std__intrinsics___wrap_1601 (int32_t n) noexcept
        {
            int32_t  x;

            x = intrinsics::modulo (n, int32_t {1601});
            return (x < int32_t {0}) ? (x + int32_t {1601}) : x;
        }

        int32_t std__intrinsics___wrapped_post_inc_2053 (int32_t& n) noexcept
        {
            int32_t  result;

            result = n;
            n = std__intrinsics___wrap_2053 (n + int32_t {1});
            return result;
        }

        int32_t std__intrinsics___wrap_2053 (int32_t n) noexcept
        {
            int32_t  x;

            x = intrinsics::modulo (n, int32_t {2053});
            return (x < int32_t {0}) ? (x + int32_t {2053}) : x;
        }

        int32_t std__intrinsics___wrapped_post_inc_2251 (int32_t& n) noexcept
        {
            int32_t  result;

            result = n;
            n = std__intrinsics___wrap_2251 (n + int32_t {1});
            return result;
        }

        int32_t std__intrinsics___wrap_2251 (int32_t n) noexcept
        {
            int32_t  x;

            x = intrinsics::modulo (n, int32_t {2251});
            return (x < int32_t {0}) ? (x + int32_t {2251}) : x;
        }

        int32_t std__intrinsics___wrapped_post_inc_389 (int32_t& n) noexcept
        {
            int32_t  result;

            result = n;
            n = std__intrinsics___wrap_389 (n + int32_t {1});
            return result;
        }

        int32_t std__intrinsics___wrap_389 (int32_t n) noexcept
        {
            int32_t  x;

            x = intrinsics::modulo (n, int32_t {389});
            return (x < int32_t {0}) ? (x + int32_t {389}) : x;
        }

        int32_t std__intrinsics___wrapped_post_inc_307 (int32_t& n) noexcept
        {
            int32_t  result;

            result = n;
            n = std__intrinsics___wrap_307 (n + int32_t {1});
            return result;
        }

        int32_t std__intrinsics___wrap_307 (int32_t n) noexcept
        {
            int32_t  x;

            x = intrinsics::modulo (n, int32_t {307});
            return (x < int32_t {0}) ? (x + int32_t {307}) : x;
        }

        int32_t std__intrinsics___wrap_1024 (int32_t n) noexcept
        {
            return n & int32_t {1023};
        }

        int32_t std__intrinsics___wrapped_post_inc_1024 (int32_t& n) noexcept
        {
            int32_t  result;

            result = n;
            n = std__intrinsics___wrap_1024 (n + int32_t {1});
            return result;
        }
        ...
        void FXMixer__main (_FXMixer_1_State& _state, _FXMixer_1_IO& _io) noexcept
        {
            ...
            for (;;)
            {
                _temp_0 = 1.0f - _state.chorusAmt;
                _temp[int32_t {0}] = _temp_0;
                _temp[int32_t {1}] = _temp_0;
                _temp_2 = _state.chorusAmt;
                _temp_1[int32_t {0}] = _temp_2;
                _temp_1[int32_t {1}] = _temp_2;
                body = (_io.dryIn * _temp) + (_io.chorusIn * _temp_1);
                _temp_4 = _state.reverbAmt * 1.5f;
                _temp_3[int32_t {0}] = _temp_4;
                _temp_3[int32_t {1}] = _temp_4;
                mix = body + (_io.reverbIn * _temp_3);
                _temp_5[int32_t {0}] = intrinsics::tanh (mix[int32_t {0}] * 0.9f);
                _temp_5[int32_t {1}] = intrinsics::tanh (mix[int32_t {1}] * 0.9f);
                _io.audioOut = (_io.audioOut + _temp_5);
                return;
            }
        }
        ...
        static _JWD50_State& state_upcast_struct_JWD50_State_struc_ITdCWc (_ParamsProcessor_1_State& child) { return *reinterpret_cast<_JWD50_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_JWD50_State, paramsProcessor))); }
        static _JWD50_State& state_upcast_struct_JWD50_State_struc_RaRSs (std_voices_std_voices_VoiceAllocator_specialised_lf0wne_1_State& child) { return *reinterpret_cast<_JWD50_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_JWD50_State, voiceAllocator))); }
        static JWD50_State& state_upcast_struct_JWD50_State_struct_Zu4RG (_JWD50_State& child) { return *reinterpret_cast<JWD50_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (JWD50_State, _state))); }
        static _JWD50_State& state_upcast_struct_JWD50_State_struc_Cjt4te (std_midi_MPEConverter_1_State& child) { return *reinterpret_cast<_JWD50_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_JWD50_State, noteTap))); }
        static _JWD50_State& state_upcast_struct_JWD50_State_struc_NywlWb (std_midi_MPEConverter_1_State& child) { return *reinterpret_cast<_JWD50_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_JWD50_State, mpe))); }
        ...
        double g__frequency {};
        static constexpr int32_t g_AL { int32_t {4096} };
        static constexpr float g_refHz { 261.63f };
        static constexpr float g_damp { 0.34f };
        static constexpr float g_fb { 0.86f };
...
struct JWD50
{
    using PerformerClass = performer::JWD50;
    static constexpr auto filename = "D50.cmajorpatch";
    ...
    static constexpr const char* cmaj_api_cmajpatchconnection_js =
        ...
        "    }\n"
        "}\n";
    static constexpr const char* D50_cmajorpatch =
        "{\n"
        "  \"CmajorVersion\": 1,\n"
        "  \"ID\": \"com.plugincorp.jwd50\",\n"
        "  \"version\": \"0.1\",\n"
        "  \"name\": \"JW-D50\",\n"
        "  \"description\": \"TOI JW-D50 - glossy LA-synthesis digital workstation by plugin corp\",\n"
        "  \"category\": \"instrument\",\n"
        "  \"manufacturer\": \"TOI\",\n"
        "  \"isInstrument\": true,\n"
    ...
    static constexpr std::array files =
    {
        ...
        File { "D50.cmajorpatch", std::string_view (D50_cmajorpatch, 381) },
...
// native faceplate (replaces the generic Cmajor UI). See D50Native.h.
#include "D50Native.h"

juce::AudioProcessor* JUCE_CALLTYPE createPluginFilter()
{
    return new JWD50Plugin (std::make_shared<cmaj::Patch>());
}
