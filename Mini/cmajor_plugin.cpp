...
// Auto-generated Cmajor code for patch 'Mini'
...
//==============================================================================
/// Auto-generated C++ class for the 'Mini' processor
///
...
namespace performer
{
    struct Mini
    {
        ...
        //==============================================================================
        static constexpr uint32_t numInputEndpoints  = 33;
        ...
        static constexpr uint32_t getEndpointHandleForName (std::string_view endpointName)
        {
            ...
            if (endpointName == "o2Det")       return static_cast<uint32_t> (EndpointHandles::o2Det);
            ...
            if (endpointName == "o3Det")       return static_cast<uint32_t> (EndpointHandles::o3Det);
            if (endpointName == "o3Lfo")       return static_cast<uint32_t> (EndpointHandles::o3Lfo);
            if (endpointName == "lvl1")        return static_cast<uint32_t> (EndpointHandles::lvl1);
            if (endpointName == "lvl2")        return static_cast<uint32_t> (EndpointHandles::lvl2);
            if (endpointName == "lvl3")        return static_cast<uint32_t> (EndpointHandles::lvl3);
            ...
            if (endpointName == "envAmt")      return static_cast<uint32_t> (EndpointHandles::envAmt);
            if (endpointName == "kbd")         return static_cast<uint32_t> (EndpointHandles::kbd);
            if (endpointName == "fAtk")        return static_cast<uint32_t> (EndpointHandles::fAtk);
            if (endpointName == "fDec")        return static_cast<uint32_t> (EndpointHandles::fDec);
            if (endpointName == "fSus")        return static_cast<uint32_t> (EndpointHandles::fSus);
            if (endpointName == "aAtk")        return static_cast<uint32_t> (EndpointHandles::aAtk);
            if (endpointName == "aDec")        return static_cast<uint32_t> (EndpointHandles::aDec);
            if (endpointName == "aSus")        return static_cast<uint32_t> (EndpointHandles::aSus);
            if (endpointName == "glide")       return static_cast<uint32_t> (EndpointHandles::glide);
            if (endpointName == "on1")         return static_cast<uint32_t> (EndpointHandles::on1);
            if (endpointName == "on2")         return static_cast<uint32_t> (EndpointHandles::on2);
            if (endpointName == "on3")         return static_cast<uint32_t> (EndpointHandles::on3);
            if (endpointName == "noiseCol")    return static_cast<uint32_t> (EndpointHandles::noiseCol);
            if (endpointName == "volume")      return static_cast<uint32_t> (EndpointHandles::volume);
            if (endpointName == "filtModOn")   return static_cast<uint32_t> (EndpointHandles::filtModOn);
            if (endpointName == "filtModAmt")  return static_cast<uint32_t> (EndpointHandles::filtModAmt);
        ...
        static constexpr EndpointInfo inputEndpoints[] =
        {
            ...
            { 6,   "o2Det",       EndpointType::event },
            { 7,   "o3Wave",      EndpointType::event },
            { 8,   "o3Oct",       EndpointType::event },
            { 9,   "o3Det",       EndpointType::event },
            { 10,  "o3Lfo",       EndpointType::event },
            { 11,  "lvl1",        EndpointType::event },
            { 12,  "lvl2",        EndpointType::event },
            { 13,  "lvl3",        EndpointType::event },
            { 14,  "noise",       EndpointType::event },
            { 15,  "drive",       EndpointType::event },
            { 16,  "cutoff",      EndpointType::event },
            { 17,  "reso",        EndpointType::event },
            { 18,  "envAmt",      EndpointType::event },
            { 19,  "kbd",         EndpointType::event },
            { 20,  "fAtk",        EndpointType::event },
            { 21,  "fDec",        EndpointType::event },
            { 22,  "fSus",        EndpointType::event },
            { 23,  "aAtk",        EndpointType::event },
            { 24,  "aDec",        EndpointType::event },
            { 25,  "aSus",        EndpointType::event },
            { 26,  "glide",       EndpointType::event },
            { 27,  "on1",         EndpointType::event },
            { 28,  "on2",         EndpointType::event },
            { 29,  "on3",         EndpointType::event },
            { 30,  "noiseCol",    EndpointType::event },
            { 31,  "volume",      EndpointType::event },
            { 32,  "filtModOn",   EndpointType::event },
            { 33,  "filtModAmt",  EndpointType::event }
        };

        static constexpr EndpointInfo outputEndpoints[] =
        {
            { 34,  "audioOut",  EndpointType::stream },
            { 35,  "noteOut",   EndpointType::event  }
        };
        ...
        struct _Mini_State
        {
            int32_t _sessionID = {};
            double _frequency = {};
            Array<_MiniVoice_1_State, 1> voices;
        ...
        struct Mini_eventValue_noteOut
        {
            int32_t frame = {};
            int32_t type = {};
        ...
        struct Mini_State
        {
            ...
            Array<Mini_eventValue_noteOut, 32> noteOut;
        };
        ...
        {
            if (endpointHandle == 34) { std::memcpy (reinterpret_cast<char*> (dest), std::addressof (cmajIO.audioOut), 8 * numFramesToCopy); std::memset (reinterpret_cast<char*> (std::addressof (cmajIO.audioOut)), 0, 8 * numFramesToCopy); return; }
        ...
        {
            if (endpointHandle == 35) return (uint32_t) cmajState.noteOut_eventCount;
        ...
        {
            if (endpointHandle == 35) { cmajState.noteOut_eventCount = 0; return; }
        }

        uint32_t getOutputEventType (EndpointHandle endpointHandle, uint32_t index)
        {
            if (endpointHandle == 35) return (uint32_t) cmajState.noteOut[(IndexType) index].type;
            throw std::runtime_error ("Unknown event endpointHandle:" + std::to_string (endpointHandle));
            return {};
        }
        ...
        static uint32_t getOutputEventDataSize (EndpointHandle endpointHandle, uint32_t typeIndex)
        {
            ...
            if (endpointHandle == 35 && typeIndex == 0) return 8;
            throw std::runtime_error ("Unknown event endpointHandle:" + std::to_string (endpointHandle));
            return 0;
        }
        ...
        void addEvent_o2Det (float event)
        {
            _sendEvent_o2Det (cmajState, event);
        }
        ...
        void addEvent_o3Det (float event)
        {
            _sendEvent_o3Det (cmajState, event);
        }

        void addEvent_o3Lfo (float event)
        {
            _sendEvent_o3Lfo (cmajState, event);
        }

        void addEvent_lvl1 (float event)
        {
            _sendEvent_lvl1 (cmajState, event);
        }

        void addEvent_lvl2 (float event)
        {
            _sendEvent_lvl2 (cmajState, event);
        }

        void addEvent_lvl3 (float event)
        {
            _sendEvent_lvl3 (cmajState, event);
        }
        ...
        void addEvent_envAmt (float event)
        {
            _sendEvent_envAmt (cmajState, event);
        }
        ...
        void addEvent_fAtk (float event)
        {
            _sendEvent_fAtk (cmajState, event);
        }

        void addEvent_fDec (float event)
        {
            _sendEvent_fDec (cmajState, event);
        }

        void addEvent_fSus (float event)
        {
            _sendEvent_fSus (cmajState, event);
        }

        void addEvent_aAtk (float event)
        {
            _sendEvent_aAtk (cmajState, event);
        }

        void addEvent_aDec (float event)
        {
            _sendEvent_aDec (cmajState, event);
        }

        void addEvent_aSus (float event)
        {
            _sendEvent_aSus (cmajState, event);
        }
        ...
        void addEvent_on1 (float event)
        {
            _sendEvent_on1 (cmajState, event);
        }

        void addEvent_on2 (float event)
        {
            _sendEvent_on2 (cmajState, event);
        }

        void addEvent_on3 (float event)
        {
            _sendEvent_on3 (cmajState, event);
        }

        void addEvent_noiseCol (float event)
        {
            _sendEvent_noiseCol (cmajState, event);
        }
        ...
        void addEvent_filtModOn (float event)
        {
            _sendEvent_filtModOn (cmajState, event);
        }

        void addEvent_filtModAmt (float event)
        {
            _sendEvent_filtModAmt (cmajState, event);
        }
        ...
        void addEvent (EndpointHandle endpointHandle, uint32_t typeIndex, const unsigned char* eventData)
        {
            ...
            if (endpointHandle == 30)
            {
                ...
                memcpy (&value, eventData, 4);
                eventData += 4;
                return addEvent_noiseCol (value);
            }
            ...
            if (endpointHandle == 32)
            {
                ...
                memcpy (&value, eventData, 4);
                eventData += 4;
                return addEvent_filtModOn (value);
            }
            ...
            if (endpointHandle == 33)
            {
                ...
                memcpy (&value, eventData, 4);
                eventData += 4;
                return addEvent_filtModAmt (value);
            }
        }
        ...
        Mini_State cmajState = {};
        Mini_IO cmajIO = {};

        //==============================================================================
        void _sendEvent_midiIn (Mini_State& _state, std_midi_Message value) noexcept
        {
            _Mini__midiIn (_state._state, value);
        }

        void _Mini__midiIn (_Mini_State& _state, std_midi_Message value) noexcept
        {
        ...
        {
            _Mini___forwardEvent (state_upcast_struct_Mini_State_struct_Jn4Esd(_state), value);
            _Mini___forwardEvent_5 (state_upcast_struct_Mini_State_struct_Jn4Esd(_state), value);
        }

        void _Mini___forwardEvent (_Mini_State& _state, std_notes_NoteOn value) noexcept
        {
        ...
        {
            _Mini___forwardEvent_3 (state_upcast_struct_Mini_State_struct_mRzn3d(_state), value);
        }

        void _Mini___forwardEvent_3 (_Mini_State& _state, plugincorp_NoteEvent value) noexcept
        {
            _Mini___writeEvent_noteOut (_state, value);
        }

        void _Mini___writeEvent_noteOut (_Mini_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
            Mini___writeEvent_noteOut (state_upcast_struct_Mini_State_struct__0jbNNc(_state), value);
        }

        void Mini___writeEvent_noteOut (Mini_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
            Mini___writeEvent_noteOut_0 (_state, value);
        }

        void Mini___writeEvent_noteOut_0 (Mini_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
        ...
        void _Mini___forwardEvent_5 (_Mini_State& _state, std_notes_NoteOn value) noexcept
        {
        ...
        {
            _Mini___forwardEvent_12 (state_upcast_struct_Mini_State_struct_fsJGdd(_state), index, value);
        }

        void _Mini___forwardEvent_12 (_Mini_State& _state, int32_t index, std_notes_NoteOff value) noexcept
        {
            MiniVoice__eventsIn (_state.voices[index], value);
        }

        void MiniVoice__eventsIn (_MiniVoice_1_State& _state, std_notes_NoteOff e) noexcept
        {
        ...
        {
            _Mini___forwardEvent_11 (state_upcast_struct_Mini_State_struct_fsJGdd(_state), index, value);
        }

        void _Mini___forwardEvent_11 (_Mini_State& _state, int32_t index, std_notes_NoteOn value) noexcept
        {
            MiniVoice__eventsIn_0 (_state.voices[index], value);
        }

        void MiniVoice__eventsIn_0 (_MiniVoice_1_State& _state, std_notes_NoteOn e) noexcept
        {
            float  sr;
            float  glideTime;
            ...
            glideTime = _state.params.glide * 0.5f;
        ...
        {
            _Mini___forwardEvent_2 (state_upcast_struct_Mini_State_struct_Jn4Esd(_state), value);
            _Mini___forwardEvent_6 (state_upcast_struct_Mini_State_struct_Jn4Esd(_state), value);
        }

        void _Mini___forwardEvent_2 (_Mini_State& _state, std_notes_NoteOff value) noexcept
        {
        ...
        void _Mini___forwardEvent_6 (_Mini_State& _state, std_notes_NoteOff value) noexcept
        {
        ...
        {
            _Mini___forwardEvent_7 (state_upcast_struct_Mini_State_struct_Jn4Esd(_state), value);
        }

        void _Mini___forwardEvent_7 (_Mini_State& _state, std_notes_PitchBend value) noexcept
        {
        ...
        {
            _Mini___forwardEvent_9 (state_upcast_struct_Mini_State_struct_Jn4Esd(_state), value);
        }

        void _Mini___forwardEvent_9 (_Mini_State& _state, std_notes_Pressure value) noexcept
        {
        ...
        {
            _Mini___forwardEvent_8 (state_upcast_struct_Mini_State_struct_Jn4Esd(_state), value);
        }

        void _Mini___forwardEvent_8 (_Mini_State& _state, std_notes_Slide value) noexcept
        {
        ...
        {
            _Mini___forwardEvent_10 (state_upcast_struct_Mini_State_struct_Jn4Esd(_state), value);
        }

        void _Mini___forwardEvent_10 (_Mini_State& _state, std_notes_Control value) noexcept
        {
        ...
        void _sendEvent_o1Wave (Mini_State& _state, float value) noexcept
        {
            _Mini__o1Wave (_state._state, value);
        }

        void _Mini__o1Wave (_Mini_State& _state, float value) noexcept
        {
        ...
        {
            _Mini___forwardEvent_4 (state_upcast_struct_Mini_State_struct_XEy93c(_state), value);
        }

        void _Mini___forwardEvent_4 (_Mini_State& _state, plugincorp_Params value) noexcept
        {
            MiniVoice__paramsIn (_state.voices[int32_t {0}], value);
        }

        void MiniVoice__paramsIn (_MiniVoice_1_State& _state, plugincorp_Params p) noexcept
        {
            _state.params = p;
        }

        void _sendEvent_o1Oct (Mini_State& _state, float value) noexcept
        {
            _Mini__o1Oct (_state._state, value);
        }

        void _Mini__o1Oct (_Mini_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_o2Wave (Mini_State& _state, float value) noexcept
        {
            _Mini__o2Wave (_state._state, value);
        }

        void _Mini__o2Wave (_Mini_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_o2Oct (Mini_State& _state, float value) noexcept
        {
            _Mini__o2Oct (_state._state, value);
        }

        void _Mini__o2Oct (_Mini_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_o2Det (Mini_State& _state, float value) noexcept
        {
            _Mini__o2Det (_state._state, value);
        }

        void _Mini__o2Det (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__o2Det (_state.paramsProcessor, value);
        }

        void ParamsProcessor__o2Det (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.o2Det = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_o3Wave (Mini_State& _state, float value) noexcept
        {
            _Mini__o3Wave (_state._state, value);
        }

        void _Mini__o3Wave (_Mini_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_o3Oct (Mini_State& _state, float value) noexcept
        {
            _Mini__o3Oct (_state._state, value);
        }

        void _Mini__o3Oct (_Mini_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_o3Det (Mini_State& _state, float value) noexcept
        {
            _Mini__o3Det (_state._state, value);
        }

        void _Mini__o3Det (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__o3Det (_state.paramsProcessor, value);
        }

        void ParamsProcessor__o3Det (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.o3Det = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_o3Lfo (Mini_State& _state, float value) noexcept
        {
            _Mini__o3Lfo (_state._state, value);
        }

        void _Mini__o3Lfo (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__o3Lfo (_state.paramsProcessor, value);
        }

        void ParamsProcessor__o3Lfo (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.o3Lfo = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_lvl1 (Mini_State& _state, float value) noexcept
        {
            _Mini__lvl1 (_state._state, value);
        }

        void _Mini__lvl1 (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__lvl1 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__lvl1 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.lvl1 = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_lvl2 (Mini_State& _state, float value) noexcept
        {
            _Mini__lvl2 (_state._state, value);
        }

        void _Mini__lvl2 (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__lvl2 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__lvl2 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.lvl2 = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_lvl3 (Mini_State& _state, float value) noexcept
        {
            _Mini__lvl3 (_state._state, value);
        }

        void _Mini__lvl3 (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__lvl3 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__lvl3 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.lvl3 = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_noise (Mini_State& _state, float value) noexcept
        {
            _Mini__noise (_state._state, value);
        }

        void _Mini__noise (_Mini_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_drive (Mini_State& _state, float value) noexcept
        {
            _Mini__drive (_state._state, value);
        }

        void _Mini__drive (_Mini_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_cutoff (Mini_State& _state, float value) noexcept
        {
            _Mini__cutoff (_state._state, value);
        }

        void _Mini__cutoff (_Mini_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_reso (Mini_State& _state, float value) noexcept
        {
            _Mini__reso (_state._state, value);
        }

        void _Mini__reso (_Mini_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_envAmt (Mini_State& _state, float value) noexcept
        {
            _Mini__envAmt (_state._state, value);
        }

        void _Mini__envAmt (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__envAmt (_state.paramsProcessor, value);
        }

        void ParamsProcessor__envAmt (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.envAmt = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_kbd (Mini_State& _state, float value) noexcept
        {
            _Mini__kbd (_state._state, value);
        }

        void _Mini__kbd (_Mini_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_fAtk (Mini_State& _state, float value) noexcept
        {
            _Mini__fAtk (_state._state, value);
        }

        void _Mini__fAtk (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__fAtk (_state.paramsProcessor, value);
        }

        void ParamsProcessor__fAtk (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.fAtk = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_fDec (Mini_State& _state, float value) noexcept
        {
            _Mini__fDec (_state._state, value);
        }

        void _Mini__fDec (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__fDec (_state.paramsProcessor, value);
        }

        void ParamsProcessor__fDec (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.fDec = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_fSus (Mini_State& _state, float value) noexcept
        {
            _Mini__fSus (_state._state, value);
        }

        void _Mini__fSus (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__fSus (_state.paramsProcessor, value);
        }

        void ParamsProcessor__fSus (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.fSus = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_aAtk (Mini_State& _state, float value) noexcept
        {
            _Mini__aAtk (_state._state, value);
        }

        void _Mini__aAtk (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__aAtk (_state.paramsProcessor, value);
        }

        void ParamsProcessor__aAtk (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.aAtk = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_aDec (Mini_State& _state, float value) noexcept
        {
            _Mini__aDec (_state._state, value);
        }

        void _Mini__aDec (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__aDec (_state.paramsProcessor, value);
        }

        void ParamsProcessor__aDec (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.aDec = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_aSus (Mini_State& _state, float value) noexcept
        {
            _Mini__aSus (_state._state, value);
        }

        void _Mini__aSus (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__aSus (_state.paramsProcessor, value);
        }

        void ParamsProcessor__aSus (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.aSus = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_glide (Mini_State& _state, float value) noexcept
        {
            _Mini__glide (_state._state, value);
        }

        void _Mini__glide (_Mini_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_on1 (Mini_State& _state, float value) noexcept
        {
            _Mini__on1 (_state._state, value);
        }

        void _Mini__on1 (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__on1 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__on1 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.on1 = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_on2 (Mini_State& _state, float value) noexcept
        {
            _Mini__on2 (_state._state, value);
        }

        void _Mini__on2 (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__on2 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__on2 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.on2 = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_on3 (Mini_State& _state, float value) noexcept
        {
            _Mini__on3 (_state._state, value);
        }

        void _Mini__on3 (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__on3 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__on3 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.on3 = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_noiseCol (Mini_State& _state, float value) noexcept
        {
            _Mini__noiseCol (_state._state, value);
        }

        void _Mini__noiseCol (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__noiseCol (_state.paramsProcessor, value);
        }

        void ParamsProcessor__noiseCol (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.noiseCol = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_volume (Mini_State& _state, float value) noexcept
        {
            _Mini__volume (_state._state, value);
        }

        void _Mini__volume (_Mini_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_filtModOn (Mini_State& _state, float value) noexcept
        {
            _Mini__filtModOn (_state._state, value);
        }

        void _Mini__filtModOn (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__filtModOn (_state.paramsProcessor, value);
        }

        void ParamsProcessor__filtModOn (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.filtModOn = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_filtModAmt (Mini_State& _state, float value) noexcept
        {
            _Mini__filtModAmt (_state._state, value);
        }

        void _Mini__filtModAmt (_Mini_State& _state, float value) noexcept
        {
            ParamsProcessor__filtModAmt (_state.paramsProcessor, value);
        }

        void ParamsProcessor__filtModAmt (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.filtModAmt = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _initialise (Mini_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            _Mini___initialise (_state._state, processorID, sessionID, frequency);
        }

        void _Mini___initialise (_Mini_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
            _state.voices[int32_t {0}]._instanceIndex = int32_t {0};
            MiniVoice___initialise (_state.voices[int32_t {0}], processorID, sessionID, frequency);
        ...
        void MiniVoice___initialise (_MiniVoice_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
        ...
        void ParamsProcessor___initialise (_ParamsProcessor_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
            _state.params = plugincorp_Params { 0.2f, 0.5f, 0.2f, 0.5f, 0.53f, 0.2f, 0.25f, 0.47f, 0.0f, 0.8f, 0.7f, 0.55f, 0.0f, 0.45f, 0.55f, 0.25f, 0.45f, 0.7f, 0.02f, 0.45f, 0.3f, 0.02f, 0.5f, 0.8f, 0.0f, 1.0f, 1.0f, 1.0f, 0.0f, 0.8f, 0.0f, 0.3f };
        }
        ...
        void _advance (Mini_State& _state, Mini_IO& _io, int32_t _frames) noexcept
        {
            _Mini_IO  ioCopy;
        ...
        void main (_Mini_State& _state, _Mini_IO& _io) noexcept
        {
            Array<_MiniVoice_1_IO, 1>  voices_io;
            ...
            voices_io = Array<_MiniVoice_1_IO, 1> {};
            ...
            MiniVoice__main (_state.voices[int32_t {0}], voices_io[int32_t {0}]);
            {
            }
        ...
        void MiniVoice__main (_MiniVoice_1_State& _state, _MiniVoice_1_IO& _io) noexcept
        {
            float  out;
            float  driftC;
            bool  o3IsLfo;
            ...
            _state.d1 = MiniVoice__rnd (_state);
            _state.d2 = MiniVoice__rnd (_state);
            _state.d3 = MiniVoice__rnd (_state);
            _branch0_0:
            {
                for (;;)
                {
                    ...
                    if (_state.active)
                    {
                        ...
                        _state.driftPh = (_state.driftPh + (6.0f * _state.invSr));
                        ...
                        driftC = 0.0008f;
                        o3IsLfo = _state.params.o3Lfo > 0.5f;
                        lfoHz = 0.1f + ((_state.params.o3Det * _state.params.o3Det) * 24.0f);
                        if (o3IsLfo)
                        {
                            _state.ph3 = (_state.ph3 + (lfoHz * _state.invSr));
                            for (;;)
                            {
                                if  (! (_state.ph3 >= 1.0f))
                                {
                        ...
                        lfo = intrinsics::sin (_state.ph3 * 6.2831855f);
                        modDepth = o3IsLfo ? _state.params.lvl3 : 0.0f;
                        vib = o3IsLfo ? (1.0f + ((lfo * modDepth) * 0.03f)) : 1.0f;
                        base = _state.glideFreq * vib;
                        f1 = (base * MiniVoice__octMul (_state.params.o1Oct)) * (1.0f + (_state.d1 * driftC));
                        f2 = ((base * MiniVoice__octMul (_state.params.o2Oct)) * intrinsics::pow (2.0f, (_state.params.o2Det - 0.5f) * 0.5f)) * (1.0f + (_state.d2 * driftC));
                        ...
                        if (! o3IsLfo)
                        {
                            f3 = ((base * MiniVoice__octMul (_state.params.o3Oct)) * intrinsics::pow (2.0f, (_state.params.o3Det - 0.5f) * 0.5f)) * (1.0f + (_state.d3 * driftC));
                            ...
                            o3 = ((MiniVoice__shape (_state.ph3, inc3, _state.params.o3Wave) * _state.params.lvl3) * ((_state.params.on3 >= 0.5f) ? 1.0f : 0.0f));
                        }
                        o1 = (MiniVoice__shape (_state.ph1, inc1, _state.params.o1Wave) * _state.params.lvl1) * ((_state.params.on1 >= 0.5f) ? 1.0f : 0.0f);
                        o2 = (MiniVoice__shape (_state.ph2, inc2, _state.params.o2Wave) * _state.params.lvl2) * ((_state.params.on2 >= 0.5f) ? 1.0f : 0.0f);
                        nraw = MiniVoice__rnd (_state);
                        _state.npink = (_state.npink + ((nraw - _state.npink) * 0.12f));
                        nz = (((_state.params.noiseCol >= 0.5f) ? (_state.npink * 2.2f) : nraw) * _state.params.noise) * 0.5f;
                        g = 1.0f + (_state.params.drive * 4.0f);
                        mix = (((o1 + o2) + o3) + nz) * g;
                        mix = intrinsics::tanh (mix * 0.6f);
                        fAInc = 1.0f / (intrinsics::max (0.0005f, 0.001f + ((_state.params.fAtk * _state.params.fAtk) * 5.0f)) * _state.sr);
                        fDDec = 1.0f / (intrinsics::max (0.0005f, 0.004f + ((_state.params.fDec * _state.params.fDec) * 8.0f)) * _state.sr);
                        if (_state.fStage == int32_t {0})
                        {
                            _state.fEnv = (_state.fEnv + fAInc);
                            if (_state.fEnv >= 1.0f)
                            {
                                _state.fEnv = 1.0f;
                                _state.fStage = int32_t {1};
                        ...
                        else
                        {
                            ...
                            {
                                _state.fEnv = (_state.fEnv - fDDec);
                                if (_state.fEnv <= _state.params.fSus)
                                {
                                    _state.fEnv = _state.params.fSus;
                        ...
                        track = 1.0f + (_state.params.kbd * ((_state.glideFreq / 261.6f) - 1.0f));
                        envHz = (_state.params.envAmt * _state.fEnv) * 8000.0f;
                        fc = (kHz + envHz) * track;
                        if (_state.params.filtModOn >= 0.5f)
                        {
                            fc = (fc * intrinsics::pow (2.0f, (lfo * _state.params.filtModAmt) * 2.0f));
                        }
                        ...
                        aAInc = 1.0f / (intrinsics::max (0.0005f, 0.001f + ((_state.params.aAtk * _state.params.aAtk) * 5.0f)) * _state.sr);
                        aDDec = 1.0f / (intrinsics::max (0.0005f, 0.004f + ((_state.params.aDec * _state.params.aDec) * 8.0f)) * _state.sr);
                        if (_state.aStage == int32_t {0})
                        {
                            _state.aEnv = (_state.aEnv + aAInc);
                            if (_state.aEnv >= 1.0f)
                            {
                                _state.aEnv = 1.0f;
                                _state.aStage = int32_t {1};
                        ...
                        else
                        {
                            ...
                            {
                                _state.aEnv = (_state.aEnv - aDDec);
                                if (_state.aEnv <= _state.params.aSus)
                                {
                                    _state.aEnv = _state.params.aSus;
                        ...
                        if ((_state.aStage == int32_t {-1}) ? (! _state.gateOpen) : false)
                        {
                            _state.active = false;
                        }
                        out = intrinsics::tanh (((_state.y4 * _state.aEnv) * (0.4f + (0.6f * _state.vel))) * (0.35f + (_state.params.volume * 1.1f)));
        ...
        float MiniVoice__rnd (_MiniVoice_1_State& _state) noexcept
        {
            float  s;
        ...
        float MiniVoice__octMul (float v) noexcept
        {
            int32_t  i;

            i = static_cast<int32_t> (intrinsics::clamp (v * 5.0f, 0.0f, 4.0f));
            return (i == int32_t {0}) ? 0.25f : ((i == int32_t {1}) ? 0.5f : ((i == int32_t {2}) ? 1.0f : ((i == int32_t {3}) ? 2.0f : 4.0f)));
        }
        ...
        float MiniVoice__shape (float ph, float inc, float w) noexcept
        {
            int32_t  idx;
            float  saw;
            float  width;
            ...
            idx = static_cast<int32_t> (intrinsics::clamp (w * 6.0f, 0.0f, 5.0f));
            ...
            width = (idx == int32_t {3}) ? 0.5f : ((idx == int32_t {4}) ? 0.25f : 0.12f);
            ph2 = ph + width;
            if (ph2 >= 1.0f)
            {
                ph2 = (ph2 - 1.0f);
            }
            saw2 = ((2.0f * ph2) - 1.0f) - toi__dsp__polyBlep (ph2, inc);
            return (saw - saw2) * 0.7f;
        }
        ...
        static _Mini_State& state_upcast_struct_Mini_State_struct_XEy93c (_ParamsProcessor_1_State& child) { return *reinterpret_cast<_Mini_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_Mini_State, paramsProcessor))); }
        static Mini_State& state_upcast_struct_Mini_State_struct__0jbNNc (_Mini_State& child) { return *reinterpret_cast<Mini_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (Mini_State, _state))); }
        static _Mini_State& state_upcast_struct_Mini_State_struct_fsJGdd (std_voices_std_voices_VoiceAllocator_specialised_Sitcze_1_State& child) { return *reinterpret_cast<_Mini_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_Mini_State, voiceAllocator))); }
        static _Mini_State& state_upcast_struct_Mini_State_struct_mRzn3d (std_midi_MPEConverter_1_State& child) { return *reinterpret_cast<_Mini_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_Mini_State, noteTap))); }
        static _Mini_State& state_upcast_struct_Mini_State_struct_Jn4Esd (std_midi_MPEConverter_1_State& child) { return *reinterpret_cast<_Mini_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_Mini_State, mpe))); }
...
struct Mini
{
    using PerformerClass = performer::Mini;
    static constexpr auto filename = "Mini.cmajorpatch";
    ...
    static constexpr const char* cmaj_api_cmajpatchconnection_js =
        ...
        "    }\n"
        "}\n";
    static constexpr const char* Mini_cmajorpatch =
        "{\n"
        "  \"CmajorVersion\": 1,\n"
        "  \"ID\": \"com.plugincorp.jwmini\",\n"
        "  \"version\": \"0.1\",\n"
        "  \"name\": \"JW-MINI\",\n"
    ...
    static constexpr std::array files =
    {
        ...
        File { "Mini.cmajorpatch", std::string_view (Mini_cmajorpatch, 371) },
...
// native faceplate (replaces the generic Cmajor UI). See MiniNative.h.
#include "MiniNative.h"

juce::AudioProcessor* JUCE_CALLTYPE createPluginFilter()
{
    return new MiniPlugin (std::make_shared<cmaj::Patch>());
}
