...
// Auto-generated Cmajor code for patch 'SEM'
...
//==============================================================================
/// Auto-generated C++ class for the 'SEM' processor
///
...
namespace performer
{
    struct SEM
    {
        ...
        static constexpr std::string_view name = "SEM";
        ...
        //==============================================================================
        static constexpr uint32_t numInputEndpoints  = 32;
        ...
        static constexpr uint32_t getEndpointHandleForName (std::string_view endpointName)
        {
            ...
            if (endpointName == "v1wave")    return static_cast<uint32_t> (EndpointHandles::v1wave);
            if (endpointName == "v1pw")      return static_cast<uint32_t> (EndpointHandles::v1pw);
            if (endpointName == "v1mod")     return static_cast<uint32_t> (EndpointHandles::v1mod);
            if (endpointName == "v1tune")    return static_cast<uint32_t> (EndpointHandles::v1tune);
            if (endpointName == "v2wave")    return static_cast<uint32_t> (EndpointHandles::v2wave);
            if (endpointName == "v2pw")      return static_cast<uint32_t> (EndpointHandles::v2pw);
            if (endpointName == "v2mod")     return static_cast<uint32_t> (EndpointHandles::v2mod);
            if (endpointName == "v2tune")    return static_cast<uint32_t> (EndpointHandles::v2tune);
            ...
            if (endpointName == "bp")        return static_cast<uint32_t> (EndpointHandles::bp);
            if (endpointName == "filtEnv")   return static_cast<uint32_t> (EndpointHandles::filtEnv);
            if (endpointName == "lfoCut")    return static_cast<uint32_t> (EndpointHandles::lfoCut);
            if (endpointName == "kbd")       return static_cast<uint32_t> (EndpointHandles::kbd);
            ...
            if (endpointName == "vibRate")   return static_cast<uint32_t> (EndpointHandles::vibRate);
            if (endpointName == "vibAmt")    return static_cast<uint32_t> (EndpointHandles::vibAmt);
        ...
        static constexpr EndpointInfo inputEndpoints[] =
        {
            ...
            { 2,   "v1wave",   EndpointType::event },
            { 3,   "v1pw",     EndpointType::event },
            { 4,   "v1mod",    EndpointType::event },
            { 5,   "v1tune",   EndpointType::event },
            { 6,   "v2wave",   EndpointType::event },
            { 7,   "v2pw",     EndpointType::event },
            { 8,   "v2mod",    EndpointType::event },
            { 9,   "v2tune",   EndpointType::event },
            { 10,  "sync",     EndpointType::event },
            { 11,  "mix1",     EndpointType::event },
            { 12,  "mix2",     EndpointType::event },
            { 13,  "noise",    EndpointType::event },
            { 14,  "cutoff",   EndpointType::event },
            { 15,  "reso",     EndpointType::event },
            { 16,  "mode",     EndpointType::event },
            { 17,  "bp",       EndpointType::event },
            { 18,  "filtEnv",  EndpointType::event },
            { 19,  "lfoCut",   EndpointType::event },
            { 20,  "kbd",      EndpointType::event },
            { 21,  "a1",       EndpointType::event },
            { 22,  "d1",       EndpointType::event },
            { 23,  "s1",       EndpointType::event },
            { 24,  "a2",       EndpointType::event },
            { 25,  "d2",       EndpointType::event },
            { 26,  "s2",       EndpointType::event },
            { 27,  "lfoRate",  EndpointType::event },
            { 28,  "lfoWave",  EndpointType::event },
            { 29,  "volume",   EndpointType::event },
            { 30,  "glide",    EndpointType::event },
            { 31,  "vibRate",  EndpointType::event },
            { 32,  "vibAmt",   EndpointType::event }
        };

        static constexpr EndpointInfo outputEndpoints[] =
        {
            { 33,  "audioOut",  EndpointType::stream },
            { 34,  "noteOut",   EndpointType::event  }
        };
        ...
        struct _SEM_State
        {
            int32_t _sessionID = {};
            double _frequency = {};
            Array<_SEMVoice_1_State, 8> voices;
        ...
        struct SEM_State
        {
            ...
            Array<SEM_eventValue_noteOut, 32> noteOut;
        };
        ...
        {
            if (endpointHandle == 33) { std::memcpy (reinterpret_cast<char*> (dest), std::addressof (cmajIO.audioOut), 8 * numFramesToCopy); std::memset (reinterpret_cast<char*> (std::addressof (cmajIO.audioOut)), 0, 8 * numFramesToCopy); return; }
        ...
        {
            if (endpointHandle == 34) return (uint32_t) cmajState.noteOut_eventCount;
        ...
        {
            if (endpointHandle == 34) { cmajState.noteOut_eventCount = 0; return; }
        }

        uint32_t getOutputEventType (EndpointHandle endpointHandle, uint32_t index)
        {
            if (endpointHandle == 34) return (uint32_t) cmajState.noteOut[(IndexType) index].type;
            throw std::runtime_error ("Unknown event endpointHandle:" + std::to_string (endpointHandle));
            return {};
        }
        ...
        static uint32_t getOutputEventDataSize (EndpointHandle endpointHandle, uint32_t typeIndex)
        {
            ...
            if (endpointHandle == 34 && typeIndex == 0) return 8;
            throw std::runtime_error ("Unknown event endpointHandle:" + std::to_string (endpointHandle));
            return 0;
        }
        ...
        void addEvent_v1wave (float event)
        {
            _sendEvent_v1wave (cmajState, event);
        }

        void addEvent_v1pw (float event)
        {
            _sendEvent_v1pw (cmajState, event);
        }

        void addEvent_v1mod (float event)
        {
            _sendEvent_v1mod (cmajState, event);
        }

        void addEvent_v1tune (float event)
        {
            _sendEvent_v1tune (cmajState, event);
        }

        void addEvent_v2wave (float event)
        {
            _sendEvent_v2wave (cmajState, event);
        }

        void addEvent_v2pw (float event)
        {
            _sendEvent_v2pw (cmajState, event);
        }

        void addEvent_v2mod (float event)
        {
            _sendEvent_v2mod (cmajState, event);
        }

        void addEvent_v2tune (float event)
        {
            _sendEvent_v2tune (cmajState, event);
        }
        ...
        void addEvent_mode (float event)
        {
            _sendEvent_mode (cmajState, event);
        }

        void addEvent_bp (float event)
        {
            _sendEvent_bp (cmajState, event);
        }
        ...
        void addEvent_lfoCut (float event)
        {
            _sendEvent_lfoCut (cmajState, event);
        }

        void addEvent_kbd (float event)
        {
            _sendEvent_kbd (cmajState, event);
        }
        ...
        void addEvent_vibRate (float event)
        {
            _sendEvent_vibRate (cmajState, event);
        }

        void addEvent_vibAmt (float event)
        {
            _sendEvent_vibAmt (cmajState, event);
        }
        ...
        //==============================================================================
        void _sendEvent_midiIn (SEM_State& _state, std_midi_Message value) noexcept
        {
            _SEM__midiIn (_state._state, value);
        }

        void _SEM__midiIn (_SEM_State& _state, std_midi_Message value) noexcept
        {
        ...
        {
            _SEM___forwardEvent (state_upcast_struct_SEM_State_struct__KJqYAd(_state), value);
            _SEM___forwardEvent_5 (state_upcast_struct_SEM_State_struct__KJqYAd(_state), value);
        }

        void _SEM___forwardEvent (_SEM_State& _state, std_notes_NoteOn value) noexcept
        {
        ...
        {
            _SEM___forwardEvent_3 (state_upcast_struct_SEM_State_struct__TYB3Pc(_state), value);
        }

        void _SEM___forwardEvent_3 (_SEM_State& _state, plugincorp_NoteEvent value) noexcept
        {
            _SEM___writeEvent_noteOut (_state, value);
        }

        void _SEM___writeEvent_noteOut (_SEM_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
            SEM___writeEvent_noteOut (state_upcast_struct_SEM_State_struct__EB0l1d(_state), value);
        }

        void SEM___writeEvent_noteOut (SEM_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
            SEM___writeEvent_noteOut_0 (_state, value);
        }

        void SEM___writeEvent_noteOut_0 (SEM_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
        ...
        void _SEM___forwardEvent_5 (_SEM_State& _state, std_notes_NoteOn value) noexcept
        {
        ...
        {
            _SEM___forwardEvent_12 (state_upcast_struct_SEM_State_struct__fmAlXb(_state), index, value);
        }

        void _SEM___forwardEvent_12 (_SEM_State& _state, int32_t index, std_notes_NoteOff value) noexcept
        {
            SEMVoice__eventsIn (_state.voices[index], value);
        }

        void SEMVoice__eventsIn (_SEMVoice_1_State& _state, std_notes_NoteOff e) noexcept
        {
        ...
        {
            _SEM___forwardEvent_11 (state_upcast_struct_SEM_State_struct__fmAlXb(_state), index, value);
        }

        void _SEM___forwardEvent_11 (_SEM_State& _state, int32_t index, std_notes_NoteOn value) noexcept
        {
            SEMVoice__eventsIn_0 (_state.voices[index], value);
        }

        void SEMVoice__eventsIn_0 (_SEMVoice_1_State& _state, std_notes_NoteOn e) noexcept
        {
            float  sr;
            float  gt;
            ...
            _state.vDetune = SEMVoice__rnd (_state);
            _state.vDrift = SEMVoice__rnd (_state);
        }
        ...
        float SEMVoice__rnd (_SEMVoice_1_State& _state) noexcept
        {
            float  s;
        ...
        {
            _SEM___forwardEvent_2 (state_upcast_struct_SEM_State_struct__KJqYAd(_state), value);
            _SEM___forwardEvent_6 (state_upcast_struct_SEM_State_struct__KJqYAd(_state), value);
        }

        void _SEM___forwardEvent_2 (_SEM_State& _state, std_notes_NoteOff value) noexcept
        {
        ...
        void _SEM___forwardEvent_6 (_SEM_State& _state, std_notes_NoteOff value) noexcept
        {
        ...
        {
            _SEM___forwardEvent_7 (state_upcast_struct_SEM_State_struct__KJqYAd(_state), value);
        }

        void _SEM___forwardEvent_7 (_SEM_State& _state, std_notes_PitchBend value) noexcept
        {
        ...
        {
            _SEM___forwardEvent_9 (state_upcast_struct_SEM_State_struct__KJqYAd(_state), value);
        }

        void _SEM___forwardEvent_9 (_SEM_State& _state, std_notes_Pressure value) noexcept
        {
        ...
        {
            _SEM___forwardEvent_8 (state_upcast_struct_SEM_State_struct__KJqYAd(_state), value);
        }

        void _SEM___forwardEvent_8 (_SEM_State& _state, std_notes_Slide value) noexcept
        {
        ...
        {
            _SEM___forwardEvent_10 (state_upcast_struct_SEM_State_struct__KJqYAd(_state), value);
        }

        void _SEM___forwardEvent_10 (_SEM_State& _state, std_notes_Control value) noexcept
        {
        ...
        void _sendEvent_v1wave (SEM_State& _state, float value) noexcept
        {
            _SEM__v1wave (_state._state, value);
        }

        void _SEM__v1wave (_SEM_State& _state, float value) noexcept
        {
            ParamsProcessor__v1wave (_state.paramsProcessor, value);
        }

        void ParamsProcessor__v1wave (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.v1wave = v;
        ...
        {
            _SEM___forwardEvent_4 (state_upcast_struct_SEM_State_struct__FKkLoc(_state), value);
        }

        void _SEM___forwardEvent_4 (_SEM_State& _state, plugincorp_Params value) noexcept
        {
            int32_t  i;

            {
                ...
                for (;;)
                {
                    ...
                    SEMVoice__paramsIn (_state.voices[i], value);
                    {
                        ++i;
                    }
                }
        ...
        void SEMVoice__paramsIn (_SEMVoice_1_State& _state, plugincorp_Params p) noexcept
        {
            _state.params = p;
        }

        void _sendEvent_v1pw (SEM_State& _state, float value) noexcept
        {
            _SEM__v1pw (_state._state, value);
        }

        void _SEM__v1pw (_SEM_State& _state, float value) noexcept
        {
            ParamsProcessor__v1pw (_state.paramsProcessor, value);
        }

        void ParamsProcessor__v1pw (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.v1pw = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_v1mod (SEM_State& _state, float value) noexcept
        {
            _SEM__v1mod (_state._state, value);
        }

        void _SEM__v1mod (_SEM_State& _state, float value) noexcept
        {
            ParamsProcessor__v1mod (_state.paramsProcessor, value);
        }

        void ParamsProcessor__v1mod (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.v1mod = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_v1tune (SEM_State& _state, float value) noexcept
        {
            _SEM__v1tune (_state._state, value);
        }

        void _SEM__v1tune (_SEM_State& _state, float value) noexcept
        {
            ParamsProcessor__v1tune (_state.paramsProcessor, value);
        }

        void ParamsProcessor__v1tune (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.v1tune = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_v2wave (SEM_State& _state, float value) noexcept
        {
            _SEM__v2wave (_state._state, value);
        }

        void _SEM__v2wave (_SEM_State& _state, float value) noexcept
        {
            ParamsProcessor__v2wave (_state.paramsProcessor, value);
        }

        void ParamsProcessor__v2wave (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.v2wave = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_v2pw (SEM_State& _state, float value) noexcept
        {
            _SEM__v2pw (_state._state, value);
        }

        void _SEM__v2pw (_SEM_State& _state, float value) noexcept
        {
            ParamsProcessor__v2pw (_state.paramsProcessor, value);
        }

        void ParamsProcessor__v2pw (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.v2pw = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_v2mod (SEM_State& _state, float value) noexcept
        {
            _SEM__v2mod (_state._state, value);
        }

        void _SEM__v2mod (_SEM_State& _state, float value) noexcept
        {
            ParamsProcessor__v2mod (_state.paramsProcessor, value);
        }

        void ParamsProcessor__v2mod (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.v2mod = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_v2tune (SEM_State& _state, float value) noexcept
        {
            _SEM__v2tune (_state._state, value);
        }

        void _SEM__v2tune (_SEM_State& _state, float value) noexcept
        {
            ParamsProcessor__v2tune (_state.paramsProcessor, value);
        }

        void ParamsProcessor__v2tune (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.v2tune = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_sync (SEM_State& _state, float value) noexcept
        {
            _SEM__sync (_state._state, value);
        }

        void _SEM__sync (_SEM_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_mix1 (SEM_State& _state, float value) noexcept
        {
            _SEM__mix1 (_state._state, value);
        }

        void _SEM__mix1 (_SEM_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_mix2 (SEM_State& _state, float value) noexcept
        {
            _SEM__mix2 (_state._state, value);
        }

        void _SEM__mix2 (_SEM_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_noise (SEM_State& _state, float value) noexcept
        {
            _SEM__noise (_state._state, value);
        }

        void _SEM__noise (_SEM_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_cutoff (SEM_State& _state, float value) noexcept
        {
            _SEM__cutoff (_state._state, value);
        }

        void _SEM__cutoff (_SEM_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_reso (SEM_State& _state, float value) noexcept
        {
            _SEM__reso (_state._state, value);
        }

        void _SEM__reso (_SEM_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_mode (SEM_State& _state, float value) noexcept
        {
            _SEM__mode (_state._state, value);
        }

        void _SEM__mode (_SEM_State& _state, float value) noexcept
        {
            ParamsProcessor__mode (_state.paramsProcessor, value);
        }

        void ParamsProcessor__mode (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.mode = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_bp (SEM_State& _state, float value) noexcept
        {
            _SEM__bp (_state._state, value);
        }

        void _SEM__bp (_SEM_State& _state, float value) noexcept
        {
            ParamsProcessor__bp (_state.paramsProcessor, value);
        }

        void ParamsProcessor__bp (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.bp = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_filtEnv (SEM_State& _state, float value) noexcept
        {
            _SEM__filtEnv (_state._state, value);
        }

        void _SEM__filtEnv (_SEM_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_lfoCut (SEM_State& _state, float value) noexcept
        {
            _SEM__lfoCut (_state._state, value);
        }

        void _SEM__lfoCut (_SEM_State& _state, float value) noexcept
        {
            ParamsProcessor__lfoCut (_state.paramsProcessor, value);
        }

        void ParamsProcessor__lfoCut (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.lfoCut = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_kbd (SEM_State& _state, float value) noexcept
        {
            _SEM__kbd (_state._state, value);
        }

        void _SEM__kbd (_SEM_State& _state, float value) noexcept
        {
            ParamsProcessor__kbd (_state.paramsProcessor, value);
        }

        void ParamsProcessor__kbd (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.kbd = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_a1 (SEM_State& _state, float value) noexcept
        {
            _SEM__a1 (_state._state, value);
        }

        void _SEM__a1 (_SEM_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_d1 (SEM_State& _state, float value) noexcept
        {
            _SEM__d1 (_state._state, value);
        }

        void _SEM__d1 (_SEM_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_s1 (SEM_State& _state, float value) noexcept
        {
            _SEM__s1 (_state._state, value);
        }

        void _SEM__s1 (_SEM_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_a2 (SEM_State& _state, float value) noexcept
        {
            _SEM__a2 (_state._state, value);
        }

        void _SEM__a2 (_SEM_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_d2 (SEM_State& _state, float value) noexcept
        {
            _SEM__d2 (_state._state, value);
        }

        void _SEM__d2 (_SEM_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_s2 (SEM_State& _state, float value) noexcept
        {
            _SEM__s2 (_state._state, value);
        }

        void _SEM__s2 (_SEM_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_lfoRate (SEM_State& _state, float value) noexcept
        {
            _SEM__lfoRate (_state._state, value);
        }

        void _SEM__lfoRate (_SEM_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_lfoWave (SEM_State& _state, float value) noexcept
        {
            _SEM__lfoWave (_state._state, value);
        }

        void _SEM__lfoWave (_SEM_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_volume (SEM_State& _state, float value) noexcept
        {
            _SEM__volume (_state._state, value);
        }

        void _SEM__volume (_SEM_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_glide (SEM_State& _state, float value) noexcept
        {
            _SEM__glide (_state._state, value);
        }

        void _SEM__glide (_SEM_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_vibRate (SEM_State& _state, float value) noexcept
        {
            _SEM__vibRate (_state._state, value);
        }

        void _SEM__vibRate (_SEM_State& _state, float value) noexcept
        {
            ParamsProcessor__vibRate (_state.paramsProcessor, value);
        }

        void ParamsProcessor__vibRate (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.vibRate = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_vibAmt (SEM_State& _state, float value) noexcept
        {
            _SEM__vibAmt (_state._state, value);
        }

        void _SEM__vibAmt (_SEM_State& _state, float value) noexcept
        {
            ParamsProcessor__vibAmt (_state.paramsProcessor, value);
        }

        void ParamsProcessor__vibAmt (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.vibAmt = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _initialise (SEM_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            _SEM___initialise (_state._state, processorID, sessionID, frequency);
        }

        void _SEM___initialise (_SEM_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            int32_t  i;
            ...
            g__frequency = frequency;
            {
                ...
                for (;;)
                {
                    ...
                    SEMVoice___initialise (_state.voices[i], processorID, sessionID, frequency);
                    {
                        ++i;
                    }
                }
        ...
        void SEMVoice___initialise (_SEMVoice_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
        ...
        void ParamsProcessor___initialise (_ParamsProcessor_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
            _state.params = plugincorp_Params { 0.0f, 0.5f, 0.0f, 0.5f, 0.0f, 0.5f, 0.0f, 0.52f, 0.0f, 0.75f, 0.7f, 0.0f, 0.6f, 0.2f, 0.0f, 0.0f, 0.45f, 0.0f, 0.5f, 0.02f, 0.5f, 0.8f, 0.04f, 0.45f, 0.4f, 0.3f, 0.0f, 0.8f, 0.0f, 0.35f, 0.0f };
        }
        ...
        void _advance (SEM_State& _state, SEM_IO& _io, int32_t _frames) noexcept
        {
            _SEM_IO  ioCopy;
        ...
        void main (_SEM_State& _state, _SEM_IO& _io) noexcept
        {
            Array<_SEMVoice_1_IO, 8>  voices_io;
            ...
            voices_io = Array<_SEMVoice_1_IO, 8> {};
            ...
            }
            {
                ...
                for (;;)
                {
                    ...
                    SEMVoice__main (_state.voices[i], voices_io[i]);
                    {
                        ++i;
                    }
                }
        ...
        void SEMVoice__main (_SEMVoice_1_State& _state, _SEMVoice_1_IO& _io) noexcept
        {
            float  out;
            float  driftC;
            float  lfoHz;
            ...
            _state.drift = SEMVoice__rnd (_state);
            _branch0_0:
            {
                for (;;)
                {
                    ...
                    if (_state.active)
                    {
                        ...
                        driftC = 1.0f + ((_state.drift * 0.0009f) + (_state.vDrift * 0.0006f));
                        lfoHz = 0.05f + ((_state.params.lfoRate * _state.params.lfoRate) * 18.0f);
                        ...
                        lfo = (lsin * (1.0f - _state.params.lfoWave)) + (lsq * _state.params.lfoWave);
                        _state.glideFreq = (_state.glideFreq + ((_state.freq - _state.glideFreq) * _state.glideCoef));
                        vibHz = 0.1f + ((_state.params.vibRate * _state.params.vibRate) * 9.0f);
                        _state.vibPh = (_state.vibPh + (vibHz * _state.invSr));
                        if (_state.vibPh >= 1.0f)
                        {
                            _state.vibPh = (_state.vibPh - 1.0f);
                        }
                        pitchBase = _state.glideFreq * intrinsics::pow (2.0f, (intrinsics::sin (_state.vibPh * 6.2831855f) * _state.params.vibAmt) * 0.04f);
                        det = ((_state.params.v2tune - 0.5f) * 0.5f) + (_state.vDetune * 0.04f);
                        f1 = (pitchBase * driftC) * intrinsics::pow (2.0f, (_state.params.v1tune - 0.5f) * 0.16f);
                        f2 = (pitchBase * driftC) * intrinsics::pow (2.0f, det);
                        inc1 = f1 * _state.invSr;
                        inc2 = f2 * _state.invSr;
                        pw1 = _state.params.v1pw + ((lfo * _state.params.v1mod) * 0.4f);
                        pw2 = _state.params.v2pw + ((lfo * _state.params.v2mod) * 0.4f);
                        prevPh1 = _state.ph1;
                        ...
                        if ((_state.params.sync > 0.5f) ? (_state.ph1 < prevPh1) : false)
                        {
                            _state.ph2 = 0.0f;
                        }
                        o1 = SEMVoice__osc (_state.ph1, inc1, _state.params.v1wave, pw1) * _state.params.mix1;
                        o2 = SEMVoice__osc (_state.ph2, inc2, _state.params.v2wave, pw2) * _state.params.mix2;
                        nz = (SEMVoice__rnd (_state) * _state.params.noise) * 0.4f;
                        sig = ((o1 + o2) + nz) * 0.5f;
                        sig = intrinsics::tanh (sig * 1.2f);
                        yb = (sig - _state.dcx) + (0.9985f * _state.dcy);
                        _state.dcx = sig;
                        _state.dcy = yb;
                        sig = yb;
                        e1a = 1.0f / (intrinsics::max (0.0005f, 0.001f + ((_state.params.a1 * _state.params.a1) * 4.0f)) * _state.sr);
                        e1d = 1.0f / (intrinsics::max (0.0005f, 0.004f + ((_state.params.d1 * _state.params.d1) * 8.0f)) * _state.sr);
                        ...
                        e2a = 1.0f / (intrinsics::max (0.0005f, 0.001f + ((_state.params.a2 * _state.params.a2) * 4.0f)) * _state.sr);
                        e2d = 1.0f / (intrinsics::max (0.0005f, 0.004f + ((_state.params.d2 * _state.params.d2) * 8.0f)) * _state.sr);
                        ...
                        track = 1.0f + (_state.params.kbd * ((_state.freq / 261.6f) - 1.0f));
                        envHz = (_state.params.filtEnv * _state.e2) * 7000.0f;
                        lfoHzC = (_state.params.lfoCut * lfo) * 3500.0f;
                        fc = ((kHz + envHz) + lfoHzC) * track;
                        fc = intrinsics::clamp (fc, 20.0f, _state.sr * 0.45f);
                        g = intrinsics::clamp (intrinsics::tan ((3.1415927f * fc) / _state.sr), 0.0f, 2.0f);
                        k = 2.0f - (_state.params.reso * 1.92f);
                        a1c = 1.0f / (1.0f + (g * (g + k)));
                        v3 = sig - _state.ic2;
                        v1c = (a1c * _state.ic1) + ((a1c * g) * v3);
                        v2c = _state.ic2 + (g * v1c);
                        _state.ic1 = ((2.0f * v1c) - _state.ic1);
                        _state.ic2 = ((2.0f * v2c) - _state.ic2);
                        lp = v2c;
                        bpf = v1c;
                        hp = (sig - (k * v1c)) - v2c;
                        morph = (_state.params.mode < 0.5f) ? ((lp * (1.0f - (_state.params.mode * 2.0f))) + (((lp + hp) * 0.5f) * (_state.params.mode * 2.0f))) : ((((lp + hp) * 0.5f) * (1.0f - ((_state.params.mode - 0.5f) * 2.0f))) + (hp * ((_state.params.mode - 0.5f) * 2.0f)));
                        filtered = (_state.params.bp > 0.5f) ? (bpf * 1.4f) : morph;
                        out = intrinsics::tanh (((filtered * _state.e1) * (0.4f + (0.6f * _state.vel))) * (0.4f + (_state.params.volume * 1.1f)));
                        if (((out != out) ? true : (out > 2.0f)) ? true : (out < -2.0f))
                        {
                            out = 0.0f;
                            _state.ic1 = 0.0f;
                            _state.ic2 = 0.0f;
        ...
        float SEMVoice__osc (float ph, float inc, float wave, float pw) noexcept
        {
            float  saw;
            float  p2;
            float  saw2;
            ...
            pulse = saw - saw2;
            return (saw * (1.0f - wave)) + ((pulse * 0.6f) * wave);
        }
        ...
        static _SEM_State& state_upcast_struct_SEM_State_struct__FKkLoc (_ParamsProcessor_1_State& child) { return *reinterpret_cast<_SEM_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_SEM_State, paramsProcessor))); }
        static _SEM_State& state_upcast_struct_SEM_State_struct__fmAlXb (std_voices_std_voices_VoiceAllocator_specialised_Th5A4_1_State& child) { return *reinterpret_cast<_SEM_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_SEM_State, voiceAllocator))); }
        static SEM_State& state_upcast_struct_SEM_State_struct__EB0l1d (_SEM_State& child) { return *reinterpret_cast<SEM_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (SEM_State, _state))); }
        static _SEM_State& state_upcast_struct_SEM_State_struct__TYB3Pc (std_midi_MPEConverter_1_State& child) { return *reinterpret_cast<_SEM_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_SEM_State, noteTap))); }
        static _SEM_State& state_upcast_struct_SEM_State_struct__KJqYAd (std_midi_MPEConverter_1_State& child) { return *reinterpret_cast<_SEM_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_SEM_State, mpe))); }
...
struct SEM
{
    using PerformerClass = performer::SEM;
    static constexpr auto filename = "SEM.cmajorpatch";
    ...
    static constexpr const char* cmaj_api_cmajpatchconnection_js =
        ...
        "    }\n"
        "}\n";
    static constexpr const char* SEM_cmajorpatch =
        "{\n"
        "  \"CmajorVersion\": 1,\n"
        "  \"ID\": \"com.plugincorp.jwsem\",\n"
        "  \"version\": \"0.1\",\n"
        "  \"name\": \"JW-SEM\",\n"
        "  \"description\": \"TOI JW-SEM - state-variable expander module by plugin corp\",\n"
        "  \"category\": \"instrument\",\n"
        "  \"manufacturer\": \"TOI\",\n"
        "  \"isInstrument\": true,\n"
    ...
    static constexpr std::array files =
    {
        ...
        File { "SEM.cmajorpatch", std::string_view (SEM_cmajorpatch, 372) },
...
// native faceplate (replaces the generic Cmajor UI). See SEMNative.h.
#include "SEMNative.h"

juce::AudioProcessor* JUCE_CALLTYPE createPluginFilter()
{
    return new SemPlugin (std::make_shared<cmaj::Patch>());
}
