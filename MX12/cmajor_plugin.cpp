...
// Auto-generated Cmajor code for patch 'JWMX12'
...
//==============================================================================
/// Auto-generated C++ class for the 'JWMX12' processor
///
...
namespace performer
{
    struct JWMX12
    {
        ...
        static constexpr std::string_view name = "JWMX12";
        ...
        //==============================================================================
        static constexpr uint32_t numInputEndpoints  = 53;
        ...
        static constexpr uint32_t getEndpointHandleForName (std::string_view endpointName)
        {
            ...
            if (endpointName == "fmAmt")      return static_cast<uint32_t> (EndpointHandles::fmAmt);
            if (endpointName == "pw")         return static_cast<uint32_t> (EndpointHandles::pw);
            if (endpointName == "mix")        return static_cast<uint32_t> (EndpointHandles::mix);
            ...
            if (endpointName == "fSlope")     return static_cast<uint32_t> (EndpointHandles::fSlope);
            ...
            if (endpointName == "a3")         return static_cast<uint32_t> (EndpointHandles::a3);
            if (endpointName == "d3")         return static_cast<uint32_t> (EndpointHandles::d3);
            if (endpointName == "s3")         return static_cast<uint32_t> (EndpointHandles::s3);
            ...
            if (endpointName == "m1Src")      return static_cast<uint32_t> (EndpointHandles::m1Src);
            if (endpointName == "m1Dst")      return static_cast<uint32_t> (EndpointHandles::m1Dst);
            if (endpointName == "m1Amt")      return static_cast<uint32_t> (EndpointHandles::m1Amt);
            if (endpointName == "m2Src")      return static_cast<uint32_t> (EndpointHandles::m2Src);
            if (endpointName == "m2Dst")      return static_cast<uint32_t> (EndpointHandles::m2Dst);
            if (endpointName == "m2Amt")      return static_cast<uint32_t> (EndpointHandles::m2Amt);
            if (endpointName == "m3Src")      return static_cast<uint32_t> (EndpointHandles::m3Src);
            if (endpointName == "m3Dst")      return static_cast<uint32_t> (EndpointHandles::m3Dst);
            if (endpointName == "m3Amt")      return static_cast<uint32_t> (EndpointHandles::m3Amt);
            if (endpointName == "m4Src")      return static_cast<uint32_t> (EndpointHandles::m4Src);
            if (endpointName == "m4Dst")      return static_cast<uint32_t> (EndpointHandles::m4Dst);
            if (endpointName == "m4Amt")      return static_cast<uint32_t> (EndpointHandles::m4Amt);
            if (endpointName == "m5Src")      return static_cast<uint32_t> (EndpointHandles::m5Src);
            if (endpointName == "m5Dst")      return static_cast<uint32_t> (EndpointHandles::m5Dst);
            if (endpointName == "m5Amt")      return static_cast<uint32_t> (EndpointHandles::m5Amt);
            if (endpointName == "m6Src")      return static_cast<uint32_t> (EndpointHandles::m6Src);
            if (endpointName == "m6Dst")      return static_cast<uint32_t> (EndpointHandles::m6Dst);
            if (endpointName == "m6Amt")      return static_cast<uint32_t> (EndpointHandles::m6Amt);
        ...
        static constexpr EndpointInfo inputEndpoints[] =
        {
            ...
            { 4,   "o2Wave",     EndpointType::event },
            { 5,   "o2Oct",      EndpointType::event },
            { 6,   "o2Tune",     EndpointType::event },
            { 7,   "sync",       EndpointType::event },
            { 8,   "fmAmt",      EndpointType::event },
            { 9,   "pw",         EndpointType::event },
            { 10,  "mix",        EndpointType::event },
            { 11,  "noiseLvl",   EndpointType::event },
            { 12,  "cutoff",     EndpointType::event },
            { 13,  "reso",       EndpointType::event },
            { 14,  "fMode",      EndpointType::event },
            { 15,  "fSlope",     EndpointType::event },
            { 16,  "fKeyTrack",  EndpointType::event },
            { 17,  "a1",         EndpointType::event },
            { 18,  "d1",         EndpointType::event },
            { 19,  "s1",         EndpointType::event },
            { 20,  "r1",         EndpointType::event },
            { 21,  "a2",         EndpointType::event },
            { 22,  "d2",         EndpointType::event },
            { 23,  "s2",         EndpointType::event },
            { 24,  "r2",         EndpointType::event },
            { 25,  "a3",         EndpointType::event },
            { 26,  "d3",         EndpointType::event },
            { 27,  "s3",         EndpointType::event },
            { 28,  "r3",         EndpointType::event },
            { 29,  "lfo1Rate",   EndpointType::event },
            { 30,  "lfo1Shape",  EndpointType::event },
            { 31,  "lfo2Rate",   EndpointType::event },
            { 32,  "lfo2Shape",  EndpointType::event },
            { 33,  "glide",      EndpointType::event },
            { 34,  "volume",     EndpointType::event },
            { 35,  "tune",       EndpointType::event },
            { 36,  "m1Src",      EndpointType::event },
            { 37,  "m1Dst",      EndpointType::event },
            { 38,  "m1Amt",      EndpointType::event },
            { 39,  "m2Src",      EndpointType::event },
            { 40,  "m2Dst",      EndpointType::event },
            { 41,  "m2Amt",      EndpointType::event },
            { 42,  "m3Src",      EndpointType::event },
            { 43,  "m3Dst",      EndpointType::event },
            { 44,  "m3Amt",      EndpointType::event },
            { 45,  "m4Src",      EndpointType::event },
            { 46,  "m4Dst",      EndpointType::event },
            { 47,  "m4Amt",      EndpointType::event },
            { 48,  "m5Src",      EndpointType::event },
            { 49,  "m5Dst",      EndpointType::event },
            { 50,  "m5Amt",      EndpointType::event },
            { 51,  "m6Src",      EndpointType::event },
            { 52,  "m6Dst",      EndpointType::event },
            { 53,  "m6Amt",      EndpointType::event }
        };

        static constexpr EndpointInfo outputEndpoints[] =
        {
            { 54,  "audioOut",  EndpointType::stream },
            { 55,  "noteOut",   EndpointType::event  }
        };
        ...
        struct _JWMX12_State
        {
            int32_t _sessionID = {};
            double _frequency = {};
            Array<_MX12Voice_1_State, 8> voices;
        ...
        struct JWMX12_eventValue_noteOut
        {
            int32_t frame = {};
            int32_t type = {};
        ...
        struct JWMX12_State
        {
            ...
            _JWMX12_State _state;
            int32_t noteOut_eventCount = {};
            Array<JWMX12_eventValue_noteOut, 32> noteOut;
        };
        ...
        {
            if (endpointHandle == 54) { std::memcpy (reinterpret_cast<char*> (dest), std::addressof (cmajIO.audioOut), 8 * numFramesToCopy); std::memset (reinterpret_cast<char*> (std::addressof (cmajIO.audioOut)), 0, 8 * numFramesToCopy); return; }
        ...
        {
            if (endpointHandle == 55) return (uint32_t) cmajState.noteOut_eventCount;
        ...
        {
            if (endpointHandle == 55) { cmajState.noteOut_eventCount = 0; return; }
        }

        uint32_t getOutputEventType (EndpointHandle endpointHandle, uint32_t index)
        {
            if (endpointHandle == 55) return (uint32_t) cmajState.noteOut[(IndexType) index].type;
            throw std::runtime_error ("Unknown event endpointHandle:" + std::to_string (endpointHandle));
            return {};
        }
        ...
        static uint32_t getOutputEventDataSize (EndpointHandle endpointHandle, uint32_t typeIndex)
        {
            ...
            if (endpointHandle == 55 && typeIndex == 0) return 8;
            throw std::runtime_error ("Unknown event endpointHandle:" + std::to_string (endpointHandle));
            return 0;
        }
        ...
        void addEvent_fmAmt (float event)
        {
            _sendEvent_fmAmt (cmajState, event);
        }
        ...
        void addEvent_mix (float event)
        {
            _sendEvent_mix (cmajState, event);
        }
        ...
        void addEvent_fSlope (float event)
        {
            _sendEvent_fSlope (cmajState, event);
        }
        ...
        void addEvent_a3 (float event)
        {
            _sendEvent_a3 (cmajState, event);
        }

        void addEvent_d3 (float event)
        {
            _sendEvent_d3 (cmajState, event);
        }

        void addEvent_s3 (float event)
        {
            _sendEvent_s3 (cmajState, event);
        }
        ...
        void addEvent_m1Src (float event)
        {
            _sendEvent_m1Src (cmajState, event);
        }

        void addEvent_m1Dst (float event)
        {
            _sendEvent_m1Dst (cmajState, event);
        }

        void addEvent_m1Amt (float event)
        {
            _sendEvent_m1Amt (cmajState, event);
        }

        void addEvent_m2Src (float event)
        {
            _sendEvent_m2Src (cmajState, event);
        }

        void addEvent_m2Dst (float event)
        {
            _sendEvent_m2Dst (cmajState, event);
        }

        void addEvent_m2Amt (float event)
        {
            _sendEvent_m2Amt (cmajState, event);
        }

        void addEvent_m3Src (float event)
        {
            _sendEvent_m3Src (cmajState, event);
        }

        void addEvent_m3Dst (float event)
        {
            _sendEvent_m3Dst (cmajState, event);
        }

        void addEvent_m3Amt (float event)
        {
            _sendEvent_m3Amt (cmajState, event);
        }

        void addEvent_m4Src (float event)
        {
            _sendEvent_m4Src (cmajState, event);
        }

        void addEvent_m4Dst (float event)
        {
            _sendEvent_m4Dst (cmajState, event);
        }

        void addEvent_m4Amt (float event)
        {
            _sendEvent_m4Amt (cmajState, event);
        }

        void addEvent_m5Src (float event)
        {
            _sendEvent_m5Src (cmajState, event);
        }

        void addEvent_m5Dst (float event)
        {
            _sendEvent_m5Dst (cmajState, event);
        }

        void addEvent_m5Amt (float event)
        {
            _sendEvent_m5Amt (cmajState, event);
        }

        void addEvent_m6Src (float event)
        {
            _sendEvent_m6Src (cmajState, event);
        }

        void addEvent_m6Dst (float event)
        {
            _sendEvent_m6Dst (cmajState, event);
        }

        void addEvent_m6Amt (float event)
        {
            _sendEvent_m6Amt (cmajState, event);
        }
        ...
        JWMX12_State cmajState = {};
        JWMX12_IO cmajIO = {};

        //==============================================================================
        void _sendEvent_midiIn (JWMX12_State& _state, std_midi_Message value) noexcept
        {
            _JWMX12__midiIn (_state._state, value);
        }

        void _JWMX12__midiIn (_JWMX12_State& _state, std_midi_Message value) noexcept
        {
        ...
        {
            _JWMX12___forwardEvent (state_upcast_struct_JWMX12_State_stru_iF3Ny(_state), value);
            _JWMX12___forwardEvent_5 (state_upcast_struct_JWMX12_State_stru_iF3Ny(_state), value);
        }

        void _JWMX12___forwardEvent (_JWMX12_State& _state, std_notes_NoteOn value) noexcept
        {
        ...
        {
            _JWMX12___forwardEvent_3 (state_upcast_struct_JWMX12_State_stru_wAbbfe(_state), value);
        }

        void _JWMX12___forwardEvent_3 (_JWMX12_State& _state, plugincorp_NoteEvent value) noexcept
        {
            _JWMX12___writeEvent_noteOut (_state, value);
        }

        void _JWMX12___writeEvent_noteOut (_JWMX12_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
            JWMX12___writeEvent_noteOut (state_upcast_struct_JWMX12_State_struc_hyTEkd(_state), value);
        }

        void JWMX12___writeEvent_noteOut (JWMX12_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
            JWMX12___writeEvent_noteOut_0 (_state, value);
        }

        void JWMX12___writeEvent_noteOut_0 (JWMX12_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
        ...
        void _JWMX12___forwardEvent_5 (_JWMX12_State& _state, std_notes_NoteOn value) noexcept
        {
        ...
        {
            _JWMX12___forwardEvent_12 (state_upcast_struct_JWMX12_State_stru_hzq6uc(_state), index, value);
        }

        void _JWMX12___forwardEvent_12 (_JWMX12_State& _state, int32_t index, std_notes_NoteOff value) noexcept
        {
            MX12Voice__eventsIn (_state.voices[index], value);
        }

        void MX12Voice__eventsIn (_MX12Voice_1_State& _state, std_notes_NoteOff e) noexcept
        {
            _state.e1Stage = int32_t {3};
            _state.e2Stage = int32_t {3};
            _state.e3Stage = int32_t {3};
        }
        ...
        {
            _JWMX12___forwardEvent_11 (state_upcast_struct_JWMX12_State_stru_hzq6uc(_state), index, value);
        }

        void _JWMX12___forwardEvent_11 (_JWMX12_State& _state, int32_t index, std_notes_NoteOn value) noexcept
        {
            MX12Voice__eventsIn_0 (_state.voices[index], value);
        }

        void MX12Voice__eventsIn_0 (_MX12Voice_1_State& _state, std_notes_NoteOn e) noexcept
        {
        ...
        {
            _JWMX12___forwardEvent_2 (state_upcast_struct_JWMX12_State_stru_iF3Ny(_state), value);
            _JWMX12___forwardEvent_6 (state_upcast_struct_JWMX12_State_stru_iF3Ny(_state), value);
        }

        void _JWMX12___forwardEvent_2 (_JWMX12_State& _state, std_notes_NoteOff value) noexcept
        {
        ...
        void _JWMX12___forwardEvent_6 (_JWMX12_State& _state, std_notes_NoteOff value) noexcept
        {
        ...
        {
            _JWMX12___forwardEvent_7 (state_upcast_struct_JWMX12_State_stru_iF3Ny(_state), value);
        }

        void _JWMX12___forwardEvent_7 (_JWMX12_State& _state, std_notes_PitchBend value) noexcept
        {
        ...
        {
            _JWMX12___forwardEvent_9 (state_upcast_struct_JWMX12_State_stru_iF3Ny(_state), value);
        }

        void _JWMX12___forwardEvent_9 (_JWMX12_State& _state, std_notes_Pressure value) noexcept
        {
        ...
        {
            _JWMX12___forwardEvent_8 (state_upcast_struct_JWMX12_State_stru_iF3Ny(_state), value);
        }

        void _JWMX12___forwardEvent_8 (_JWMX12_State& _state, std_notes_Slide value) noexcept
        {
        ...
        {
            _JWMX12___forwardEvent_10 (state_upcast_struct_JWMX12_State_stru_iF3Ny(_state), value);
        }

        void _JWMX12___forwardEvent_10 (_JWMX12_State& _state, std_notes_Control value) noexcept
        {
        ...
        void _sendEvent_o1Wave (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__o1Wave (_state._state, value);
        }

        void _JWMX12__o1Wave (_JWMX12_State& _state, float value) noexcept
        {
        ...
        {
            _JWMX12___forwardEvent_4 (state_upcast_struct_JWMX12_State_stru_A7AoNe(_state), value);
        }

        void _JWMX12___forwardEvent_4 (_JWMX12_State& _state, plugincorp_Params value) noexcept
        {
            int32_t  i;

            {
                ...
                for (;;)
                {
                    ...
                    MX12Voice__paramsIn (_state.voices[i], value);
                    {
                        ++i;
                    }
                }
        ...
        void MX12Voice__paramsIn (_MX12Voice_1_State& _state, plugincorp_Params p) noexcept
        {
            _state.params = p;
        }

        void _sendEvent_o1Oct (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__o1Oct (_state._state, value);
        }

        void _JWMX12__o1Oct (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_o2Wave (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__o2Wave (_state._state, value);
        }

        void _JWMX12__o2Wave (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_o2Oct (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__o2Oct (_state._state, value);
        }

        void _JWMX12__o2Oct (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_o2Tune (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__o2Tune (_state._state, value);
        }

        void _JWMX12__o2Tune (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_sync (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__sync (_state._state, value);
        }

        void _JWMX12__sync (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_fmAmt (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__fmAmt (_state._state, value);
        }

        void _JWMX12__fmAmt (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__fmAmt (_state.paramsProcessor, value);
        }

        void ParamsProcessor__fmAmt (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.fmAmt = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_pw (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__pw (_state._state, value);
        }

        void _JWMX12__pw (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_mix (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__mix (_state._state, value);
        }

        void _JWMX12__mix (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__mix (_state.paramsProcessor, value);
        }

        void ParamsProcessor__mix (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.mix = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_noiseLvl (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__noiseLvl (_state._state, value);
        }

        void _JWMX12__noiseLvl (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_cutoff (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__cutoff (_state._state, value);
        }

        void _JWMX12__cutoff (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_reso (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__reso (_state._state, value);
        }

        void _JWMX12__reso (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_fMode (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__fMode (_state._state, value);
        }

        void _JWMX12__fMode (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_fSlope (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__fSlope (_state._state, value);
        }

        void _JWMX12__fSlope (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__fSlope (_state.paramsProcessor, value);
        }

        void ParamsProcessor__fSlope (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.fSlope = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_fKeyTrack (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__fKeyTrack (_state._state, value);
        }

        void _JWMX12__fKeyTrack (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_a1 (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__a1 (_state._state, value);
        }

        void _JWMX12__a1 (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_d1 (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__d1 (_state._state, value);
        }

        void _JWMX12__d1 (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_s1 (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__s1 (_state._state, value);
        }

        void _JWMX12__s1 (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_r1 (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__r1 (_state._state, value);
        }

        void _JWMX12__r1 (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_a2 (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__a2 (_state._state, value);
        }

        void _JWMX12__a2 (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_d2 (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__d2 (_state._state, value);
        }

        void _JWMX12__d2 (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_s2 (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__s2 (_state._state, value);
        }

        void _JWMX12__s2 (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_r2 (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__r2 (_state._state, value);
        }

        void _JWMX12__r2 (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_a3 (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__a3 (_state._state, value);
        }

        void _JWMX12__a3 (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__a3 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__a3 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.a3 = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_d3 (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__d3 (_state._state, value);
        }

        void _JWMX12__d3 (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__d3 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__d3 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.d3 = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_s3 (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__s3 (_state._state, value);
        }

        void _JWMX12__s3 (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__s3 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__s3 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.s3 = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_r3 (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__r3 (_state._state, value);
        }

        void _JWMX12__r3 (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_lfo1Rate (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__lfo1Rate (_state._state, value);
        }

        void _JWMX12__lfo1Rate (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_lfo1Shape (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__lfo1Shape (_state._state, value);
        }

        void _JWMX12__lfo1Shape (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_lfo2Rate (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__lfo2Rate (_state._state, value);
        }

        void _JWMX12__lfo2Rate (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_lfo2Shape (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__lfo2Shape (_state._state, value);
        }

        void _JWMX12__lfo2Shape (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_glide (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__glide (_state._state, value);
        }

        void _JWMX12__glide (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_volume (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__volume (_state._state, value);
        }

        void _JWMX12__volume (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_tune (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__tune (_state._state, value);
        }

        void _JWMX12__tune (_JWMX12_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_m1Src (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__m1Src (_state._state, value);
        }

        void _JWMX12__m1Src (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__m1Src (_state.paramsProcessor, value);
        }

        void ParamsProcessor__m1Src (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.m1Src = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_m1Dst (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__m1Dst (_state._state, value);
        }

        void _JWMX12__m1Dst (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__m1Dst (_state.paramsProcessor, value);
        }

        void ParamsProcessor__m1Dst (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.m1Dst = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_m1Amt (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__m1Amt (_state._state, value);
        }

        void _JWMX12__m1Amt (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__m1Amt (_state.paramsProcessor, value);
        }

        void ParamsProcessor__m1Amt (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.m1Amt = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_m2Src (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__m2Src (_state._state, value);
        }

        void _JWMX12__m2Src (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__m2Src (_state.paramsProcessor, value);
        }

        void ParamsProcessor__m2Src (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.m2Src = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_m2Dst (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__m2Dst (_state._state, value);
        }

        void _JWMX12__m2Dst (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__m2Dst (_state.paramsProcessor, value);
        }

        void ParamsProcessor__m2Dst (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.m2Dst = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_m2Amt (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__m2Amt (_state._state, value);
        }

        void _JWMX12__m2Amt (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__m2Amt (_state.paramsProcessor, value);
        }

        void ParamsProcessor__m2Amt (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.m2Amt = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_m3Src (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__m3Src (_state._state, value);
        }

        void _JWMX12__m3Src (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__m3Src (_state.paramsProcessor, value);
        }

        void ParamsProcessor__m3Src (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.m3Src = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_m3Dst (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__m3Dst (_state._state, value);
        }

        void _JWMX12__m3Dst (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__m3Dst (_state.paramsProcessor, value);
        }

        void ParamsProcessor__m3Dst (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.m3Dst = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_m3Amt (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__m3Amt (_state._state, value);
        }

        void _JWMX12__m3Amt (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__m3Amt (_state.paramsProcessor, value);
        }

        void ParamsProcessor__m3Amt (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.m3Amt = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_m4Src (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__m4Src (_state._state, value);
        }

        void _JWMX12__m4Src (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__m4Src (_state.paramsProcessor, value);
        }

        void ParamsProcessor__m4Src (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.m4Src = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_m4Dst (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__m4Dst (_state._state, value);
        }

        void _JWMX12__m4Dst (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__m4Dst (_state.paramsProcessor, value);
        }

        void ParamsProcessor__m4Dst (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.m4Dst = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_m4Amt (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__m4Amt (_state._state, value);
        }

        void _JWMX12__m4Amt (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__m4Amt (_state.paramsProcessor, value);
        }

        void ParamsProcessor__m4Amt (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.m4Amt = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_m5Src (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__m5Src (_state._state, value);
        }

        void _JWMX12__m5Src (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__m5Src (_state.paramsProcessor, value);
        }

        void ParamsProcessor__m5Src (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.m5Src = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_m5Dst (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__m5Dst (_state._state, value);
        }

        void _JWMX12__m5Dst (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__m5Dst (_state.paramsProcessor, value);
        }

        void ParamsProcessor__m5Dst (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.m5Dst = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_m5Amt (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__m5Amt (_state._state, value);
        }

        void _JWMX12__m5Amt (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__m5Amt (_state.paramsProcessor, value);
        }

        void ParamsProcessor__m5Amt (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.m5Amt = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_m6Src (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__m6Src (_state._state, value);
        }

        void _JWMX12__m6Src (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__m6Src (_state.paramsProcessor, value);
        }

        void ParamsProcessor__m6Src (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.m6Src = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_m6Dst (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__m6Dst (_state._state, value);
        }

        void _JWMX12__m6Dst (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__m6Dst (_state.paramsProcessor, value);
        }

        void ParamsProcessor__m6Dst (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.m6Dst = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_m6Amt (JWMX12_State& _state, float value) noexcept
        {
            _JWMX12__m6Amt (_state._state, value);
        }

        void _JWMX12__m6Amt (_JWMX12_State& _state, float value) noexcept
        {
            ParamsProcessor__m6Amt (_state.paramsProcessor, value);
        }

        void ParamsProcessor__m6Amt (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.m6Amt = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _initialise (JWMX12_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            _JWMX12___initialise (_state._state, processorID, sessionID, frequency);
        }

        void _JWMX12___initialise (_JWMX12_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            int32_t  i;
            ...
            g__frequency = frequency;
            {
                ...
                for (;;)
                {
                    ...
                    MX12Voice___initialise (_state.voices[i], processorID, sessionID, frequency);
                    {
                        ++i;
                    }
                }
        ...
        void MX12Voice___initialise (_MX12Voice_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
        ...
        void ParamsProcessor___initialise (_ParamsProcessor_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
            _state.params = plugincorp_Params { 0.0f, 0.5f, 0.0f, 0.5f, 0.5f, 0.0f, 0.0f, 0.5f, 0.5f, 0.0f, 0.6f, 0.22f, 0.0f, 0.6f, 0.3f, 0.02f, 0.5f, 0.8f, 0.3f, 0.02f, 0.5f, 0.3f, 0.3f, 0.1f, 0.5f, 0.5f, 0.4f, 0.3f, 0.0f, 0.18f, 0.0f, 0.0f, 0.8f, 0.5f, 0.5f, 0.571f, 0.82f, 0.0f, 0.0f, 0.5f, 0.0f, 0.0f, 0.5f, 0.0f, 0.0f, 0.5f, 0.0f, 0.0f, 0.5f, 0.0f, 0.0f, 0.5f };
        }
        ...
        void _advance (JWMX12_State& _state, JWMX12_IO& _io, int32_t _frames) noexcept
        {
            _JWMX12_IO  ioCopy;
        ...
        void main (_JWMX12_State& _state, _JWMX12_IO& _io) noexcept
        {
            Array<_MX12Voice_1_IO, 8>  voices_io;
            ...
            voices_io = Array<_MX12Voice_1_IO, 8> {};
            ...
            }
            {
                ...
                for (;;)
                {
                    ...
                    MX12Voice__main (_state.voices[i], voices_io[i]);
                    {
                        ++i;
                    }
                }
        ...
        void MX12Voice__main (_MX12Voice_1_State& _state, _MX12Voice_1_IO& _io) noexcept
        {
            float  outL;
            float  outR;
            float  masterTune;
            ...
            Array<float, 6>  _temp;
            Array<float, 6>  ms;
            Array<float, 6>  _temp_0;
            Array<float, 6>  md;
            Array<float, 6>  _temp_1;
            Array<float, 6>  ma;
            float  dPitch;
            float  dOsc2;
            float  dPW;
            ...
            _branch0_0:
            {
                for (;;)
                {
                    ...
                    if (_state.active)
                    {
                        ...
                        l1hz = 0.05f + ((_state.params.lfo1Rate * _state.params.lfo1Rate) * 16.0f);
                        l2hz = 0.03f + ((_state.params.lfo2Rate * _state.params.lfo2Rate) * 10.0f);
                        _state.lfo1Ph = (_state.lfo1Ph + (l1hz * _state.invSr));
                        if (_state.lfo1Ph >= 1.0f)
                        {
                            _state.lfo1Ph = (_state.lfo1Ph - 1.0f);
                        }
                        _state.lfo2Ph = (_state.lfo2Ph + (l2hz * _state.invSr));
                        if (_state.lfo2Ph >= 1.0f)
                        {
                            _state.lfo2Ph = (_state.lfo2Ph - 1.0f);
                        }
                        lfo1 = MX12Voice__lfoVal (_state.lfo1Ph, _state.params.lfo1Shape);
                        lfo2 = MX12Voice__lfoVal (_state.lfo2Ph, _state.params.lfo2Shape);
                        _state.e1 = MX12Voice__advEnv (_state.e1Stage, _state.e1, _state.params.a1, _state.params.d1, _state.params.s1, _state.params.r1, _state.sr);
                        if ((_state.e1Stage == int32_t {0}) ? (_state.e1 >= 1.0f) : false)
                        {
                            _state.e1 = 1.0f;
                            _state.e1Stage = int32_t {1};
                        }
                        if ((_state.e1Stage == int32_t {1}) ? (_state.e1 <= _state.params.s1) : false)
                        {
                            _state.e1 = _state.params.s1;
                            _state.e1Stage = int32_t {2};
                        }
                        if ((_state.e1Stage == int32_t {3}) ? (_state.e1 <= 0.0f) : false)
                        {
                            _state.e1 = 0.0f;
                            _state.e1Stage = int32_t {-1};
                            _state.active = false;
                        }
                        _state.e2 = MX12Voice__advEnv (_state.e2Stage, _state.e2, _state.params.a2, _state.params.d2, _state.params.s2, _state.params.r2, _state.sr);
                        if ((_state.e2Stage == int32_t {0}) ? (_state.e2 >= 1.0f) : false)
                        {
                            _state.e2 = 1.0f;
                            _state.e2Stage = int32_t {1};
                        }
                        if ((_state.e2Stage == int32_t {1}) ? (_state.e2 <= _state.params.s2) : false)
                        {
                            _state.e2 = _state.params.s2;
                            _state.e2Stage = int32_t {2};
                        }
                        if ((_state.e2Stage == int32_t {3}) ? (_state.e2 <= 0.0f) : false)
                        {
                            _state.e2 = 0.0f;
                            _state.e2Stage = int32_t {-1};
                        }
                        _state.e3 = MX12Voice__advEnv (_state.e3Stage, _state.e3, _state.params.a3, _state.params.d3, _state.params.s3, _state.params.r3, _state.sr);
                        if ((_state.e3Stage == int32_t {0}) ? (_state.e3 >= 1.0f) : false)
                        {
                            _state.e3 = 1.0f;
                            _state.e3Stage = int32_t {1};
                        }
                        if ((_state.e3Stage == int32_t {1}) ? (_state.e3 <= _state.params.s3) : false)
                        {
                            _state.e3 = _state.params.s3;
                            _state.e3Stage = int32_t {2};
                        }
                        if ((_state.e3Stage == int32_t {3}) ? (_state.e3 <= 0.0f) : false)
                        {
                            _state.e3 = 0.0f;
                            _state.e3Stage = int32_t {-1};
                        }
                        keytrk = MX12Voice__log2f (_state.glideCur / 261.63f);
                        src = Array<float, 7> {};
                        src[int32_t {0}] = 0.0f;
                        src[int32_t {1}] = lfo1;
                        src[int32_t {2}] = lfo2;
                        src[int32_t {3}] = _state.e2;
                        src[int32_t {4}] = _state.e3;
                        src[int32_t {5}] = _state.vel;
                        src[int32_t {6}] = keytrk;
                        _temp[int32_t {0}] = _state.params.m1Src;
                        _temp[int32_t {1}] = _state.params.m2Src;
                        _temp[int32_t {2}] = _state.params.m3Src;
                        _temp[int32_t {3}] = _state.params.m4Src;
                        _temp[int32_t {4}] = _state.params.m5Src;
                        _temp[int32_t {5}] = _state.params.m6Src;
                        ms = _temp;
                        _temp_0[int32_t {0}] = _state.params.m1Dst;
                        _temp_0[int32_t {1}] = _state.params.m2Dst;
                        _temp_0[int32_t {2}] = _state.params.m3Dst;
                        _temp_0[int32_t {3}] = _state.params.m4Dst;
                        _temp_0[int32_t {4}] = _state.params.m5Dst;
                        _temp_0[int32_t {5}] = _state.params.m6Dst;
                        md = _temp_0;
                        _temp_1[int32_t {0}] = _state.params.m1Amt;
                        _temp_1[int32_t {1}] = _state.params.m2Amt;
                        _temp_1[int32_t {2}] = _state.params.m3Amt;
                        _temp_1[int32_t {3}] = _state.params.m4Amt;
                        _temp_1[int32_t {4}] = _state.params.m5Amt;
                        _temp_1[int32_t {5}] = _state.params.m6Amt;
                        ma = _temp_1;
                        dPitch = 0.0f;
                        dOsc2 = 0.0f;
                        dPW = 0.0f;
                        ...
                        dFM = 0.0f;
                        {
                            ...
                            for (;;)
                            {
                                ...
                                si = static_cast<int32_t> (intrinsics::floor ((ms[i] * 6.0f) + 0.5f));
                                di = static_cast<int32_t> (intrinsics::floor ((md[i] * 7.0f) + 0.5f));
                                cv = src[std__intrinsics___wrap_7 (si)] * ((ma[i] - 0.5f) * 2.0f);
                                if (di == int32_t {1})
                                {
                                    dPitch = (dPitch + cv);
                                }
                        ...
                        pitchMul = intrinsics::pow (2.0f, dPitch * 2.0f);
                        baseF = (_state.glideCur * masterTune) * pitchMul;
                        oct1 = intrinsics::floor ((_state.params.o1Oct * 4.0f) + 0.5f) - 2.0f;
                        oct2 = intrinsics::floor ((_state.params.o2Oct * 4.0f) + 0.5f) - 2.0f;
                        det2 = (_state.params.o2Tune - 0.5f) * 2.0f;
                        fmTot = _state.params.fmAmt + dFM;
                        f1 = intrinsics::clamp (baseF * intrinsics::pow (2.0f, oct1 + ((_state.o2prev * fmTot) * 2.0f)), 0.0f, _state.sr * 0.49f);
                        f2 = intrinsics::clamp (baseF * intrinsics::pow (2.0f, (oct2 + det2) + (dOsc2 * 2.0f)), 0.0f, _state.sr * 0.49f);
                        inc1 = f1 * _state.invSr;
                        inc2 = f2 * _state.invSr;
                        pwEff = intrinsics::clamp (_state.params.pw + (dPW * 0.45f), 0.05f, 0.95f);
                        prev1 = _state.ph1;
                        ...
                        if ((_state.params.sync >= 0.5f) ? (_state.ph1 < prev1) : false)
                        {
                            _state.ph2 = 0.0f;
                        }
                        o1 = MX12Voice__vcoWave (_state.ph1, inc1, _state.params.o1Wave, pwEff);
                        o2 = MX12Voice__vcoWave (_state.ph2, inc2, _state.params.o2Wave, pwEff);
                        _state.o2prev = o2;
                        mixv = intrinsics::clamp (_state.params.mix, 0.0f, 1.0f);
                        sig = ((o1 * (1.0f - mixv)) + (o2 * mixv)) + ((MX12Voice__rnd (_state) * _state.params.noiseLvl) * 0.5f);
                        sig = intrinsics::tanh (sig * 1.1f);
                        yb = (sig - _state.dcx) + (0.9985f * _state.dcy);
                        _state.dcx = sig;
                        _state.dcy = yb;
                        sig = yb;
                        kbd = _state.params.fKeyTrack * keytrk;
                        fcOct = ((_state.params.cutoff * 9.0f) + (dCut * 6.0f)) + kbd;
                        fcHz = intrinsics::clamp (30.0f * intrinsics::pow (2.0f, fcOct), 25.0f, _state.sr * 0.45f);
                        g = intrinsics::tan ((3.1415927f * fcHz) * _state.invSr);
                        kq = intrinsics::clamp (1.0f / (0.5f + (((_state.params.reso + dReso) * (_state.params.reso + dReso)) * 9.0f)), 0.05f, 2.0f);
                        a1c = 1.0f / (1.0f + (g * (g + kq)));
                        a2c = g * a1c;
                        a3c = g * a2c;
                        v3 = sig - _state.ic2;
                        v1 = (a1c * _state.ic1) + (a2c * v3);
                        v2 = (_state.ic2 + (a2c * _state.ic1)) + (a3c * v3);
                        ...
                        notch = lp + hp;
                        mm = lp;
                        fm = intrinsics::clamp (_state.params.fMode, 0.0f, 1.0f);
                        if (fm < 0.34f)
                        {
                            m = fm / 0.34f;
                            mm = ((lp * (1.0f - m)) + (bp * m));
                        ...
                        else
                        {
                            ...
                            {
                                m_0 = (fm - 0.34f) / 0.33f;
                                mm = ((bp * (1.0f - m_0)) + (hp * m_0));
                            }
                            else
                            {
                                m_1 = (fm - 0.67f) / 0.33f;
                                mm = ((hp * (1.0f - m_1)) + (notch * m_1));
                            }
                        }
                        v3b = mm - _state.ic2b;
                        v1b = (a1c * _state.ic1b) + (a2c * v3b);
                        v2b = (_state.ic2b + (a2c * _state.ic1b)) + (a3c * v3b);
                        _state.ic1b = ((2.0f * v1b) - _state.ic1b);
                        _state.ic2b = ((2.0f * v2b) - _state.ic2b);
                        filt = (mm * (1.0f - _state.params.fSlope)) + (v2b * _state.params.fSlope);
                        amp = (((filt * _state.e1) * intrinsics::clamp (1.0f + dVca, 0.0f, 2.0f)) * (0.4f + (0.6f * _state.vel))) * (0.35f + (_state.params.volume * 1.0f));
                    ...
                    _temp_2[int32_t {0}] = outL;
                    _temp_2[int32_t {1}] = outR;
                    _io.voiceOut = (_io.voiceOut + _temp_2);
        ...
        float MX12Voice__lfoVal (float ph, float shp) noexcept
        {
            float  tri;
            float  saw;
            float  sq;
        ...
        float MX12Voice__advEnv (int32_t stage, float lvl, float a, float d, float s, float r, float sr) noexcept
        {
            float  ar;
            float  dr;
            float  rr;
            ...
            ar = 1.0f / (intrinsics::max (0.0006f, 0.001f + ((a * a) * 5.0f)) * sr);
            dr = 1.0f / (intrinsics::max (0.0006f, 0.005f + ((d * d) * 8.0f)) * sr);
            rr = 1.0f / (intrinsics::max (0.0006f, 0.005f + ((r * r) * 8.0f)) * sr);
            L = lvl;
            if (stage == int32_t {0})
            {
                L = (L + ar);
        ...
        float MX12Voice__log2f (float x) noexcept
        {
        ...
        float MX12Voice__vcoWave (float p, float inc, float wave, float pwv) noexcept
        {
            float  saw;
            float  tri;
            float  p2;
        ...
        float MX12Voice__rnd (_MX12Voice_1_State& _state) noexcept
        {
            float  s;
        ...
        static _JWMX12_State& state_upcast_struct_JWMX12_State_stru_wAbbfe (std_midi_MPEConverter_1_State& child) { return *reinterpret_cast<_JWMX12_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_JWMX12_State, noteTap))); }
        static _JWMX12_State& state_upcast_struct_JWMX12_State_stru_A7AoNe (_ParamsProcessor_1_State& child) { return *reinterpret_cast<_JWMX12_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_JWMX12_State, paramsProcessor))); }
        static _JWMX12_State& state_upcast_struct_JWMX12_State_stru_hzq6uc (std_voices_std_voices_VoiceAllocator_specialised_Th5A4_1_State& child) { return *reinterpret_cast<_JWMX12_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_JWMX12_State, voiceAllocator))); }
        static JWMX12_State& state_upcast_struct_JWMX12_State_struc_hyTEkd (_JWMX12_State& child) { return *reinterpret_cast<JWMX12_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (JWMX12_State, _state))); }
        static _JWMX12_State& state_upcast_struct_JWMX12_State_stru_iF3Ny (std_midi_MPEConverter_1_State& child) { return *reinterpret_cast<_JWMX12_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_JWMX12_State, mpe))); }
...
struct JWMX12
{
    using PerformerClass = performer::JWMX12;
    static constexpr auto filename = "MX12.cmajorpatch";
    ...
    static constexpr const char* cmaj_api_cmajpatchconnection_js =
        ...
        "    }\n"
        "}\n";
    static constexpr const char* MX12_cmajorpatch =
        "{\n"
        "  \"CmajorVersion\": 1,\n"
        "  \"ID\": \"com.plugincorp.jwmx12\",\n"
        "  \"version\": \"0.1\",\n"
        "  \"name\": \"JW-MX12\",\n"
        "  \"description\": \"TOI JW-MX12 - deep 8-voice analog poly with a mod matrix by plugin corp\",\n"
        "  \"category\": \"instrument\",\n"
        "  \"manufacturer\": \"TOI\",\n"
        "  \"isInstrument\": true,\n"
    ...
    static constexpr std::array files =
    {
        ...
        File { "MX12.cmajorpatch", std::string_view (MX12_cmajorpatch, 388) },
...
// native faceplate (replaces the generic Cmajor UI). See MX12Native.h.
#include "MX12Native.h"

juce::AudioProcessor* JUCE_CALLTYPE createPluginFilter()
{
    return new JWMX12Plugin (std::make_shared<cmaj::Patch>());
}
