...
// Auto-generated Cmajor code for patch 'CS80'
...
//==============================================================================
/// Auto-generated C++ class for the 'CS80' processor
///
...
namespace performer
{
    struct CS80
    {
        ...
        static constexpr std::string_view name = "CS80";
        ...
        //==============================================================================
        static constexpr uint32_t numInputEndpoints  = 36;
        ...
        static constexpr uint32_t getEndpointHandleForName (std::string_view endpointName)
        {
            ...
            if (endpointName == "c1Wave")      return static_cast<uint32_t> (EndpointHandles::c1Wave);
            if (endpointName == "c1PW")        return static_cast<uint32_t> (EndpointHandles::c1PW);
            if (endpointName == "c1Hpf")       return static_cast<uint32_t> (EndpointHandles::c1Hpf);
            if (endpointName == "c1HpfR")      return static_cast<uint32_t> (EndpointHandles::c1HpfR);
            if (endpointName == "c1Lpf")       return static_cast<uint32_t> (EndpointHandles::c1Lpf);
            if (endpointName == "c1LpfR")      return static_cast<uint32_t> (EndpointHandles::c1LpfR);
            if (endpointName == "c1FEnv")      return static_cast<uint32_t> (EndpointHandles::c1FEnv);
            if (endpointName == "c1A")         return static_cast<uint32_t> (EndpointHandles::c1A);
            if (endpointName == "c1D")         return static_cast<uint32_t> (EndpointHandles::c1D);
            if (endpointName == "c1S")         return static_cast<uint32_t> (EndpointHandles::c1S);
            if (endpointName == "c1R")         return static_cast<uint32_t> (EndpointHandles::c1R);
            if (endpointName == "c1Brill")     return static_cast<uint32_t> (EndpointHandles::c1Brill);
            if (endpointName == "c1Level")     return static_cast<uint32_t> (EndpointHandles::c1Level);
            if (endpointName == "c2Wave")      return static_cast<uint32_t> (EndpointHandles::c2Wave);
            if (endpointName == "c2PW")        return static_cast<uint32_t> (EndpointHandles::c2PW);
            if (endpointName == "c2Hpf")       return static_cast<uint32_t> (EndpointHandles::c2Hpf);
            if (endpointName == "c2HpfR")      return static_cast<uint32_t> (EndpointHandles::c2HpfR);
            if (endpointName == "c2Lpf")       return static_cast<uint32_t> (EndpointHandles::c2Lpf);
            if (endpointName == "c2LpfR")      return static_cast<uint32_t> (EndpointHandles::c2LpfR);
            if (endpointName == "c2FEnv")      return static_cast<uint32_t> (EndpointHandles::c2FEnv);
            if (endpointName == "c2A")         return static_cast<uint32_t> (EndpointHandles::c2A);
            if (endpointName == "c2D")         return static_cast<uint32_t> (EndpointHandles::c2D);
            if (endpointName == "c2S")         return static_cast<uint32_t> (EndpointHandles::c2S);
            if (endpointName == "c2R")         return static_cast<uint32_t> (EndpointHandles::c2R);
            if (endpointName == "c2Brill")     return static_cast<uint32_t> (EndpointHandles::c2Brill);
            if (endpointName == "c2Level")     return static_cast<uint32_t> (EndpointHandles::c2Level);
            ...
            if (endpointName == "ring")        return static_cast<uint32_t> (EndpointHandles::ring);
            if (endpointName == "sub")         return static_cast<uint32_t> (EndpointHandles::sub);
            if (endpointName == "touchBrill")  return static_cast<uint32_t> (EndpointHandles::touchBrill);
        ...
        static constexpr EndpointInfo inputEndpoints[] =
        {
            ...
            { 2,   "lfoRate",     EndpointType::event },
            { 3,   "lfoShape",    EndpointType::event },
            { 4,   "c1Wave",      EndpointType::event },
            { 5,   "c1PW",        EndpointType::event },
            { 6,   "c1Hpf",       EndpointType::event },
            { 7,   "c1HpfR",      EndpointType::event },
            { 8,   "c1Lpf",       EndpointType::event },
            { 9,   "c1LpfR",      EndpointType::event },
            { 10,  "c1FEnv",      EndpointType::event },
            { 11,  "c1A",         EndpointType::event },
            { 12,  "c1D",         EndpointType::event },
            { 13,  "c1S",         EndpointType::event },
            { 14,  "c1R",         EndpointType::event },
            { 15,  "c1Brill",     EndpointType::event },
            { 16,  "c1Level",     EndpointType::event },
            { 17,  "c2Wave",      EndpointType::event },
            { 18,  "c2PW",        EndpointType::event },
            { 19,  "c2Hpf",       EndpointType::event },
            { 20,  "c2HpfR",      EndpointType::event },
            { 21,  "c2Lpf",       EndpointType::event },
            { 22,  "c2LpfR",      EndpointType::event },
            { 23,  "c2FEnv",      EndpointType::event },
            { 24,  "c2A",         EndpointType::event },
            { 25,  "c2D",         EndpointType::event },
            { 26,  "c2S",         EndpointType::event },
            { 27,  "c2R",         EndpointType::event },
            { 28,  "c2Brill",     EndpointType::event },
            { 29,  "c2Level",     EndpointType::event },
            { 30,  "detune",      EndpointType::event },
            { 31,  "mix",         EndpointType::event },
            { 32,  "ring",        EndpointType::event },
            { 33,  "sub",         EndpointType::event },
            { 34,  "touchBrill",  EndpointType::event },
            { 35,  "volume",      EndpointType::event },
            { 36,  "tune",        EndpointType::event }
        };

        static constexpr EndpointInfo outputEndpoints[] =
        {
            { 37,  "audioOut",  EndpointType::stream },
            { 38,  "noteOut",   EndpointType::event  }
        };
        ...
        struct _CS80_State
        {
            int32_t _sessionID = {};
            double _frequency = {};
            Array<_CS80Voice_1_State, 8> voices;
        ...
        struct CS80_eventValue_noteOut
        {
            int32_t frame = {};
            int32_t type = {};
        ...
        struct CS80_State
        {
            ...
            Array<CS80_eventValue_noteOut, 32> noteOut;
        };
        ...
        {
            if (endpointHandle == 37) { std::memcpy (reinterpret_cast<char*> (dest), std::addressof (cmajIO.audioOut), 8 * numFramesToCopy); std::memset (reinterpret_cast<char*> (std::addressof (cmajIO.audioOut)), 0, 8 * numFramesToCopy); return; }
        ...
        {
            if (endpointHandle == 38) return (uint32_t) cmajState.noteOut_eventCount;
        ...
        {
            if (endpointHandle == 38) { cmajState.noteOut_eventCount = 0; return; }
        }

        uint32_t getOutputEventType (EndpointHandle endpointHandle, uint32_t index)
        {
            if (endpointHandle == 38) return (uint32_t) cmajState.noteOut[(IndexType) index].type;
            throw std::runtime_error ("Unknown event endpointHandle:" + std::to_string (endpointHandle));
            return {};
        }
        ...
        static uint32_t getOutputEventDataSize (EndpointHandle endpointHandle, uint32_t typeIndex)
        {
            ...
            if (endpointHandle == 38 && typeIndex == 0) return 8;
            throw std::runtime_error ("Unknown event endpointHandle:" + std::to_string (endpointHandle));
            return 0;
        }
        ...
        void addEvent_c1Wave (float event)
        {
            _sendEvent_c1Wave (cmajState, event);
        }

        void addEvent_c1PW (float event)
        {
            _sendEvent_c1PW (cmajState, event);
        }

        void addEvent_c1Hpf (float event)
        {
            _sendEvent_c1Hpf (cmajState, event);
        }

        void addEvent_c1HpfR (float event)
        {
            _sendEvent_c1HpfR (cmajState, event);
        }

        void addEvent_c1Lpf (float event)
        {
            _sendEvent_c1Lpf (cmajState, event);
        }

        void addEvent_c1LpfR (float event)
        {
            _sendEvent_c1LpfR (cmajState, event);
        }

        void addEvent_c1FEnv (float event)
        {
            _sendEvent_c1FEnv (cmajState, event);
        }

        void addEvent_c1A (float event)
        {
            _sendEvent_c1A (cmajState, event);
        }

        void addEvent_c1D (float event)
        {
            _sendEvent_c1D (cmajState, event);
        }

        void addEvent_c1S (float event)
        {
            _sendEvent_c1S (cmajState, event);
        }

        void addEvent_c1R (float event)
        {
            _sendEvent_c1R (cmajState, event);
        }

        void addEvent_c1Brill (float event)
        {
            _sendEvent_c1Brill (cmajState, event);
        }

        void addEvent_c1Level (float event)
        {
            _sendEvent_c1Level (cmajState, event);
        }

        void addEvent_c2Wave (float event)
        {
            _sendEvent_c2Wave (cmajState, event);
        }

        void addEvent_c2PW (float event)
        {
            _sendEvent_c2PW (cmajState, event);
        }

        void addEvent_c2Hpf (float event)
        {
            _sendEvent_c2Hpf (cmajState, event);
        }

        void addEvent_c2HpfR (float event)
        {
            _sendEvent_c2HpfR (cmajState, event);
        }

        void addEvent_c2Lpf (float event)
        {
            _sendEvent_c2Lpf (cmajState, event);
        }

        void addEvent_c2LpfR (float event)
        {
            _sendEvent_c2LpfR (cmajState, event);
        }

        void addEvent_c2FEnv (float event)
        {
            _sendEvent_c2FEnv (cmajState, event);
        }

        void addEvent_c2A (float event)
        {
            _sendEvent_c2A (cmajState, event);
        }

        void addEvent_c2D (float event)
        {
            _sendEvent_c2D (cmajState, event);
        }

        void addEvent_c2S (float event)
        {
            _sendEvent_c2S (cmajState, event);
        }

        void addEvent_c2R (float event)
        {
            _sendEvent_c2R (cmajState, event);
        }

        void addEvent_c2Brill (float event)
        {
            _sendEvent_c2Brill (cmajState, event);
        }

        void addEvent_c2Level (float event)
        {
            _sendEvent_c2Level (cmajState, event);
        }
        ...
        void addEvent_ring (float event)
        {
            _sendEvent_ring (cmajState, event);
        }

        void addEvent_sub (float event)
        {
            _sendEvent_sub (cmajState, event);
        }

        void addEvent_touchBrill (float event)
        {
            _sendEvent_touchBrill (cmajState, event);
        }
        ...
        void addEvent (EndpointHandle endpointHandle, uint32_t typeIndex, const unsigned char* eventData)
        {
            ...
            if (endpointHandle == 34)
            {
                ...
                memcpy (&value, eventData, 4);
                eventData += 4;
                return addEvent_touchBrill (value);
            }
        ...
        CS80_State cmajState = {};
        CS80_IO cmajIO = {};

        //==============================================================================
        void _sendEvent_midiIn (CS80_State& _state, std_midi_Message value) noexcept
        {
            _CS80__midiIn (_state._state, value);
        }

        void _CS80__midiIn (_CS80_State& _state, std_midi_Message value) noexcept
        {
        ...
        {
            _CS80___forwardEvent (state_upcast_struct_CS80_State_struct_GQTfab(_state), value);
            _CS80___forwardEvent_5 (state_upcast_struct_CS80_State_struct_GQTfab(_state), value);
        }

        void _CS80___forwardEvent (_CS80_State& _state, std_notes_NoteOn value) noexcept
        {
        ...
        {
            _CS80___forwardEvent_3 (state_upcast_struct_CS80_State_struct_gAAgvc(_state), value);
        }

        void _CS80___forwardEvent_3 (_CS80_State& _state, plugincorp_NoteEvent value) noexcept
        {
            _CS80___writeEvent_noteOut (_state, value);
        }

        void _CS80___writeEvent_noteOut (_CS80_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
            CS80___writeEvent_noteOut (state_upcast_struct_CS80_State_struct__YgHJR(_state), value);
        }

        void CS80___writeEvent_noteOut (CS80_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
            CS80___writeEvent_noteOut_0 (_state, value);
        }

        void CS80___writeEvent_noteOut_0 (CS80_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
        ...
        void _CS80___forwardEvent_5 (_CS80_State& _state, std_notes_NoteOn value) noexcept
        {
        ...
        {
            _CS80___forwardEvent_12 (state_upcast_struct_CS80_State_struct_zJVDIb(_state), index, value);
        }

        void _CS80___forwardEvent_12 (_CS80_State& _state, int32_t index, std_notes_NoteOff value) noexcept
        {
            CS80Voice__eventsIn (_state.voices[index], value);
        }

        void CS80Voice__eventsIn (_CS80Voice_1_State& _state, std_notes_NoteOff e) noexcept
        {
        ...
        {
            _CS80___forwardEvent_11 (state_upcast_struct_CS80_State_struct_zJVDIb(_state), index, value);
        }

        void _CS80___forwardEvent_11 (_CS80_State& _state, int32_t index, std_notes_NoteOn value) noexcept
        {
            CS80Voice__eventsIn_0 (_state.voices[index], value);
        }

        void CS80Voice__eventsIn_0 (_CS80Voice_1_State& _state, std_notes_NoteOn e) noexcept
        {
            ...
            _state.vDrift1 = CS80Voice__rnd (_state);
            _state.vDrift2 = CS80Voice__rnd (_state);
            _state.vPan = CS80Voice__rnd (_state);
        }
        ...
        float CS80Voice__rnd (_CS80Voice_1_State& _state) noexcept
        {
            float  s;
        ...
        {
            _CS80___forwardEvent_2 (state_upcast_struct_CS80_State_struct_GQTfab(_state), value);
            _CS80___forwardEvent_6 (state_upcast_struct_CS80_State_struct_GQTfab(_state), value);
        }

        void _CS80___forwardEvent_2 (_CS80_State& _state, std_notes_NoteOff value) noexcept
        {
        ...
        void _CS80___forwardEvent_6 (_CS80_State& _state, std_notes_NoteOff value) noexcept
        {
        ...
        {
            _CS80___forwardEvent_7 (state_upcast_struct_CS80_State_struct_GQTfab(_state), value);
        }

        void _CS80___forwardEvent_7 (_CS80_State& _state, std_notes_PitchBend value) noexcept
        {
        ...
        {
            _CS80___forwardEvent_9 (state_upcast_struct_CS80_State_struct_GQTfab(_state), value);
        }

        void _CS80___forwardEvent_9 (_CS80_State& _state, std_notes_Pressure value) noexcept
        {
        ...
        {
            _CS80___forwardEvent_8 (state_upcast_struct_CS80_State_struct_GQTfab(_state), value);
        }

        void _CS80___forwardEvent_8 (_CS80_State& _state, std_notes_Slide value) noexcept
        {
        ...
        {
            _CS80___forwardEvent_10 (state_upcast_struct_CS80_State_struct_GQTfab(_state), value);
        }

        void _CS80___forwardEvent_10 (_CS80_State& _state, std_notes_Control value) noexcept
        {
        ...
        void _sendEvent_lfoRate (CS80_State& _state, float value) noexcept
        {
            _CS80__lfoRate (_state._state, value);
        }

        void _CS80__lfoRate (_CS80_State& _state, float value) noexcept
        {
        ...
        {
            _CS80___forwardEvent_4 (state_upcast_struct_CS80_State_struct_VcMpkb(_state), value);
        }

        void _CS80___forwardEvent_4 (_CS80_State& _state, plugincorp_Params value) noexcept
        {
            int32_t  i;

            {
                ...
                for (;;)
                {
                    ...
                    CS80Voice__paramsIn (_state.voices[i], value);
                    {
                        ++i;
                    }
                }
        ...
        void CS80Voice__paramsIn (_CS80Voice_1_State& _state, plugincorp_Params p) noexcept
        {
            _state.params = p;
        }

        void _sendEvent_lfoShape (CS80_State& _state, float value) noexcept
        {
            _CS80__lfoShape (_state._state, value);
        }

        void _CS80__lfoShape (_CS80_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_c1Wave (CS80_State& _state, float value) noexcept
        {
            _CS80__c1Wave (_state._state, value);
        }

        void _CS80__c1Wave (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c1Wave (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c1Wave (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c1Wave = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c1PW (CS80_State& _state, float value) noexcept
        {
            _CS80__c1PW (_state._state, value);
        }

        void _CS80__c1PW (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c1PW (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c1PW (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c1PW = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c1Hpf (CS80_State& _state, float value) noexcept
        {
            _CS80__c1Hpf (_state._state, value);
        }

        void _CS80__c1Hpf (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c1Hpf (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c1Hpf (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c1Hpf = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c1HpfR (CS80_State& _state, float value) noexcept
        {
            _CS80__c1HpfR (_state._state, value);
        }

        void _CS80__c1HpfR (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c1HpfR (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c1HpfR (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c1HpfR = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c1Lpf (CS80_State& _state, float value) noexcept
        {
            _CS80__c1Lpf (_state._state, value);
        }

        void _CS80__c1Lpf (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c1Lpf (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c1Lpf (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c1Lpf = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c1LpfR (CS80_State& _state, float value) noexcept
        {
            _CS80__c1LpfR (_state._state, value);
        }

        void _CS80__c1LpfR (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c1LpfR (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c1LpfR (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c1LpfR = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c1FEnv (CS80_State& _state, float value) noexcept
        {
            _CS80__c1FEnv (_state._state, value);
        }

        void _CS80__c1FEnv (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c1FEnv (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c1FEnv (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c1FEnv = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c1A (CS80_State& _state, float value) noexcept
        {
            _CS80__c1A (_state._state, value);
        }

        void _CS80__c1A (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c1A (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c1A (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c1A = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c1D (CS80_State& _state, float value) noexcept
        {
            _CS80__c1D (_state._state, value);
        }

        void _CS80__c1D (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c1D (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c1D (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c1D = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c1S (CS80_State& _state, float value) noexcept
        {
            _CS80__c1S (_state._state, value);
        }

        void _CS80__c1S (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c1S (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c1S (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c1S = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c1R (CS80_State& _state, float value) noexcept
        {
            _CS80__c1R (_state._state, value);
        }

        void _CS80__c1R (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c1R (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c1R (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c1R = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c1Brill (CS80_State& _state, float value) noexcept
        {
            _CS80__c1Brill (_state._state, value);
        }

        void _CS80__c1Brill (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c1Brill (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c1Brill (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c1Brill = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c1Level (CS80_State& _state, float value) noexcept
        {
            _CS80__c1Level (_state._state, value);
        }

        void _CS80__c1Level (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c1Level (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c1Level (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c1Level = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c2Wave (CS80_State& _state, float value) noexcept
        {
            _CS80__c2Wave (_state._state, value);
        }

        void _CS80__c2Wave (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c2Wave (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c2Wave (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c2Wave = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c2PW (CS80_State& _state, float value) noexcept
        {
            _CS80__c2PW (_state._state, value);
        }

        void _CS80__c2PW (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c2PW (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c2PW (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c2PW = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c2Hpf (CS80_State& _state, float value) noexcept
        {
            _CS80__c2Hpf (_state._state, value);
        }

        void _CS80__c2Hpf (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c2Hpf (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c2Hpf (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c2Hpf = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c2HpfR (CS80_State& _state, float value) noexcept
        {
            _CS80__c2HpfR (_state._state, value);
        }

        void _CS80__c2HpfR (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c2HpfR (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c2HpfR (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c2HpfR = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c2Lpf (CS80_State& _state, float value) noexcept
        {
            _CS80__c2Lpf (_state._state, value);
        }

        void _CS80__c2Lpf (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c2Lpf (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c2Lpf (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c2Lpf = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c2LpfR (CS80_State& _state, float value) noexcept
        {
            _CS80__c2LpfR (_state._state, value);
        }

        void _CS80__c2LpfR (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c2LpfR (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c2LpfR (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c2LpfR = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c2FEnv (CS80_State& _state, float value) noexcept
        {
            _CS80__c2FEnv (_state._state, value);
        }

        void _CS80__c2FEnv (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c2FEnv (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c2FEnv (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c2FEnv = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c2A (CS80_State& _state, float value) noexcept
        {
            _CS80__c2A (_state._state, value);
        }

        void _CS80__c2A (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c2A (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c2A (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c2A = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c2D (CS80_State& _state, float value) noexcept
        {
            _CS80__c2D (_state._state, value);
        }

        void _CS80__c2D (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c2D (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c2D (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c2D = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c2S (CS80_State& _state, float value) noexcept
        {
            _CS80__c2S (_state._state, value);
        }

        void _CS80__c2S (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c2S (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c2S (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c2S = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c2R (CS80_State& _state, float value) noexcept
        {
            _CS80__c2R (_state._state, value);
        }

        void _CS80__c2R (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c2R (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c2R (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c2R = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c2Brill (CS80_State& _state, float value) noexcept
        {
            _CS80__c2Brill (_state._state, value);
        }

        void _CS80__c2Brill (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c2Brill (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c2Brill (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c2Brill = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_c2Level (CS80_State& _state, float value) noexcept
        {
            _CS80__c2Level (_state._state, value);
        }

        void _CS80__c2Level (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__c2Level (_state.paramsProcessor, value);
        }

        void ParamsProcessor__c2Level (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.c2Level = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_detune (CS80_State& _state, float value) noexcept
        {
            _CS80__detune (_state._state, value);
        }

        void _CS80__detune (_CS80_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_mix (CS80_State& _state, float value) noexcept
        {
            _CS80__mix (_state._state, value);
        }

        void _CS80__mix (_CS80_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_ring (CS80_State& _state, float value) noexcept
        {
            _CS80__ring (_state._state, value);
        }

        void _CS80__ring (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__ring (_state.paramsProcessor, value);
        }

        void ParamsProcessor__ring (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.ring = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_sub (CS80_State& _state, float value) noexcept
        {
            _CS80__sub (_state._state, value);
        }

        void _CS80__sub (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__sub (_state.paramsProcessor, value);
        }

        void ParamsProcessor__sub (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.sub = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_touchBrill (CS80_State& _state, float value) noexcept
        {
            _CS80__touchBrill (_state._state, value);
        }

        void _CS80__touchBrill (_CS80_State& _state, float value) noexcept
        {
            ParamsProcessor__touchBrill (_state.paramsProcessor, value);
        }

        void ParamsProcessor__touchBrill (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.touchBrill = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_volume (CS80_State& _state, float value) noexcept
        {
            _CS80__volume (_state._state, value);
        }

        void _CS80__volume (_CS80_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_tune (CS80_State& _state, float value) noexcept
        {
            _CS80__tune (_state._state, value);
        }

        void _CS80__tune (_CS80_State& _state, float value) noexcept
        {
        ...
        void _initialise (CS80_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            _CS80___initialise (_state._state, processorID, sessionID, frequency);
        }

        void _CS80___initialise (_CS80_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            int32_t  i;
            ...
            g__frequency = frequency;
            {
                ...
                for (;;)
                {
                    ...
                    CS80Voice___initialise (_state.voices[i], processorID, sessionID, frequency);
                    {
                        ++i;
                    }
                }
        ...
        void CS80Voice___initialise (_CS80Voice_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
            _state.rngc = int32_t {0};
            _state.freq = 220.0f;
            _state.vel = 0.0f;
            _state.active = false;
        ...
        void ParamsProcessor___initialise (_ParamsProcessor_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
            _state.params = plugincorp_Params { 0.25f, 0.0f, 0.0f, 0.5f, 0.0f, 0.1f, 0.55f, 0.2f, 0.35f, 0.3f, 0.5f, 0.75f, 0.5f, 0.55f, 0.8f, 0.0f, 0.5f, 0.0f, 0.1f, 0.5f, 0.2f, 0.35f, 0.4f, 0.5f, 0.75f, 0.55f, 0.5f, 0.75f, 0.3f, 0.5f, 0.0f, 0.0f, 0.3f, 0.8f, 0.5f };
        }
        ...
        void _advance (CS80_State& _state, CS80_IO& _io, int32_t _frames) noexcept
        {
            _CS80_IO  ioCopy;
        ...
        void main (_CS80_State& _state, _CS80_IO& _io) noexcept
        {
            Array<_CS80Voice_1_IO, 8>  voices_io;
            ...
            voices_io = Array<_CS80Voice_1_IO, 8> {};
            ...
            }
            {
                ...
                for (;;)
                {
                    ...
                    CS80Voice__main (_state.voices[i], voices_io[i]);
                    {
                        ++i;
                    }
                }
        ...
        void CS80Voice__main (_CS80Voice_1_State& _state, _CS80Voice_1_IO& _io) noexcept
        {
            float  outL;
            float  outR;
            float  drift1;
            ...
            _state.drA = CS80Voice__rnd (_state);
            _state.drB = CS80Voice__rnd (_state);
            _branch0_0:
            {
                for (;;)
                {
                    ...
                    if (_state.active)
                    {
                        _state.driftPh = (_state.driftPh + (4.5f * _state.invSr));
                        if (_state.driftPh >= 1.0f)
                        {
                            _state.driftPh = (_state.driftPh - 1.0f);
                            _state.drA = CS80Voice__rnd (_state);
                            _state.drB = CS80Voice__rnd (_state);
                        }
                        drift1 = 1.0f + ((_state.drA * 0.0014f) + (_state.vDrift1 * 0.001f));
                        drift2 = 1.0f + ((_state.drB * 0.0014f) + (_state.vDrift2 * 0.001f));
                        lfoHz = 0.05f + ((_state.params.lfoRate * _state.params.lfoRate) * 12.0f);
                        ...
                        e1a = 1.0f / (intrinsics::max (0.0008f, 0.004f + ((_state.params.c1A * _state.params.c1A) * 6.0f)) * _state.sr);
                        e1d = 1.0f / (intrinsics::max (0.0008f, 0.008f + ((_state.params.c1D * _state.params.c1D) * 9.0f)) * _state.sr);
                        e1r = 1.0f / (intrinsics::max (0.0008f, 0.008f + ((_state.params.c1R * _state.params.c1R) * 9.0f)) * _state.sr);
                        if (_state.e1Stage == int32_t {0})
                        {
                            _state.e1 = (_state.e1 + e1a);
                            if (_state.e1 >= 1.0f)
                            {
                                _state.e1 = 1.0f;
                                _state.e1Stage = int32_t {1};
                        ...
                        else
                        {
                            ...
                            {
                                _state.e1 = (_state.e1 - e1d);
                                if (_state.e1 <= _state.params.c1S)
                                {
                                    _state.e1 = _state.params.c1S;
                        ...
                        e2a = 1.0f / (intrinsics::max (0.0008f, 0.004f + ((_state.params.c2A * _state.params.c2A) * 6.0f)) * _state.sr);
                        e2d = 1.0f / (intrinsics::max (0.0008f, 0.008f + ((_state.params.c2D * _state.params.c2D) * 9.0f)) * _state.sr);
                        e2r = 1.0f / (intrinsics::max (0.0008f, 0.008f + ((_state.params.c2R * _state.params.c2R) * 9.0f)) * _state.sr);
                        if (_state.e2Stage == int32_t {0})
                        {
                            _state.e2 = (_state.e2 + e2a);
                            if (_state.e2 >= 1.0f)
                            {
                                _state.e2 = 1.0f;
                                _state.e2Stage = int32_t {1};
                        ...
                        else
                        {
                            ...
                            {
                                _state.e2 = (_state.e2 - e2d);
                                if (_state.e2 <= _state.params.c2S)
                                {
                                    _state.e2 = _state.params.c2S;
                        ...
                        tune = intrinsics::pow (2.0f, (_state.params.tune - 0.5f) * 0.16f);
                        det = _state.params.detune * 0.012f;
                        touch = (_state.params.touchBrill * _state.vel) * 3500.0f;
                        pwm = lfo * 0.25f;
                        f1 = ((_state.freq * drift1) * tune) * (1.0f - det);
                        inc1 = intrinsics::clamp (f1, 0.0f, _state.sr * 0.49f) * _state.invSr;
                        _state.ph1 = (_state.ph1 + inc1);
                        if (_state.ph1 >= 1.0f)
                        {
                            _state.ph1 = (_state.ph1 - 1.0f);
                        }
                        s1 = CS80Voice__osc (_state.ph1, inc1, _state.params.c1Wave, intrinsics::clamp (_state.params.c1PW + pwm, 0.05f, 0.95f));
                        f2 = ((_state.freq * drift2) * tune) * (1.0f + det);
                        inc2 = intrinsics::clamp (f2, 0.0f, _state.sr * 0.49f) * _state.invSr;
                        _state.ph2 = (_state.ph2 + inc2);
                        if (_state.ph2 >= 1.0f)
                        {
                            _state.ph2 = (_state.ph2 - 1.0f);
                        }
                        s2 = CS80Voice__osc (_state.ph2, inc2, _state.params.c2Wave, intrinsics::clamp (_state.params.c2PW + pwm, 0.05f, 0.95f));
                        rm = (s1 * s2) * _state.params.ring;
                        _state.subPh = (_state.subPh + (inc1 * 0.5f));
                        if (_state.subPh >= 1.0f)
                        {
                            _state.subPh = (_state.subPh - 1.0f);
                        }
                        subOsc = (((_state.subPh < 0.5f) ? 1.0f : -1.0f) * _state.params.sub) * 0.5f;
                        brill1 = (((_state.params.c1Brill - 0.5f) * 5000.0f) + touch) + ((_state.params.c1FEnv * _state.e1) * 6000.0f);
                        hp1c = intrinsics::clamp (20.0f * intrinsics::exp (_state.params.c1Hpf * 6.4f), 20.0f, _state.sr * 0.45f);
                        lp1c = intrinsics::clamp ((40.0f * intrinsics::exp (_state.params.c1Lpf * 6.0f)) + brill1, 30.0f, _state.sr * 0.45f);
                        gH1 = intrinsics::tan ((3.1415927f * hp1c) * _state.invSr);
                        kH1 = intrinsics::clamp (1.6f - (1.3f * _state.params.c1HpfR), 0.25f, 1.8f);
                        aH1 = 1.0f / (1.0f + (gH1 * (gH1 + kH1)));
                        bH1 = gH1 * aH1;
                        cH1 = gH1 * bH1;
                        xv1 = (s1 + subOsc) + rm;
                        vh13 = xv1 - _state.c1h2;
                        vh11 = (aH1 * _state.c1h1) + (bH1 * vh13);
                        vh12 = (_state.c1h2 + (bH1 * _state.c1h1)) + (cH1 * vh13);
                        _state.c1h1 = ((2.0f * vh11) - _state.c1h1);
                        _state.c1h2 = ((2.0f * vh12) - _state.c1h2);
                        hp1 = (xv1 - (kH1 * vh11)) - vh12;
                        gL1 = intrinsics::tan ((3.1415927f * lp1c) * _state.invSr);
                        kL1 = intrinsics::clamp (1.6f - (1.3f * _state.params.c1LpfR), 0.22f, 1.8f);
                        aL1 = 1.0f / (1.0f + (gL1 * (gL1 + kL1)));
                        bL1 = gL1 * aL1;
                        cL1 = gL1 * bL1;
                        vl13 = hp1 - _state.c1l2;
                        vl11 = (aL1 * _state.c1l1) + (bL1 * vl13);
                        vl12 = (_state.c1l2 + (bL1 * _state.c1l1)) + (cL1 * vl13);
                        _state.c1l1 = ((2.0f * vl11) - _state.c1l1);
                        _state.c1l2 = ((2.0f * vl12) - _state.c1l2);
                        out1 = (vl12 * _state.e1) * _state.params.c1Level;
                        brill2 = (((_state.params.c2Brill - 0.5f) * 5000.0f) + touch) + ((_state.params.c2FEnv * _state.e2) * 6000.0f);
                        hp2c = intrinsics::clamp (20.0f * intrinsics::exp (_state.params.c2Hpf * 6.4f), 20.0f, _state.sr * 0.45f);
                        lp2c = intrinsics::clamp ((40.0f * intrinsics::exp (_state.params.c2Lpf * 6.0f)) + brill2, 30.0f, _state.sr * 0.45f);
                        gH2 = intrinsics::tan ((3.1415927f * hp2c) * _state.invSr);
                        kH2 = intrinsics::clamp (1.6f - (1.3f * _state.params.c2HpfR), 0.25f, 1.8f);
                        aH2 = 1.0f / (1.0f + (gH2 * (gH2 + kH2)));
                        bH2 = gH2 * aH2;
                        cH2 = gH2 * bH2;
                        xv2 = (s2 + subOsc) + rm;
                        wh23 = xv2 - _state.c2h2;
                        wh21 = (aH2 * _state.c2h1) + (bH2 * wh23);
                        wh22 = (_state.c2h2 + (bH2 * _state.c2h1)) + (cH2 * wh23);
                        _state.c2h1 = ((2.0f * wh21) - _state.c2h1);
                        _state.c2h2 = ((2.0f * wh22) - _state.c2h2);
                        hp2 = (xv2 - (kH2 * wh21)) - wh22;
                        gL2 = intrinsics::tan ((3.1415927f * lp2c) * _state.invSr);
                        kL2 = intrinsics::clamp (1.6f - (1.3f * _state.params.c2LpfR), 0.22f, 1.8f);
                        aL2 = 1.0f / (1.0f + (gL2 * (gL2 + kL2)));
                        bL2 = gL2 * aL2;
                        cL2 = gL2 * bL2;
                        wl23 = hp2 - _state.c2l2;
                        wl21 = (aL2 * _state.c2l1) + (bL2 * wl23);
                        wl22 = (_state.c2l2 + (bL2 * _state.c2l1)) + (cL2 * wl23);
                        _state.c2l1 = ((2.0f * wl21) - _state.c2l1);
                        _state.c2l2 = ((2.0f * wl22) - _state.c2l2);
                        out2 = (wl22 * _state.e2) * _state.params.c2Level;
                        gI = intrinsics::sqrt (intrinsics::clamp (1.0f - _state.params.mix, 0.0f, 1.0f)) + 0.4f;
                        gII = intrinsics::sqrt (intrinsics::clamp (_state.params.mix, 0.0f, 1.0f)) + 0.4f;
                        sig = ((out1 * gI) + (out2 * gII)) * 0.5f;
                        yb = (sig - _state.dcx) + (0.9985f * _state.dcy);
                        _state.dcx = sig;
                        _state.dcy = yb;
                        sig = yb;
                        amp = (sig * (0.45f + (0.55f * _state.vel))) * (0.4f + (_state.params.volume * 1.1f));
                        pan = _state.vPan * 0.25f;
                        outL = intrinsics::tanh ((amp * intrinsics::sqrt (intrinsics::clamp (0.5f - (pan * 0.5f), 0.0f, 1.0f))) * 1.7f);
                        outR = intrinsics::tanh ((amp * intrinsics::sqrt (intrinsics::clamp (0.5f + (pan * 0.5f), 0.0f, 1.0f))) * 1.7f);
        ...
        float CS80Voice__osc (float ph, float inc, float wave, float pw) noexcept
        {
            float  saw;
            float  p2;
            float  saw2;
            ...
            pulse = saw - saw2;
            return (saw * (1.0f - wave)) + (pulse * wave);
        }
        ...
        static _CS80_State& state_upcast_struct_CS80_State_struct_VcMpkb (_ParamsProcessor_1_State& child) { return *reinterpret_cast<_CS80_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_CS80_State, paramsProcessor))); }
        static _CS80_State& state_upcast_struct_CS80_State_struct_zJVDIb (std_voices_std_voices_VoiceAllocator_specialised_Th5A4_1_State& child) { return *reinterpret_cast<_CS80_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_CS80_State, voiceAllocator))); }
        static CS80_State& state_upcast_struct_CS80_State_struct__YgHJR (_CS80_State& child) { return *reinterpret_cast<CS80_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (CS80_State, _state))); }
        static _CS80_State& state_upcast_struct_CS80_State_struct_gAAgvc (std_midi_MPEConverter_1_State& child) { return *reinterpret_cast<_CS80_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_CS80_State, noteTap))); }
        static _CS80_State& state_upcast_struct_CS80_State_struct_GQTfab (std_midi_MPEConverter_1_State& child) { return *reinterpret_cast<_CS80_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_CS80_State, mpe))); }
...
struct CS80
{
    using PerformerClass = performer::CS80;
    static constexpr auto filename = "CS80.cmajorpatch";
    ...
    static constexpr const char* cmaj_api_cmajpatchconnection_js =
        ...
        "    }\n"
        "}\n";
    static constexpr const char* CS80_cmajorpatch =
        "{\n"
        "  \"CmajorVersion\": 1,\n"
        "  \"ID\": \"com.plugincorp.jwcs80\",\n"
        "  \"version\": \"0.1\",\n"
        "  \"name\": \"JW-CS80\",\n"
        "  \"description\": \"TOI JW-CS80 - expressive dual-layer 8-voice poly by plugin corp\",\n"
        "  \"category\": \"instrument\",\n"
        "  \"manufacturer\": \"TOI\",\n"
        "  \"isInstrument\": true,\n"
    ...
    static constexpr std::array files =
    {
        ...
        File { "CS80.cmajorpatch", std::string_view (CS80_cmajorpatch, 380) },
...
// native faceplate (replaces the generic Cmajor UI). See CS80Native.h.
#include "CS80Native.h"

juce::AudioProcessor* JUCE_CALLTYPE createPluginFilter()
{
    return new CS80Plugin (std::make_shared<cmaj::Patch>());
}
