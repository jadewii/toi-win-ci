...
struct Palette
{
    ...
    juce::Colour noteStrip = juce::Colour (0x00000000);   // note-name band fill (transparent = use panel bg)
};
...
{
public:
    enum Style { Pointer = 0, RingDot, Lollipop, Target, Ticks, Arc, Metal, Fluted, FlutedDark, Pointed, Spike, LineKnob, NumStyles };
    ...
    void setShowLabel (bool s) { showLabel = s; }
    // Optional per-knob collar tint (used by the LineKnob style for its base ring). Transparent = use pal.accent.
    void setCollar (juce::Colour c) { collar = c; repaint(); }
    // LineKnob (JW-P5) options: brushed-silver cap instead of black, and a bipolar 5..0..5 numeral ring.
    void setSilver  (bool s) { silver  = s; repaint(); }
    void setBipolar (bool b) { bipolar = b; repaint(); }
    // Overlay a printed numeral ring (0..10) + tick line AROUND the knob, on ANY style (the
    // vintage Memorymoog / numbered-dial look). The knob body shrinks to leave room. Default off.
    void setNumerals (bool n) { numerals_ = n; repaint(); }
    ...
    void paint (juce::Graphics& g) override
    {
        auto fullDisc = discArea();
        auto disc = numerals_ ? juce::Rectangle<float> (0.0f, 0.0f, fullDisc.getWidth() * 0.70f, fullDisc.getHeight() * 0.70f).withCentre (fullDisc.getCentre())
                              : fullDisc;
        const float cx = disc.getCentreX(), cy = disc.getCentreY();
        ...
        switch (style)
        {
            ...
            case LineKnob:
            {
                // Vintage analog-poly dial (JW-P5): a BLACK (or brushed-SILVER) slightly-domed cap with a
                // crisp WHITE POINTER LINE from centre to rim, surrounded by a printed NUMERAL RING -
                // white tick marks at evenly spaced positions over the ~300-deg travel with numbers
                // 0..10 (or a bipolar 5..0..5 for MASTER TUNE). No coloured collar, no value arc.
                const float disR = disc.getWidth() * 0.5f;
                const float R    = disR * 0.58f;                 // cap is small; numerals live outside it
                const float ringR = disR - 1.0f;                 // tick/numeral radius
                const float a0 = -150.0f, a1 = 150.0f;           // ~300 deg of travel
                const auto  white = juce::Colours::white;
                // pointer direction over THIS knob's 300-deg sweep (not the shared 270-deg ang)
                const float pAng = juce::degreesToRadians (a0 + (a1 - a0) * v);
                const float pdx = std::sin (pAng), pdy = -std::cos (pAng);

                // ---- printed numeral ring (ticks + numbers) ----
                const int N = 11;                                // 0..10 -> 11 marks
                g.setFont (uiFont (juce::jmax (5.0f, disR * 0.20f), true));
                for (int i = 0; i < N; ++i)
                {
                    const float t  = (float) i / (float) (N - 1);
                    const float aa = juce::degreesToRadians (a0 + (a1 - a0) * t);
                    const float sx = std::sin (aa), sy = -std::cos (aa);
                    // tick mark just outside the cap
                    g.setColour (white.withAlpha (0.85f));
                    g.drawLine (cx + sx * (R + 2.0f), cy + sy * (R + 2.0f),
                                cx + sx * (R + 5.0f), cy + sy * (R + 5.0f), 1.2f);
                    // numeral
                    juce::String num = bipolar ? juce::String (std::abs (i - 5)) : juce::String (i);
                    const float nx = cx + sx * ringR, ny = cy + sy * ringR;
                    g.setColour (white.withAlpha (0.92f));
                    g.drawText (num, juce::Rectangle<float> (nx - 7.0f, ny - 6.0f, 14.0f, 12.0f), juce::Justification::centred);
                }

                // drop shadow under the cap
                g.setColour (juce::Colours::black.withAlpha (0.45f));
                g.fillEllipse (cx - R, cy - R + 2.5f, R * 2.0f, R * 2.0f);

                if (silver)
                {
                    // brushed-silver metal cap: bright upper-left -> darker lower-right
                    juce::ColourGradient body (juce::Colour (0xffeef0f2), cx - R * 0.45f, cy - R * 0.55f,
                                               juce::Colour (0xff7c7f84), cx + R * 0.45f, cy + R * 0.75f, false);
                    body.addColour (0.5, juce::Colour (0xffc2c5ca));
                    g.setGradientFill (body); g.fillEllipse (cx - R, cy - R, R * 2.0f, R * 2.0f);
                    g.setColour (juce::Colour (0xff55585c)); g.drawEllipse (cx - R, cy - R, R * 2.0f, R * 2.0f, 1.2f);
                    // domed top face
                    const float topR = R * 0.82f;
                    juce::ColourGradient top (juce::Colour (0xfff6f7f8), cx - topR * 0.4f, cy - topR * 0.5f,
                                              juce::Colour (0xff9a9da2), cx + topR * 0.4f, cy + topR * 0.7f, true);
                    g.setGradientFill (top); g.fillEllipse (cx - topR, cy - topR, topR * 2.0f, topR * 2.0f);
                    // dark pointer line (reads on silver)
                    g.setColour (juce::Colour (0xff1a1a1c));
                    g.drawLine (cx + pdx * topR * 0.10f, cy + pdy * topR * 0.10f, cx + pdx * (R - 1.0f), cy + pdy * (R - 1.0f), STROKE);
                }
                else
                {
                    // conical BLACK cap: outer ring -> raised top face, lit from upper-left
                    juce::ColourGradient side (juce::Colour (0xff3a3a3e), cx - R * 0.5f, cy - R * 0.6f,
                                               juce::Colour (0xff050506), cx + R * 0.55f, cy + R * 0.7f, false);
                    side.addColour (0.5, juce::Colour (0xff19191c));
                    g.setGradientFill (side); g.fillEllipse (cx - R, cy - R, R * 2.0f, R * 2.0f);
                    g.setColour (juce::Colours::black); g.drawEllipse (cx - R, cy - R, R * 2.0f, R * 2.0f, 1.0f);
                    const float topR = R * 0.74f;
                    juce::ColourGradient top (juce::Colour (0xff2c2c30), cx - topR * 0.4f, cy - topR * 0.5f,
                                              juce::Colour (0xff0a0a0c), cx + topR * 0.4f, cy + topR * 0.6f, true);
                    g.setGradientFill (top); g.fillEllipse (cx - topR, cy - topR, topR * 2.0f, topR * 2.0f);
                    // upper-left rim sheen
                    { juce::Path sh; sh.addCentredArc (cx, cy, R - 1.5f, R - 1.5f, 0.0f,
                        juce::degreesToRadians (-120.0f), juce::degreesToRadians (-20.0f), true);
                      g.setColour (white.withAlpha (0.16f)); g.strokePath (sh, juce::PathStrokeType (1.6f)); }
                    // crisp WHITE pointer line from near the centre to the cap rim
                    g.setColour (juce::Colour (0xfff4f4f6));
                    g.drawLine (cx + pdx * topR * 0.10f, cy + pdy * topR * 0.10f, cx + pdx * (R - 1.0f), cy + pdy * (R - 1.0f), STROKE);
                }
                break;
            }
        ...
        // printed numeral ring (0..10) + tick line drawn AROUND the (shrunk) knob body
        if (numerals_)
        {
            const float ncx = fullDisc.getCentreX(), ncy = fullDisc.getCentreY();
            const float nr  = fullDisc.getWidth() * 0.5f - 1.0f;
            const float na0 = -150.0f, na1 = 150.0f;
            g.setFont (uiFont (juce::jmax (5.0f, nr * 0.20f), true));
            for (int i = 0; i < 11; ++i)
            {
                const float t = (float) i / 10.0f;
                const float aa = juce::degreesToRadians (na0 + (na1 - na0) * t);
                const float sx = std::sin (aa), sy = -std::cos (aa);
                g.setColour (pal.text.withAlpha (0.55f));
                g.drawLine (ncx + sx * (nr - 3.0f), ncy + sy * (nr - 3.0f), ncx + sx * (nr - 0.5f), ncy + sy * (nr - 0.5f), 1.0f);
                g.setColour (pal.text.withAlpha (0.9f));
                const float nx = ncx + sx * (nr - 7.0f), ny = ncy + sy * (nr - 7.0f);
                g.drawText (juce::String (i), juce::Rectangle<float> (nx - 6.0f, ny - 6.0f, 12.0f, 12.0f), juce::Justification::centred);
            }
        }
...
private:
    ...
    juce::Colour collar { 0x00000000 };
    bool silver = false, bipolar = false, numerals_ = false;
    std::unique_ptr<ParamKnobAttachment> attachment;
...
public:
    ...
    void setFill (juce::Colour c) { fillCol = c; repaint(); }   // optional per-button base colour override
    void flash()
    {
    ...
    void paintButton (juce::Graphics& g, bool over, bool down) override
    {
        ...
        const bool custom = ! fillCol.isTransparent();
        auto base    = custom ? fillCol : pal.button;
        auto flashCol= custom ? fillCol.brighter (0.4f) : pal.accent;
        auto fill = flashing ? flashCol : down ? base.darker (0.18f) : over ? base.brighter (0.07f) : base;
...
private:
    bool flashing = false; Palette pal; juce::Colour fillCol { 0x00000000 };
...
//==============================================================================
// MoldedButton - a clean molded-plastic square (vintage synth tone/preset button): a solid
// colour box with a soft raised bevel and NOTHING inside. Lights PINK when active / pressed.
// Shared by JW-CS80 + JW-PPG (and future synths). Base colour set per-button; pink is constant.
class MoldedButton  : public juce::Button
{
public:
    explicit MoldedButton (juce::Colour baseColour) : juce::Button ("molded"), base (baseColour) {}
    void setActive (bool a) { if (active != a) { active = a; repaint(); } }
    void setLit  (juce::Colour c) { litCol = c; }   // colour when active/pressed
    void setPink (juce::Colour c) { litCol = c; }   // (alias - PPG lights pink)
    void setCircle (bool c) { circle = c; }         // PPG: a raised 3D circular dome inside the square
    void paintButton (juce::Graphics& g, bool over, bool down) override
    {
        auto b = getLocalBounds().toFloat().reduced (1.5f);
        const bool lit = active || down;
        const juce::Colour col = lit ? litCol : base;
        if (lit) { g.setColour (litCol.withAlpha (0.40f)); g.fillRoundedRectangle (b.expanded (2.0f), 4.0f); }
        // square bezel/frame + plate
        g.setColour (col.darker (0.55f)); g.fillRoundedRectangle (b, 3.0f);
        auto inner = b.reduced (2.2f);
        if (circle)
        {
            // flat square plate, then a RAISED 3D circular dome (radial gradient lit upper-left)
            g.setColour (col.darker (lit ? 0.20f : 0.30f)); g.fillRoundedRectangle (inner, 2.5f);
            const float d = std::min (inner.getWidth(), inner.getHeight()) - 2.0f;
            auto cc = juce::Rectangle<float> (0, 0, d, d).withCentre (inner.getCentre());
            g.setColour (juce::Colours::black.withAlpha (0.35f)); g.fillEllipse (cc.translated (0.0f, 1.2f));   // contact shadow
            juce::ColourGradient rg (col.brighter (lit ? 0.45f : 0.32f), cc.getCentreX() - d * 0.24f, cc.getCentreY() - d * 0.24f,
                                     col.darker  (lit ? 0.05f : 0.20f), cc.getCentreX() + d * 0.22f, cc.getCentreY() + d * 0.30f, true);
            g.setGradientFill (rg); g.fillEllipse (cc);
            g.setColour (juce::Colours::white.withAlpha (lit ? 0.40f : 0.30f));   // upper-left sheen
            g.drawEllipse (cc.reduced (d * 0.16f).translated (-d * 0.06f, -d * 0.06f), 1.2f);
            g.setColour (juce::Colours::black.withAlpha (0.45f)); g.drawEllipse (cc, 1.0f);
        }
        else
        {
            juce::ColourGradient cg (col.brighter (lit ? 0.28f : 0.16f), inner.getCentreX(), inner.getY(),
                                     col.darker (lit ? 0.06f : 0.16f),  inner.getCentreX(), inner.getBottom(), false);
            g.setGradientFill (cg); g.fillRoundedRectangle (inner, 2.2f);
            g.setColour (juce::Colours::white.withAlpha (lit ? 0.42f : 0.26f));
            g.fillRoundedRectangle (inner.withHeight (inner.getHeight() * 0.42f).reduced (1.0f, 0.0f), 2.0f);
        }
        if (over && ! lit) { g.setColour (juce::Colours::white.withAlpha (0.10f)); g.fillRoundedRectangle (inner, 2.2f); }
        g.setColour (juce::Colours::black.withAlpha (0.55f)); g.drawRoundedRectangle (b, 3.0f, 1.0f);
    }
private:
    bool circle = false;
    juce::Colour base, litCol { 0xffff4f9d };
    bool active = false;
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR (MoldedButton)
};
...
public:
    ...
    void resized() override
    {
        ...
        if (! moduleMode_)
            placeInArtboard (keyboard, juce::Rectangle<float> (innerL + cheekInset, noteStripBottom, (innerR - innerL) - 2.0f * cheekInset, ART_H - noteStripBottom - border));
...
protected:
    ...
    void finishSetup()
    {
        ...
        bakeBtn_ = std::make_unique<juce::TextButton> ("BAKE");   // TEMP: appends a paste-ready preset line to ~/toi-bakes/<model>.txt (remove once banks are baked)
        bakeBtn_->onClick = [this] { bakeCurrent(); };
        addAndMakeVisible (*bakeBtn_);
        bakeBtn_->setBounds (4, 4, 58, 20);
        setSize ((int) ART_W, (int) ART_H);
        maybeArmSnapshot();
    }

    // TEMP preset-baking: write the current control values as a paste-ready FactoryPreset line.
    void bakeCurrent()
    {
        auto dir = juce::File::getSpecialLocation (juce::File::userHomeDirectory).getChildFile ("toi-bakes");
        dir.createDirectory();
        auto f = dir.getChildFile (cfg.model + ".txt");
        juce::String line = "        { \"NAME\", { ";
        bool first = true;
        for (auto& s : cfg.params)
        {
            float v = 0.0f;
            if (auto* p = findParam (s.id)) v = s.min + p->getValue() * (s.max - s.min);
            line << (first ? "" : ", ") << juce::String (v, 3) << "f"; first = false;
        }
        line << " } },";
        f.appendText (line + juce::newLine);
        if (bakeBtn_ != nullptr)
        {
            bakeBtn_->setButtonText ("SAVED");
            juce::Timer::callAfterDelay (800, [s = juce::Component::SafePointer<PanelEditor> (this)] { if (s != nullptr && s->bakeBtn_ != nullptr) s->bakeBtn_->setButtonText ("BAKE"); });
        }
    }
    ...
    float cheekInset = 0.0f;   // wood side-cheek width: insets the keyboard + note strip (0 = none; full-width as before)
...
private:
    ...
    {
        const float kbL = innerL + cheekInset, kbW = (innerR - innerL) - 2.0f * cheekInset;
        if (! cfg.pal.noteStrip.isTransparent())   // per-synth coloured note-name band (e.g. JW-JUNO blue)
        {
            g.setColour (cfg.pal.noteStrip);
            g.fillRect (juce::Rectangle<float> (kbL, panelBottom, kbW, noteStripBottom - panelBottom));
        }
        g.setColour (cfg.pal.ink);
        // with wood end-cheeks, keep the divider lines inside the keyboard span so the cheeks read
        // as one continuous wood panel (full-width otherwise, for cheek-less synths).
        const float lineL = cheekInset > 0.0f ? kbL : 0.0f;
        const float lineR = cheekInset > 0.0f ? kbL + kbW : ART_W;
        g.drawLine (lineL, panelBottom, lineR, panelBottom, STROKE);
        g.drawLine (lineL, noteStripBottom, lineR, noteStripBottom, STROKE);
        g.setColour (cfg.pal.text);
        g.setFont (uiFont (10.0f, true));
        const float whiteW = kbW / 15.0f;
        for (int i = 0; i < 15; ++i)
            g.drawText (kWhiteNames[i], juce::Rectangle<float> (kbL + (float) i * whiteW, panelBottom, whiteW, noteStripBottom - panelBottom), juce::Justification::centred);
    }
    ...
    void doSnapshot()
    {
        ...
        if (dir.isNotEmpty())
        {
            ...
            if (bakeBtn_ != nullptr) bakeBtn_->setVisible (false);   // keep the TEMP bake button out of product images
    ...
    std::unique_ptr<juce::TextButton> bakeBtn_;   // TEMP preset-bake (see bakeCurrent)
