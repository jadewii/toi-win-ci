...
// Auto-generated Cmajor code for patch 'JWM1'
...
//==============================================================================
/// Auto-generated C++ class for the 'JWM1' processor
///
...
namespace performer
{
    struct JWM1
    {
        ...
        static constexpr std::string_view name = "JWM1";
        ...
        //==============================================================================
        static constexpr uint32_t numInputEndpoints  = 35;
        ...
        static constexpr uint32_t getEndpointHandleForName (std::string_view endpointName)
        {
            ...
            if (endpointName == "multi1")      return static_cast<uint32_t> (EndpointHandles::multi1);
            if (endpointName == "multi2")      return static_cast<uint32_t> (EndpointHandles::multi2);
            if (endpointName == "dbl")         return static_cast<uint32_t> (EndpointHandles::dbl);
            if (endpointName == "detune")      return static_cast<uint32_t> (EndpointHandles::detune);
            if (endpointName == "osc2Oct")     return static_cast<uint32_t> (EndpointHandles::osc2Oct);
            ...
            if (endpointName == "vdfEnv")      return static_cast<uint32_t> (EndpointHandles::vdfEnv);
            if (endpointName == "vdfKbd")      return static_cast<uint32_t> (EndpointHandles::vdfKbd);
            ...
            if (endpointName == "fA")          return static_cast<uint32_t> (EndpointHandles::fA);
            if (endpointName == "fD")          return static_cast<uint32_t> (EndpointHandles::fD);
            if (endpointName == "fS")          return static_cast<uint32_t> (EndpointHandles::fS);
            ...
            if (endpointName == "pEnvDecay")   return static_cast<uint32_t> (EndpointHandles::pEnvDecay);
            ...
            if (endpointName == "lfo2Filter")  return static_cast<uint32_t> (EndpointHandles::lfo2Filter);
            ...
            if (endpointName == "fxRouting")   return static_cast<uint32_t> (EndpointHandles::fxRouting);
            ...
            if (endpointName == "delayFb")     return static_cast<uint32_t> (EndpointHandles::delayFb);
        ...
        static constexpr EndpointInfo inputEndpoints[] =
        {
            ...
            { 2,   "multi1",      EndpointType::event },
            { 3,   "multi2",      EndpointType::event },
            { 4,   "dbl",         EndpointType::event },
            { 5,   "detune",      EndpointType::event },
            { 6,   "osc2Oct",     EndpointType::event },
            { 7,   "oscMix",      EndpointType::event },
            { 8,   "cutoff",      EndpointType::event },
            { 9,   "reso",        EndpointType::event },
            { 10,  "vdfEnv",      EndpointType::event },
            { 11,  "vdfKbd",      EndpointType::event },
            { 12,  "aA",          EndpointType::event },
            { 13,  "aD",          EndpointType::event },
            { 14,  "aS",          EndpointType::event },
            { 15,  "aR",          EndpointType::event },
            { 16,  "fA",          EndpointType::event },
            { 17,  "fD",          EndpointType::event },
            { 18,  "fS",          EndpointType::event },
            { 19,  "fR",          EndpointType::event },
            { 20,  "pEnvAmt",     EndpointType::event },
            { 21,  "pEnvDecay",   EndpointType::event },
            { 22,  "glide",       EndpointType::event },
            { 23,  "lfo1Rate",    EndpointType::event },
            { 24,  "lfo1Pitch",   EndpointType::event },
            { 25,  "lfo2Rate",    EndpointType::event },
            { 26,  "lfo2Filter",  EndpointType::event },
            { 27,  "grain",       EndpointType::event },
            { 28,  "volume",      EndpointType::event },
            { 29,  "tune",        EndpointType::event },
            { 30,  "fxRouting",   EndpointType::event },
            { 31,  "chorus",      EndpointType::event },
            { 32,  "delay",       EndpointType::event },
            { 33,  "delayTime",   EndpointType::event },
            { 34,  "delayFb",     EndpointType::event },
            { 35,  "reverb",      EndpointType::event }
        };

        static constexpr EndpointInfo outputEndpoints[] =
        {
            { 36,  "audioOut",  EndpointType::stream },
            { 37,  "noteOut",   EndpointType::event  }
        };
        ...
        struct _M1Voice_1_State
        {
            plugincorp_Params params;
            Array<float, 16384> ms;
            bool built = {};
            bool active = {};
            float freqTarget = {};
            float glideCur = {};
        ...
        struct _M1Delay_1_State
        {
            Array<float, 96000> bufL;
            Array<float, 96000> bufR;
            int32_t w = {};
            float dTime = {};
            float dFb = {};
            float dAmt = {};
        ...
        struct _JWM1_State
        {
            int32_t _sessionID = {};
            double _frequency = {};
            Array<_M1Voice_1_State, 16> voices;
        ...
        struct JWM1_eventValue_noteOut
        {
            int32_t frame = {};
            int32_t type = {};
        ...
        struct JWM1_State
        {
            ...
            Array<JWM1_eventValue_noteOut, 32> noteOut;
        };
        ...
        {
            if (endpointHandle == 36) { std::memcpy (reinterpret_cast<char*> (dest), std::addressof (cmajIO.audioOut), 8 * numFramesToCopy); std::memset (reinterpret_cast<char*> (std::addressof (cmajIO.audioOut)), 0, 8 * numFramesToCopy); return; }
        ...
        {
            if (endpointHandle == 37) return (uint32_t) cmajState.noteOut_eventCount;
        ...
        {
            if (endpointHandle == 37) { cmajState.noteOut_eventCount = 0; return; }
        }

        uint32_t getOutputEventType (EndpointHandle endpointHandle, uint32_t index)
        {
            if (endpointHandle == 37) return (uint32_t) cmajState.noteOut[(IndexType) index].type;
            throw std::runtime_error ("Unknown event endpointHandle:" + std::to_string (endpointHandle));
            return {};
        }
        ...
        static uint32_t getOutputEventDataSize (EndpointHandle endpointHandle, uint32_t typeIndex)
        {
            ...
            if (endpointHandle == 37 && typeIndex == 0) return 8;
            throw std::runtime_error ("Unknown event endpointHandle:" + std::to_string (endpointHandle));
            return 0;
        }
        ...
        void addEvent_multi1 (float event)
        {
            _sendEvent_multi1 (cmajState, event);
        }

        void addEvent_multi2 (float event)
        {
            _sendEvent_multi2 (cmajState, event);
        }

        void addEvent_dbl (float event)
        {
            _sendEvent_dbl (cmajState, event);
        }
        ...
        void addEvent_osc2Oct (float event)
        {
            _sendEvent_osc2Oct (cmajState, event);
        }
        ...
        void addEvent_vdfEnv (float event)
        {
            _sendEvent_vdfEnv (cmajState, event);
        }

        void addEvent_vdfKbd (float event)
        {
            _sendEvent_vdfKbd (cmajState, event);
        }
        ...
        void addEvent_fA (float event)
        {
            _sendEvent_fA (cmajState, event);
        }

        void addEvent_fD (float event)
        {
            _sendEvent_fD (cmajState, event);
        }

        void addEvent_fS (float event)
        {
            _sendEvent_fS (cmajState, event);
        }
        ...
        void addEvent_pEnvDecay (float event)
        {
            _sendEvent_pEnvDecay (cmajState, event);
        }
        ...
        void addEvent_lfo2Filter (float event)
        {
            _sendEvent_lfo2Filter (cmajState, event);
        }
        ...
        void addEvent_fxRouting (float event)
        {
            _sendEvent_fxRouting (cmajState, event);
        }
        ...
        {
            _sendEvent_delayTime (cmajState, event);
        }

        void addEvent_delayFb (float event)
        {
            _sendEvent_delayFb (cmajState, event);
        }
        ...
        void addEvent (EndpointHandle endpointHandle, uint32_t typeIndex, const unsigned char* eventData)
        {
            ...
            if (endpointHandle == 21)
            {
                ...
                memcpy (&value, eventData, 4);
                eventData += 4;
                return addEvent_pEnvDecay (value);
            }
            ...
            if (endpointHandle == 26)
            {
                ...
                memcpy (&value, eventData, 4);
                eventData += 4;
                return addEvent_lfo2Filter (value);
            }
            ...
            if (endpointHandle == 30)
            {
                ...
                memcpy (&value, eventData, 4);
                eventData += 4;
                return addEvent_fxRouting (value);
            }
        ...
        JWM1_State cmajState = {};
        JWM1_IO cmajIO = {};

        //==============================================================================
        void _sendEvent_midiIn (JWM1_State& _state, std_midi_Message value) noexcept
        {
            _JWM1__midiIn (_state._state, value);
        }

        void _JWM1__midiIn (_JWM1_State& _state, std_midi_Message value) noexcept
        {
        ...
        {
            _JWM1___forwardEvent (state_upcast_struct_JWM1_State_struct_txQTzc(_state), value);
            _JWM1___forwardEvent_6 (state_upcast_struct_JWM1_State_struct_txQTzc(_state), value);
        }

        void _JWM1___forwardEvent (_JWM1_State& _state, std_notes_NoteOn value) noexcept
        {
        ...
        {
            _JWM1___forwardEvent_3 (state_upcast_struct_JWM1_State_struct_6FaRbe(_state), value);
        }

        void _JWM1___forwardEvent_3 (_JWM1_State& _state, plugincorp_NoteEvent value) noexcept
        {
            _JWM1___writeEvent_noteOut (_state, value);
        }

        void _JWM1___writeEvent_noteOut (_JWM1_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
            JWM1___writeEvent_noteOut (state_upcast_struct_JWM1_State_struct__aa8MLc(_state), value);
        }

        void JWM1___writeEvent_noteOut (JWM1_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
            JWM1___writeEvent_noteOut_0 (_state, value);
        }

        void JWM1___writeEvent_noteOut_0 (JWM1_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
        ...
        void _JWM1___forwardEvent_6 (_JWM1_State& _state, std_notes_NoteOn value) noexcept
        {
        ...
        {
            _JWM1___forwardEvent_13 (state_upcast_struct_JWM1_State_struct_AvMHY(_state), index, value);
        }

        void _JWM1___forwardEvent_13 (_JWM1_State& _state, int32_t index, std_notes_NoteOff value) noexcept
        {
            M1Voice__eventsIn (_state.voices[index], value);
        }

        void M1Voice__eventsIn (_M1Voice_1_State& _state, std_notes_NoteOff e) noexcept
        {
        ...
        {
            _JWM1___forwardEvent_12 (state_upcast_struct_JWM1_State_struct_AvMHY(_state), index, value);
        }

        void _JWM1___forwardEvent_12 (_JWM1_State& _state, int32_t index, std_notes_NoteOn value) noexcept
        {
            M1Voice__eventsIn_0 (_state.voices[index], value);
        }

        void M1Voice__eventsIn_0 (_M1Voice_1_State& _state, std_notes_NoteOn e) noexcept
        {
        ...
        {
            _JWM1___forwardEvent_2 (state_upcast_struct_JWM1_State_struct_txQTzc(_state), value);
            _JWM1___forwardEvent_7 (state_upcast_struct_JWM1_State_struct_txQTzc(_state), value);
        }

        void _JWM1___forwardEvent_2 (_JWM1_State& _state, std_notes_NoteOff value) noexcept
        {
        ...
        void _JWM1___forwardEvent_7 (_JWM1_State& _state, std_notes_NoteOff value) noexcept
        {
        ...
        {
            _JWM1___forwardEvent_8 (state_upcast_struct_JWM1_State_struct_txQTzc(_state), value);
        }

        void _JWM1___forwardEvent_8 (_JWM1_State& _state, std_notes_PitchBend value) noexcept
        {
        ...
        {
            _JWM1___forwardEvent_10 (state_upcast_struct_JWM1_State_struct_txQTzc(_state), value);
        }

        void _JWM1___forwardEvent_10 (_JWM1_State& _state, std_notes_Pressure value) noexcept
        {
        ...
        {
            _JWM1___forwardEvent_9 (state_upcast_struct_JWM1_State_struct_txQTzc(_state), value);
        }

        void _JWM1___forwardEvent_9 (_JWM1_State& _state, std_notes_Slide value) noexcept
        {
        ...
        {
            _JWM1___forwardEvent_11 (state_upcast_struct_JWM1_State_struct_txQTzc(_state), value);
        }

        void _JWM1___forwardEvent_11 (_JWM1_State& _state, std_notes_Control value) noexcept
        {
        ...
        void _sendEvent_multi1 (JWM1_State& _state, float value) noexcept
        {
            _JWM1__multi1 (_state._state, value);
        }

        void _JWM1__multi1 (_JWM1_State& _state, float value) noexcept
        {
            ParamsProcessor__multi1 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__multi1 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.multi1 = v;
        ...
        {
            _JWM1___forwardEvent_4 (state_upcast_struct_JWM1_State_struct_VID64d(_state), value);
            _JWM1___forwardEvent_5 (state_upcast_struct_JWM1_State_struct_VID64d(_state), value);
        }

        void _JWM1___forwardEvent_4 (_JWM1_State& _state, plugincorp_Params value) noexcept
        {
            int32_t  i;

            {
                ...
                for (;;)
                {
                    ...
                    M1Voice__paramsIn (_state.voices[i], value);
                    {
                        ++i;
                    }
                }
        ...
        void M1Voice__paramsIn (_M1Voice_1_State& _state, plugincorp_Params p) noexcept
        {
            _state.params = p;
        }

        void _JWM1___forwardEvent_5 (_JWM1_State& _state, plugincorp_Params value) noexcept
        {
        ...
        {
            M1Delay__paramsIn (_state.delay, value);
            FXMixer__paramsIn (_state.mix, value);
        }

        void M1Delay__paramsIn (_M1Delay_1_State& _state, plugincorp_Params p) noexcept
        {
            _state.dTime = p.delayTime;
            _state.dFb = p.delayFb;
            _state.dAmt = p.delay;
        }
        ...
        void FXMixer__paramsIn (_FXMixer_1_State& _state, plugincorp_Params p) noexcept
        {
            ...
            _state.routing = p.fxRouting;
        }

        void _sendEvent_multi2 (JWM1_State& _state, float value) noexcept
        {
            _JWM1__multi2 (_state._state, value);
        }

        void _JWM1__multi2 (_JWM1_State& _state, float value) noexcept
        {
            ParamsProcessor__multi2 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__multi2 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.multi2 = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_dbl (JWM1_State& _state, float value) noexcept
        {
            _JWM1__dbl (_state._state, value);
        }

        void _JWM1__dbl (_JWM1_State& _state, float value) noexcept
        {
            ParamsProcessor__dbl (_state.paramsProcessor, value);
        }

        void ParamsProcessor__dbl (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.dbl = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_detune (JWM1_State& _state, float value) noexcept
        {
            _JWM1__detune (_state._state, value);
        }

        void _JWM1__detune (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_osc2Oct (JWM1_State& _state, float value) noexcept
        {
            _JWM1__osc2Oct (_state._state, value);
        }

        void _JWM1__osc2Oct (_JWM1_State& _state, float value) noexcept
        {
            ParamsProcessor__osc2Oct (_state.paramsProcessor, value);
        }

        void ParamsProcessor__osc2Oct (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.osc2Oct = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_oscMix (JWM1_State& _state, float value) noexcept
        {
            _JWM1__oscMix (_state._state, value);
        }

        void _JWM1__oscMix (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_cutoff (JWM1_State& _state, float value) noexcept
        {
            _JWM1__cutoff (_state._state, value);
        }

        void _JWM1__cutoff (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_reso (JWM1_State& _state, float value) noexcept
        {
            _JWM1__reso (_state._state, value);
        }

        void _JWM1__reso (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_vdfEnv (JWM1_State& _state, float value) noexcept
        {
            _JWM1__vdfEnv (_state._state, value);
        }

        void _JWM1__vdfEnv (_JWM1_State& _state, float value) noexcept
        {
            ParamsProcessor__vdfEnv (_state.paramsProcessor, value);
        }

        void ParamsProcessor__vdfEnv (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.vdfEnv = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_vdfKbd (JWM1_State& _state, float value) noexcept
        {
            _JWM1__vdfKbd (_state._state, value);
        }

        void _JWM1__vdfKbd (_JWM1_State& _state, float value) noexcept
        {
            ParamsProcessor__vdfKbd (_state.paramsProcessor, value);
        }

        void ParamsProcessor__vdfKbd (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.vdfKbd = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_aA (JWM1_State& _state, float value) noexcept
        {
            _JWM1__aA (_state._state, value);
        }

        void _JWM1__aA (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_aD (JWM1_State& _state, float value) noexcept
        {
            _JWM1__aD (_state._state, value);
        }

        void _JWM1__aD (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_aS (JWM1_State& _state, float value) noexcept
        {
            _JWM1__aS (_state._state, value);
        }

        void _JWM1__aS (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_aR (JWM1_State& _state, float value) noexcept
        {
            _JWM1__aR (_state._state, value);
        }

        void _JWM1__aR (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_fA (JWM1_State& _state, float value) noexcept
        {
            _JWM1__fA (_state._state, value);
        }

        void _JWM1__fA (_JWM1_State& _state, float value) noexcept
        {
            ParamsProcessor__fA (_state.paramsProcessor, value);
        }

        void ParamsProcessor__fA (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.fA = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_fD (JWM1_State& _state, float value) noexcept
        {
            _JWM1__fD (_state._state, value);
        }

        void _JWM1__fD (_JWM1_State& _state, float value) noexcept
        {
            ParamsProcessor__fD (_state.paramsProcessor, value);
        }

        void ParamsProcessor__fD (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.fD = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_fS (JWM1_State& _state, float value) noexcept
        {
            _JWM1__fS (_state._state, value);
        }

        void _JWM1__fS (_JWM1_State& _state, float value) noexcept
        {
            ParamsProcessor__fS (_state.paramsProcessor, value);
        }

        void ParamsProcessor__fS (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.fS = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_fR (JWM1_State& _state, float value) noexcept
        {
            _JWM1__fR (_state._state, value);
        }

        void _JWM1__fR (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_pEnvAmt (JWM1_State& _state, float value) noexcept
        {
            _JWM1__pEnvAmt (_state._state, value);
        }

        void _JWM1__pEnvAmt (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_pEnvDecay (JWM1_State& _state, float value) noexcept
        {
            _JWM1__pEnvDecay (_state._state, value);
        }

        void _JWM1__pEnvDecay (_JWM1_State& _state, float value) noexcept
        {
            ParamsProcessor__pEnvDecay (_state.paramsProcessor, value);
        }

        void ParamsProcessor__pEnvDecay (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.pEnvDecay = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_glide (JWM1_State& _state, float value) noexcept
        {
            _JWM1__glide (_state._state, value);
        }

        void _JWM1__glide (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_lfo1Rate (JWM1_State& _state, float value) noexcept
        {
            _JWM1__lfo1Rate (_state._state, value);
        }

        void _JWM1__lfo1Rate (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_lfo1Pitch (JWM1_State& _state, float value) noexcept
        {
            _JWM1__lfo1Pitch (_state._state, value);
        }

        void _JWM1__lfo1Pitch (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_lfo2Rate (JWM1_State& _state, float value) noexcept
        {
            _JWM1__lfo2Rate (_state._state, value);
        }

        void _JWM1__lfo2Rate (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_lfo2Filter (JWM1_State& _state, float value) noexcept
        {
            _JWM1__lfo2Filter (_state._state, value);
        }

        void _JWM1__lfo2Filter (_JWM1_State& _state, float value) noexcept
        {
            ParamsProcessor__lfo2Filter (_state.paramsProcessor, value);
        }

        void ParamsProcessor__lfo2Filter (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.lfo2Filter = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_grain (JWM1_State& _state, float value) noexcept
        {
            _JWM1__grain (_state._state, value);
        }

        void _JWM1__grain (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_volume (JWM1_State& _state, float value) noexcept
        {
            _JWM1__volume (_state._state, value);
        }

        void _JWM1__volume (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_tune (JWM1_State& _state, float value) noexcept
        {
            _JWM1__tune (_state._state, value);
        }

        void _JWM1__tune (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_fxRouting (JWM1_State& _state, float value) noexcept
        {
            _JWM1__fxRouting (_state._state, value);
        }

        void _JWM1__fxRouting (_JWM1_State& _state, float value) noexcept
        {
            ParamsProcessor__fxRouting (_state.paramsProcessor, value);
        }

        void ParamsProcessor__fxRouting (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.fxRouting = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_chorus (JWM1_State& _state, float value) noexcept
        {
            _JWM1__chorus (_state._state, value);
        }

        void _JWM1__chorus (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_delay (JWM1_State& _state, float value) noexcept
        {
            _JWM1__delay (_state._state, value);
        }

        void _JWM1__delay (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_delayTime (JWM1_State& _state, float value) noexcept
        {
            _JWM1__delayTime (_state._state, value);
        }

        void _JWM1__delayTime (_JWM1_State& _state, float value) noexcept
        {
            ParamsProcessor__delayTime (_state.paramsProcessor, value);
        }

        void ParamsProcessor__delayTime (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.delayTime = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_delayFb (JWM1_State& _state, float value) noexcept
        {
            _JWM1__delayFb (_state._state, value);
        }

        void _JWM1__delayFb (_JWM1_State& _state, float value) noexcept
        {
            ParamsProcessor__delayFb (_state.paramsProcessor, value);
        }

        void ParamsProcessor__delayFb (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.delayFb = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_reverb (JWM1_State& _state, float value) noexcept
        {
            _JWM1__reverb (_state._state, value);
        }

        void _JWM1__reverb (_JWM1_State& _state, float value) noexcept
        {
        ...
        void _initialise (JWM1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            _JWM1___initialise (_state._state, processorID, sessionID, frequency);
        }

        void _JWM1___initialise (_JWM1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            int32_t  i;
            ...
            g__frequency = frequency;
            {
                ...
                for (;;)
                {
                    ...
                    M1Voice___initialise (_state.voices[i], processorID, sessionID, frequency);
                    {
                        ++i;
                    }
                }
        ...
        void M1Voice___initialise (_M1Voice_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
        ...
        void ParamsProcessor___initialise (_ParamsProcessor_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
            _state.params = plugincorp_Params { 0.0f, 0.57f, 0.0f, 0.52f, 0.5f, 0.5f, 0.72f, 0.0f, 0.4f, 0.4f, 0.02f, 0.5f, 0.7f, 0.3f, 0.04f, 0.45f, 0.4f, 0.35f, 0.5f, 0.2f, 0.0f, 0.35f, 0.0f, 0.28f, 0.0f, 0.3f, 0.8f, 0.5f, 0.0f, 0.5f, 0.3f, 0.38f, 0.35f, 0.45f };
        }
        ...
        void _initialise_2 (_FXBus_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            ...
            M1Delay___initialise (_state.delay, processorID, sessionID, frequency);
        ...
        void M1Delay___initialise (_M1Delay_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
            _state.dTime = 0.38f;
        ...
        void FXMixer___initialise (_FXMixer_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            ...
            _state.delayAmt = 0.3f;
            _state.reverbAmt = 0.45f;
            _state.routing = 0.0f;
        }

        void _advance (JWM1_State& _state, JWM1_IO& _io, int32_t _frames) noexcept
        {
            _JWM1_IO  ioCopy;
        ...
        void main (_JWM1_State& _state, _JWM1_IO& _io) noexcept
        {
            Array<_M1Voice_1_IO, 16>  voices_io;
            ...
            voices_io = Array<_M1Voice_1_IO, 16> {};
            ...
            }
            {
                ...
                for (;;)
                {
                    ...
                    M1Voice__main (_state.voices[i], voices_io[i]);
                    {
                        ++i;
                    }
                }
        ...
        void M1Voice__main (_M1Voice_1_State& _state, _M1Voice_1_IO& _io) noexcept
        {
            float  outL;
            float  outR;
            float  masterTune;
            ...
            if (! _state.built)
            {
                M1Voice__buildMultisounds (_state);
                _state.built = true;
            }
            ...
            _branch0_0:
            {
                for (;;)
                {
                    ...
                    if (_state.active)
                    {
                        ...
                        _state.lfo1Ph = (_state.lfo1Ph + ((0.05f + ((_state.params.lfo1Rate * _state.params.lfo1Rate) * 12.0f)) * _state.invSr));
                        if (_state.lfo1Ph >= 1.0f)
                        {
                            _state.lfo1Ph = (_state.lfo1Ph - 1.0f);
                        }
                        _state.lfo2Ph = (_state.lfo2Ph + ((0.05f + ((_state.params.lfo2Rate * _state.params.lfo2Rate) * 10.0f)) * _state.invSr));
                        if (_state.lfo2Ph >= 1.0f)
                        {
                            _state.lfo2Ph = (_state.lfo2Ph - 1.0f);
                        }
                        lfo1 = intrinsics::sin (_state.lfo1Ph * 6.2831855f);
                        lfo2 = M1Voice__lfoTri (_state.lfo2Ph);
                        _state.ae = M1Voice__advEnv (_state.aStage, _state.ae, _state.params.aA, _state.params.aD, _state.params.aS, _state.params.aR, _state.sr);
                        ...
                        _state.fe = M1Voice__advEnv (_state.fStage, _state.fe, _state.params.fA, _state.params.fD, _state.params.fS, _state.params.fR, _state.sr);
                        if ((_state.fStage == int32_t {0}) ? (_state.fe >= 1.0f) : false)
                        {
                            _state.fe = 1.0f;
                            _state.fStage = int32_t {1};
                        }
                        if ((_state.fStage == int32_t {1}) ? (_state.fe <= _state.params.fS) : false)
                        {
                            _state.fe = _state.params.fS;
                            _state.fStage = int32_t {2};
                        }
                        if ((_state.fStage == int32_t {3}) ? (_state.fe <= 0.0f) : false)
                        {
                            _state.fe = 0.0f;
                            _state.fStage = int32_t {-1};
                        }
                        pScoop = ((_state.params.pEnvAmt - 0.5f) * 4.0f) * intrinsics::exp ((- _state.noteT) / intrinsics::max (0.01f, _state.params.pEnvDecay * 0.5f));
                        pitchMul = intrinsics::pow (2.0f, ((lfo1 * _state.params.lfo1Pitch) * 0.5f) + pScoop);
                        baseF = (_state.glideCur * masterTune) * pitchMul;
                        fam1 = static_cast<int32_t> (intrinsics::floor ((_state.params.multi1 * 7.0f) + 0.5f));
                        ...
                        sig = M1Voice__readMulti (_state, fam1, _state.ph1) * M1Voice__famDecay (fam1, _state.noteT);
                        doubleOn = _state.params.dbl >= 0.5f;
                        if (doubleOn)
                        {
                            fam2 = static_cast<int32_t> (intrinsics::floor ((_state.params.multi2 * 7.0f) + 0.5f));
                            oct2 = intrinsics::floor ((_state.params.osc2Oct * 4.0f) + 0.5f) - 2.0f;
                            det2 = intrinsics::pow (2.0f, (_state.params.detune - 0.5f) * 0.1f);
                            f2 = intrinsics::clamp ((baseF * intrinsics::pow (2.0f, oct2)) * det2, 0.0f, _state.ny);
                            ...
                            o2 = M1Voice__readMulti (_state, fam2, _state.ph2) * M1Voice__famDecay (fam2, _state.noteT);
                            mx = intrinsics::clamp (_state.params.oscMix, 0.0f, 1.0f);
                            sig = ((sig * (1.0f - mx)) + (o2 * mx));
                        }
                        {
                            atkEnv = intrinsics::exp ((- _state.noteT) / intrinsics::max (0.003f, M1Voice__famAtkTime (fam1)));
                            if (atkEnv > 0.0008f)
                            {
                                chiff = M1Voice__readMulti (_state, fam1, _state.ph1 * 2.0f);
                                nz = M1Voice__rnd (_state);
                                mixN = M1Voice__famAtkNoise (fam1);
                                burst = ((((mixN * nz) + ((1.0f - mixN) * chiff)) * atkEnv) * M1Voice__famAtkAmt (fam1)) * (0.5f + (0.5f * _state.vel));
                                sig = (sig + burst);
                            }
                        }
                        ...
                        _state.dcy = yb;
                        sig = yb;
                        kbd = _state.params.vdfKbd * M1Voice__log2f (_state.glideCur / 261.63f);
                        fcOct = (((_state.params.cutoff * 9.0f) + ((_state.params.vdfEnv * _state.fe) * 5.0f)) + kbd) + ((lfo2 * _state.params.lfo2Filter) * 3.0f);
                        fcHz = intrinsics::clamp (40.0f * intrinsics::pow (2.0f, fcOct), 25.0f, _state.ny);
                        g = intrinsics::tan ((3.1415927f * fcHz) * _state.invSr);
                        gc = g / (1.0f + g);
                        k = (_state.params.reso * _state.params.reso) * 3.6f;
                        resoIn = sig - (k * _state.bp);
                        i1v = resoIn - _state.lp1;
                        _state.lp1 = (_state.lp1 + (gc * i1v));
                        i2v = _state.lp1 - _state.lp2;
                        _state.lp2 = (_state.lp2 + (gc * i2v));
                        i3v = _state.lp2 - _state.lp3;
                        _state.lp3 = (_state.lp3 + (gc * i3v));
                        i4v = _state.lp3 - _state.lp4;
                        _state.lp4 = (_state.lp4 + (gc * i4v));
                        _state.bp = _state.lp4;
                        vdf = _state.lp4 * (1.0f + (k * 0.18f));
                        post = vdf;
                        if (_state.params.grain > 0.001f)
                        {
                            steps = 65536.0f - (_state.params.grain * 65400.0f);
                            crushed = intrinsics::floor ((post * steps) + 0.5f) / steps;
                            srDiv = static_cast<int32_t> (1.0f + (_state.params.grain * 4.0f));
                            if (intrinsics::modulo (_state.rngc, srDiv) == int32_t {0})
                            {
                                _state.held = crushed;
                            }
                            ++_state.rngc;
                            post = ((post * (1.0f - _state.params.grain)) + (_state.held * _state.params.grain));
                        }
                        amp = ((post * _state.ae) * (0.4f + (0.6f * _state.vel))) * (0.4f + (_state.params.volume * 1.1f));
        ...
        void M1Voice__buildMultisounds (_M1Voice_1_State& _state) noexcept
        {
            int32_t  fy;
            int32_t  f;
            float  maxv;
                ...
                for (;;)
                {
                    ...
                    maxv = 0.0001f;
                    {
                        ...
                        for (;;)
                        {
                            ...
                            n = nn;
                            ph = static_cast<float> (n) / 2048.0f;
                            th = 6.2831855f * ph;
                            s = 0.0f;
                            if (f == int32_t {0})
                            {
                                {
                                    ...
                                    for (;;)
                                    {
                                        ...
                                        stretch = 1.0f + ((0.0009f * k) * k);
                                        s = (s + ((1.0f / k) * intrinsics::sin ((th * k) * stretch)));
                                        {
                                            ++kk;
                                        }
                                    }
                            ...
                            else
                            {
                                if (f == int32_t {1})
                                {
                                    s = (((intrinsics::sin (th) + (0.5f * intrinsics::sin (th * 2.0f))) + (0.35f * intrinsics::sin (th * 14.0f))) + (0.18f * intrinsics::sin (th * 5.0f)));
                                }
                                else
                                {
                                    if (f == int32_t {2})
                                    {
                                        s = ((((((0.9f * intrinsics::sin (th * 0.5f)) + (1.0f * intrinsics::sin (th * 1.0f))) + (0.7f * intrinsics::sin (th * 1.5f))) + (0.8f * intrinsics::sin (th * 2.0f))) + (0.5f * intrinsics::sin (th * 3.0f))) + (0.6f * intrinsics::sin (th * 4.0f)));
                                    }
                                    else
                                    {
                                        if (f == int32_t {3})
                                        {
                                            {
                                                ...
                                                for (;;)
                                                {
                                                    ...
                                                    g1 = intrinsics::exp ((-0.5f * ((k_0 - 2.7f) / 1.1f)) * ((k_0 - 2.7f) / 1.1f));
                                                    g2 = intrinsics::exp ((-0.5f * ((k_0 - 4.2f) / 1.6f)) * ((k_0 - 4.2f) / 1.6f)) * 0.8f;
                                                    g3 = intrinsics::exp ((-0.5f * ((k_0 - 10.0f) / 3.2f)) * ((k_0 - 10.0f) / 3.2f)) * 0.4f;
                                                    s = (s + ((((g1 + g2) + g3) * (1.0f / intrinsics::sqrt (k_0))) * intrinsics::sin (th * k_0)));
                                                    {
                                                        ++kk_0;
                                                    }
                                                }
                                        ...
                                        else
                                        {
                                            if (f == int32_t {4})
                                            {
                                                ...
                                                r3 = 8.93f;
                                                r4 = 13.34f;
                                                s = ((((intrinsics::sin (th * r0) + (0.7f * intrinsics::sin (th * r1))) + (0.5f * intrinsics::sin (th * r2))) + (0.35f * intrinsics::sin (th * r3))) + (0.22f * intrinsics::sin (th * r4)));
                                            }
                                            else
                                            {
                                                if (f == int32_t {5})
                                                {
                                                    {
                                                        ...
                                                        for (;;)
                                                        {
                                                            ...
                                                            k_1 = static_cast<float> (kk_1 + int32_t {1});
                                                            s = (s + ((1.0f / k_1) * intrinsics::sin (th * k_1)));
                                                            {
                                                                ++kk_1;
                                                            }
                                                        }
                                                ...
                                                else
                                                {
                                                    if (f == int32_t {6})
                                                    {
                                                        s = (((intrinsics::sin (th) + (0.6f * intrinsics::sin (th * 2.0f))) + (0.3f * intrinsics::sin (th * 3.0f))) + (0.12f * intrinsics::sin (th * 7.0f)));
                                                    }
                                                    else
                                                    {
                                                        {
                                                            ...
                                                            for (;;)
                                                            {
                                                                ...
                                                                k_2 = static_cast<float> (kk_2 + int32_t {4});
                                                                s = (s + ((1.0f / k_2) * intrinsics::sin ((th * k_2) * (1.0f + (0.0015f * k_2)))));
                                                                {
                                                                    ++kk_2;
                                                                }
                                                            }
                            ...
                            _state.ms[std__intrinsics___wrap_16384 ((f * g_ML) + n)] = s;
                            a = intrinsics::abs (s);
                            if (a > maxv)
                            {
                                maxv = a;
                    ...
                    inv = 1.0f / maxv;
                    {
                        ...
                        for (;;)
                        {
                            if (nn_0 >= int32_t {2048})
                            {
                                break;
                            }
                            {
                                t = std__intrinsics___wrap_16384 ((f * g_ML) + nn_0);
                                _state.ms[t] = (_state.ms[t] * inv);
                            }
                            {
                                ++nn_0;
                            }
        ...
        int32_t std__intrinsics___wrap_16384 (int32_t n) noexcept
        {
            return n & int32_t {16383};
        }
        ...
        float M1Voice__lfoTri (float ph) noexcept
        {
            return (ph < 0.5f) ? ((4.0f * ph) - 1.0f) : (3.0f - (4.0f * ph));
        }

        float M1Voice__advEnv (int32_t stage, float lvl, float a, float d, float s, float r, float sr) noexcept
        {
            float  ar;
            float  dr;
            float  rr;
        ...
        float M1Voice__readMulti (_M1Voice_1_State& _state, int32_t fam, float p) noexcept
        {
            float  fp;
            int32_t  i0;
            int32_t  i1;
            ...
            fp = (p - intrinsics::floor (p)) * 2048.0f;
            i0 = intrinsics::modulo (static_cast<int32_t> (fp), g_ML);
            i1 = intrinsics::modulo (i0 + int32_t {1}, g_ML);
            fr = fp - intrinsics::floor (fp);
            a = _state.ms[std__intrinsics___wrap_16384 ((fam * g_ML) + i0)];
            b = _state.ms[std__intrinsics___wrap_16384 ((fam * g_ML) + i1)];
            return (a * (1.0f - fr)) + (b * fr);
        }

        float M1Voice__famDecay (int32_t fam, float t) noexcept
        {
            if (fam == int32_t {0})
            {
                return (intrinsics::exp ((- t) * 1.6f) * 0.85f) + 0.15f;
            }
            if (fam == int32_t {1})
            {
                return (intrinsics::exp ((- t) * 1.1f) * 0.8f) + 0.2f;
            }
            if (fam == int32_t {4})
            {
                return (intrinsics::exp ((- t) * 0.9f) * 0.7f) + 0.3f;
            }
            if (fam == int32_t {6})
            {
                return (intrinsics::exp ((- t) * 2.2f) * 0.7f) + 0.3f;
            }
            return 1.0f;
        }

        float M1Voice__famAtkTime (int32_t fam) noexcept
        {
            if (fam == int32_t {0})
            {
                return 0.01f;
        ...
        float M1Voice__rnd (_M1Voice_1_State& _state) noexcept
        {
            float  s;
        ...
        float M1Voice__famAtkNoise (int32_t fam) noexcept
        {
            if (fam == int32_t {0})
            {
                return 0.55f;
        ...
        float M1Voice__famAtkAmt (int32_t fam) noexcept
        {
            if (fam == int32_t {0})
            {
                return 0.55f;
        ...
        float M1Voice__log2f (float x) noexcept
        {
        ...
        void main_3 (std_midi_MPEConverter_1_State& _state, std_voices_std_voices_VoiceAllocator_specialised_N0jbA_1_IO& _io) noexcept
        {
        }
        ...
        void main_4 (_FXBus_1_State& _state, _FXBus_1_IO& _io) noexcept
        {
            ...
            M1Delay__main (_state.delay, delay_io);
            {
                reverb_io.audioIn = (reverb_io.audioIn + delay_io.audioOut);
            }
        ...
        void M1Delay__main (_M1Delay_1_State& _state, _FXBus_1_IO& _io) noexcept
        {
            float  dL;
            float  dR;
            float  fb;
            ...
            _branch0_0:
            {
                for (;;)
                {
                    dL = intrinsics::clamp ((0.04f + (_state.dTime * 0.75f)) * _state.sr, 1.0f, 95000.0f);
                    dR = intrinsics::clamp (dL * 1.5f, 1.0f, 95000.0f);
                    fb = intrinsics::clamp (_state.dFb * 0.92f, 0.0f, 0.92f);
                    wetL = M1Delay__rd (_state, _state.bufL, dL);
                    wetR = M1Delay__rd (_state, _state.bufR, dR);
                    _state.bufL[_state.w] = (_io.audioIn[int32_t {0}] + (wetR * fb));
                    _state.bufR[_state.w] = (_io.audioIn[int32_t {1}] + (wetL * fb));
                    std__intrinsics___wrapped_post_inc_96000 (_state.w);
                    _temp[int32_t {0}] = (_io.audioIn[int32_t {0}] + (wetL * _state.dAmt));
                    _temp[int32_t {1}] = (_io.audioIn[int32_t {1}] + (wetR * _state.dAmt));
                    _io.audioOut = (_io.audioOut + _temp);
                    _state._resumeIndex = int32_t {1};
                    return;
                }
        ...
        float M1Delay__rd (_M1Delay_1_State& _state, Array<float, 96000>& buf, float d) noexcept
        {
            float  rp;
            int32_t  i0;
            int32_t  i1;
            ...
            i0 = std__intrinsics___wrap_96000 (static_cast<int32_t> (rp));
            i1 = std__intrinsics___wrap_96000 (static_cast<int32_t> (rp) + int32_t {1});
            fr = rp - intrinsics::floor (rp);
            return (buf[i0] * (1.0f - fr)) + (buf[i1] * fr);
        }

        int32_t std__intrinsics___wrap_96000 (int32_t n) noexcept
        {
            int32_t  x;

            x = intrinsics::modulo (n, int32_t {0x17700});
            return (x < int32_t {0}) ? (x + int32_t {0x17700}) : x;
        }

        int32_t std__intrinsics___wrapped_post_inc_96000 (int32_t& n) noexcept
        {
            int32_t  result;

            result = n;
            n = std__intrinsics___wrap_96000 (n + int32_t {1});
            return result;
        }
        ...
        void FXMixer__main (_FXMixer_1_State& _state, _FXMixer_1_IO& _io) noexcept
        {
            ...
            float  _temp_8;
            Vector<float, 2>  _temp_9;
            float  _temp_10;
            Vector<float, 2>  mix;
            Vector<float, 2>  _temp_11;

            for (;;)
            {
                parallel = _state.routing >= 0.5f;
                body = Vector<float, 2>();
                if (parallel)
                {
                    _temp_0 = _state.chorusAmt;
                    _temp[int32_t {0}] = _temp_0;
                    _temp[int32_t {1}] = _temp_0;
                    _temp_2 = _state.delayAmt * 0.6f;
                    _temp_1[int32_t {0}] = _temp_2;
                    _temp_1[int32_t {1}] = _temp_2;
                    body = ((_io.dryIn + ((_io.chorusIn - _io.dryIn) * _temp)) + (_io.delayIn * _temp_1));
                }
                else
                {
                    _temp_4 = 1.0f - _state.chorusAmt;
                    _temp_3[int32_t {0}] = _temp_4;
                    _temp_3[int32_t {1}] = _temp_4;
                    _temp_6 = _state.chorusAmt;
                    _temp_5[int32_t {0}] = _temp_6;
                    _temp_5[int32_t {1}] = _temp_6;
                    body = ((_io.dryIn * _temp_3) + (_io.chorusIn * _temp_5));
                    _temp_8 = _state.delayAmt * 0.6f;
                    _temp_7[int32_t {0}] = _temp_8;
                    _temp_7[int32_t {1}] = _temp_8;
                    body = (body + (_io.delayIn * _temp_7));
                }
                _temp_10 = _state.reverbAmt * 1.6f;
                _temp_9[int32_t {0}] = _temp_10;
                _temp_9[int32_t {1}] = _temp_10;
                mix = body + (_io.reverbIn * _temp_9);
                _temp_11[int32_t {0}] = intrinsics::tanh (mix[int32_t {0}] * 0.85f);
                _temp_11[int32_t {1}] = intrinsics::tanh (mix[int32_t {1}] * 0.85f);
                _io.audioOut = (_io.audioOut + _temp_11);
                return;
            }
        }
        ...
        static _JWM1_State& state_upcast_struct_JWM1_State_struct_VID64d (_ParamsProcessor_1_State& child) { return *reinterpret_cast<_JWM1_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_JWM1_State, paramsProcessor))); }
        static _JWM1_State& state_upcast_struct_JWM1_State_struct_AvMHY (std_voices_std_voices_VoiceAllocator_specialised_N0jbA_1_State& child) { return *reinterpret_cast<_JWM1_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_JWM1_State, voiceAllocator))); }
        static _JWM1_State& state_upcast_struct_JWM1_State_struct_6FaRbe (std_midi_MPEConverter_1_State& child) { return *reinterpret_cast<_JWM1_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_JWM1_State, noteTap))); }
        static JWM1_State& state_upcast_struct_JWM1_State_struct__aa8MLc (_JWM1_State& child) { return *reinterpret_cast<JWM1_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (JWM1_State, _state))); }
        static _JWM1_State& state_upcast_struct_JWM1_State_struct_txQTzc (std_midi_MPEConverter_1_State& child) { return *reinterpret_cast<_JWM1_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_JWM1_State, mpe))); }
        ...
        double g__frequency {};
        static constexpr int32_t g_ML { int32_t {2048} };
...
struct JWM1
{
    using PerformerClass = performer::JWM1;
    static constexpr auto filename = "M1.cmajorpatch";
    ...
    static constexpr const char* cmaj_api_cmajpatchconnection_js =
        ...
        "    }\n"
        "}\n";
    static constexpr const char* M1_cmajorpatch =
        "{\n"
        "  \"CmajorVersion\": 1,\n"
        "  \"ID\": \"com.plugincorp.jwm1\",\n"
        "  \"version\": \"0.1\",\n"
        "  \"name\": \"JW-M1\",\n"
        "  \"description\": \"TOI JW-M1 - PCM workstation with synthesized multisounds, a non-resonant digital VDF and dual multi-FX by plugin corp\",\n"
        "  \"category\": \"instrument\",\n"
        "  \"manufacturer\": \"TOI\",\n"
        "  \"isInstrument\": true,\n"
    ...
    static constexpr std::array files =
    {
        ...
        File { "M1.cmajorpatch", std::string_view (M1_cmajorpatch, 428) },
...
// native faceplate (replaces the generic Cmajor UI). See M1Native.h.
#include "M1Native.h"

juce::AudioProcessor* JUCE_CALLTYPE createPluginFilter()
{
    return new JWM1Plugin (std::make_shared<cmaj::Patch>());
}
