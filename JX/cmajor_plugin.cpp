...
// Auto-generated Cmajor code for patch 'JWJX'
...
//==============================================================================
/// Auto-generated C++ class for the 'JWJX' processor
///
...
namespace performer
{
    struct JWJX
    {
        ...
        static constexpr std::string_view name = "JWJX";
        ...
        //==============================================================================
        static constexpr uint32_t numInputEndpoints  = 41;
        ...
        static constexpr uint32_t getEndpointHandleForName (std::string_view endpointName)
        {
            ...
            if (endpointName == "o1Tune")    return static_cast<uint32_t> (EndpointHandles::o1Tune);
            if (endpointName == "o1Lfo")     return static_cast<uint32_t> (EndpointHandles::o1Lfo);
            if (endpointName == "o1Env")     return static_cast<uint32_t> (EndpointHandles::o1Env);
            if (endpointName == "o1EnvSel")  return static_cast<uint32_t> (EndpointHandles::o1EnvSel);
            ...
            if (endpointName == "xDepth")    return static_cast<uint32_t> (EndpointHandles::xDepth);
            ...
            if (endpointName == "fLfo")      return static_cast<uint32_t> (EndpointHandles::fLfo);
            ...
            if (endpointName == "fKey")      return static_cast<uint32_t> (EndpointHandles::fKey);
            if (endpointName == "dynamics")  return static_cast<uint32_t> (EndpointHandles::dynamics);
            ...
            if (endpointName == "inv1")      return static_cast<uint32_t> (EndpointHandles::inv1);
            ...
            if (endpointName == "inv2")      return static_cast<uint32_t> (EndpointHandles::inv2);
            ...
            if (endpointName == "atVib")     return static_cast<uint32_t> (EndpointHandles::atVib);
            if (endpointName == "atBri")     return static_cast<uint32_t> (EndpointHandles::atBri);
        ...
        static constexpr EndpointInfo inputEndpoints[] =
        {
            ...
            { 2,   "o1Range",   EndpointType::event },
            { 3,   "o1Wave",    EndpointType::event },
            { 4,   "o1Tune",    EndpointType::event },
            { 5,   "o1Lfo",     EndpointType::event },
            { 6,   "o1Env",     EndpointType::event },
            { 7,   "o1EnvSel",  EndpointType::event },
            { 8,   "o2Range",   EndpointType::event },
            { 9,   "o2Wave",    EndpointType::event },
            { 10,  "o2Tune",    EndpointType::event },
            { 11,  "o2Fine",    EndpointType::event },
            { 12,  "xMod",      EndpointType::event },
            { 13,  "xDepth",    EndpointType::event },
            { 14,  "mix",       EndpointType::event },
            { 15,  "pwm",       EndpointType::event },
            { 16,  "hpf",       EndpointType::event },
            { 17,  "cutoff",    EndpointType::event },
            { 18,  "reso",      EndpointType::event },
            { 19,  "fLfo",      EndpointType::event },
            { 20,  "fEnv",      EndpointType::event },
            { 21,  "fEnvSel",   EndpointType::event },
            { 22,  "fKey",      EndpointType::event },
            { 23,  "dynamics",  EndpointType::event },
            { 24,  "a1",        EndpointType::event },
            { 25,  "d1",        EndpointType::event },
            { 26,  "s1",        EndpointType::event },
            { 27,  "r1",        EndpointType::event },
            { 28,  "inv1",      EndpointType::event },
            { 29,  "a2",        EndpointType::event },
            { 30,  "d2",        EndpointType::event },
            { 31,  "s2",        EndpointType::event },
            { 32,  "r2",        EndpointType::event },
            { 33,  "inv2",      EndpointType::event },
            { 34,  "lfoRate",   EndpointType::event },
            { 35,  "lfoShape",  EndpointType::event },
            { 36,  "lfoDelay",  EndpointType::event },
            { 37,  "atVib",     EndpointType::event },
            { 38,  "atBri",     EndpointType::event },
            { 39,  "chorus",    EndpointType::event },
        ...
        {
            { 42,  "audioOut",  EndpointType::stream },
            { 43,  "noteOut",   EndpointType::event  }
        };
        ...
        struct _JWJX_State
        {
            int32_t _sessionID = {};
            double _frequency = {};
            Array<_JXVoice_1_State, 6> voices;
        ...
        struct JWJX_eventValue_noteOut
        {
            int32_t frame = {};
            int32_t type = {};
        ...
        struct JWJX_State
        {
            ...
            Array<JWJX_eventValue_noteOut, 32> noteOut;
        };
        ...
        {
            if (endpointHandle == 42) { std::memcpy (reinterpret_cast<char*> (dest), std::addressof (cmajIO.audioOut), 8 * numFramesToCopy); std::memset (reinterpret_cast<char*> (std::addressof (cmajIO.audioOut)), 0, 8 * numFramesToCopy); return; }
        ...
        {
            if (endpointHandle == 43) return (uint32_t) cmajState.noteOut_eventCount;
        ...
        {
            if (endpointHandle == 43) { cmajState.noteOut_eventCount = 0; return; }
        }

        uint32_t getOutputEventType (EndpointHandle endpointHandle, uint32_t index)
        {
            if (endpointHandle == 43) return (uint32_t) cmajState.noteOut[(IndexType) index].type;
            throw std::runtime_error ("Unknown event endpointHandle:" + std::to_string (endpointHandle));
            return {};
        }
        ...
        static uint32_t getOutputEventDataSize (EndpointHandle endpointHandle, uint32_t typeIndex)
        {
            ...
            if (endpointHandle == 43 && typeIndex == 0) return 8;
            throw std::runtime_error ("Unknown event endpointHandle:" + std::to_string (endpointHandle));
            return 0;
        }
        ...
        void addEvent_o1Tune (float event)
        {
            _sendEvent_o1Tune (cmajState, event);
        }

        void addEvent_o1Lfo (float event)
        {
            _sendEvent_o1Lfo (cmajState, event);
        }

        void addEvent_o1Env (float event)
        {
            _sendEvent_o1Env (cmajState, event);
        }

        void addEvent_o1EnvSel (float event)
        {
            _sendEvent_o1EnvSel (cmajState, event);
        }
        ...
        void addEvent_xDepth (float event)
        {
            _sendEvent_xDepth (cmajState, event);
        }
        ...
        void addEvent_fLfo (float event)
        {
            _sendEvent_fLfo (cmajState, event);
        }
        ...
        void addEvent_fKey (float event)
        {
            _sendEvent_fKey (cmajState, event);
        }

        void addEvent_dynamics (float event)
        {
            _sendEvent_dynamics (cmajState, event);
        }
        ...
        void addEvent_inv1 (float event)
        {
            _sendEvent_inv1 (cmajState, event);
        }
        ...
        void addEvent_inv2 (float event)
        {
            _sendEvent_inv2 (cmajState, event);
        }
        ...
        void addEvent_atVib (float event)
        {
            _sendEvent_atVib (cmajState, event);
        }

        void addEvent_atBri (float event)
        {
            _sendEvent_atBri (cmajState, event);
        }
        ...
        void addEvent (EndpointHandle endpointHandle, uint32_t typeIndex, const unsigned char* eventData)
        {
            ...
            if (endpointHandle == 7)
            {
                ...
                memcpy (&value, eventData, 4);
                eventData += 4;
                return addEvent_o1EnvSel (value);
            }
            ...
            if (endpointHandle == 23)
            {
                ...
                memcpy (&value, eventData, 4);
                eventData += 4;
                return addEvent_dynamics (value);
            }
        ...
        JWJX_State cmajState = {};
        JWJX_IO cmajIO = {};

        //==============================================================================
        void _sendEvent_midiIn (JWJX_State& _state, std_midi_Message value) noexcept
        {
            _JWJX__midiIn (_state._state, value);
        }

        void _JWJX__midiIn (_JWJX_State& _state, std_midi_Message value) noexcept
        {
        ...
        {
            _JWJX___forwardEvent (state_upcast_struct_JWJX_State_struct_CxsOI(_state), value);
            _JWJX___forwardEvent_6 (state_upcast_struct_JWJX_State_struct_CxsOI(_state), value);
        }

        void _JWJX___forwardEvent (_JWJX_State& _state, std_notes_NoteOn value) noexcept
        {
        ...
        {
            _JWJX___forwardEvent_3 (state_upcast_struct_JWJX_State_struct_ZLmQTd(_state), value);
        }

        void _JWJX___forwardEvent_3 (_JWJX_State& _state, plugincorp_NoteEvent value) noexcept
        {
            _JWJX___writeEvent_noteOut (_state, value);
        }

        void _JWJX___writeEvent_noteOut (_JWJX_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
            JWJX___writeEvent_noteOut (state_upcast_struct_JWJX_State_struct__FmHWN(_state), value);
        }

        void JWJX___writeEvent_noteOut (JWJX_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
            JWJX___writeEvent_noteOut_0 (_state, value);
        }

        void JWJX___writeEvent_noteOut_0 (JWJX_State& _state, const plugincorp_NoteEvent& value) noexcept
        {
        ...
        void _JWJX___forwardEvent_6 (_JWJX_State& _state, std_notes_NoteOn value) noexcept
        {
        ...
        {
            _JWJX___forwardEvent_13 (state_upcast_struct_JWJX_State_struct_jECcjd(_state), index, value);
        }

        void _JWJX___forwardEvent_13 (_JWJX_State& _state, int32_t index, std_notes_NoteOff value) noexcept
        {
            JXVoice__eventsIn (_state.voices[index], value);
        }

        void JXVoice__eventsIn (_JXVoice_1_State& _state, std_notes_NoteOff e) noexcept
        {
        ...
        {
            _JWJX___forwardEvent_12 (state_upcast_struct_JWJX_State_struct_jECcjd(_state), index, value);
        }

        void _JWJX___forwardEvent_12 (_JWJX_State& _state, int32_t index, std_notes_NoteOn value) noexcept
        {
            JXVoice__eventsIn_0 (_state.voices[index], value);
        }

        void JXVoice__eventsIn_0 (_JXVoice_1_State& _state, std_notes_NoteOn e) noexcept
        {
        ...
        {
            _JWJX___forwardEvent_2 (state_upcast_struct_JWJX_State_struct_CxsOI(_state), value);
            _JWJX___forwardEvent_7 (state_upcast_struct_JWJX_State_struct_CxsOI(_state), value);
        }

        void _JWJX___forwardEvent_2 (_JWJX_State& _state, std_notes_NoteOff value) noexcept
        {
        ...
        void _JWJX___forwardEvent_7 (_JWJX_State& _state, std_notes_NoteOff value) noexcept
        {
        ...
        {
            _JWJX___forwardEvent_8 (state_upcast_struct_JWJX_State_struct_CxsOI(_state), value);
        }

        void _JWJX___forwardEvent_8 (_JWJX_State& _state, std_notes_PitchBend value) noexcept
        {
        ...
        {
            _JWJX___forwardEvent_10 (state_upcast_struct_JWJX_State_struct_CxsOI(_state), value);
        }

        void _JWJX___forwardEvent_10 (_JWJX_State& _state, std_notes_Pressure value) noexcept
        {
        ...
        {
            _JWJX___forwardEvent_9 (state_upcast_struct_JWJX_State_struct_CxsOI(_state), value);
        }

        void _JWJX___forwardEvent_9 (_JWJX_State& _state, std_notes_Slide value) noexcept
        {
        ...
        {
            _JWJX___forwardEvent_11 (state_upcast_struct_JWJX_State_struct_CxsOI(_state), value);
        }

        void _JWJX___forwardEvent_11 (_JWJX_State& _state, std_notes_Control value) noexcept
        {
        ...
        void _sendEvent_o1Range (JWJX_State& _state, float value) noexcept
        {
            _JWJX__o1Range (_state._state, value);
        }

        void _JWJX__o1Range (_JWJX_State& _state, float value) noexcept
        {
        ...
        {
            _JWJX___forwardEvent_4 (state_upcast_struct_JWJX_State_struct_JLl2fd(_state), value);
            _JWJX___forwardEvent_5 (state_upcast_struct_JWJX_State_struct_JLl2fd(_state), value);
        }

        void _JWJX___forwardEvent_4 (_JWJX_State& _state, plugincorp_Params value) noexcept
        {
            int32_t  i;

            {
                ...
                for (;;)
                {
                    ...
                    JXVoice__paramsIn (_state.voices[i], value);
                    {
                        ++i;
                    }
                }
        ...
        void JXVoice__paramsIn (_JXVoice_1_State& _state, plugincorp_Params p) noexcept
        {
            _state.params = p;
        }

        void _JWJX___forwardEvent_5 (_JWJX_State& _state, plugincorp_Params value) noexcept
        {
        ...
        {
            JXChorus__paramsIn (_state.chorus, value);
        }

        void JXChorus__paramsIn (_JXChorus_1_State& _state, plugincorp_Params p) noexcept
        {
            _state.mode = p.chorus;
        }

        void _sendEvent_o1Wave (JWJX_State& _state, float value) noexcept
        {
            _JWJX__o1Wave (_state._state, value);
        }

        void _JWJX__o1Wave (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_o1Tune (JWJX_State& _state, float value) noexcept
        {
            _JWJX__o1Tune (_state._state, value);
        }

        void _JWJX__o1Tune (_JWJX_State& _state, float value) noexcept
        {
            ParamsProcessor__o1Tune (_state.paramsProcessor, value);
        }

        void ParamsProcessor__o1Tune (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.o1Tune = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_o1Lfo (JWJX_State& _state, float value) noexcept
        {
            _JWJX__o1Lfo (_state._state, value);
        }

        void _JWJX__o1Lfo (_JWJX_State& _state, float value) noexcept
        {
            ParamsProcessor__o1Lfo (_state.paramsProcessor, value);
        }

        void ParamsProcessor__o1Lfo (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.o1Lfo = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_o1Env (JWJX_State& _state, float value) noexcept
        {
            _JWJX__o1Env (_state._state, value);
        }

        void _JWJX__o1Env (_JWJX_State& _state, float value) noexcept
        {
            ParamsProcessor__o1Env (_state.paramsProcessor, value);
        }

        void ParamsProcessor__o1Env (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.o1Env = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_o1EnvSel (JWJX_State& _state, float value) noexcept
        {
            _JWJX__o1EnvSel (_state._state, value);
        }

        void _JWJX__o1EnvSel (_JWJX_State& _state, float value) noexcept
        {
            ParamsProcessor__o1EnvSel (_state.paramsProcessor, value);
        }

        void ParamsProcessor__o1EnvSel (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.o1EnvSel = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_o2Range (JWJX_State& _state, float value) noexcept
        {
            _JWJX__o2Range (_state._state, value);
        }

        void _JWJX__o2Range (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_o2Wave (JWJX_State& _state, float value) noexcept
        {
            _JWJX__o2Wave (_state._state, value);
        }

        void _JWJX__o2Wave (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_o2Tune (JWJX_State& _state, float value) noexcept
        {
            _JWJX__o2Tune (_state._state, value);
        }

        void _JWJX__o2Tune (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_o2Fine (JWJX_State& _state, float value) noexcept
        {
            _JWJX__o2Fine (_state._state, value);
        }

        void _JWJX__o2Fine (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_xMod (JWJX_State& _state, float value) noexcept
        {
            _JWJX__xMod (_state._state, value);
        }

        void _JWJX__xMod (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_xDepth (JWJX_State& _state, float value) noexcept
        {
            _JWJX__xDepth (_state._state, value);
        }

        void _JWJX__xDepth (_JWJX_State& _state, float value) noexcept
        {
            ParamsProcessor__xDepth (_state.paramsProcessor, value);
        }

        void ParamsProcessor__xDepth (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.xDepth = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_mix (JWJX_State& _state, float value) noexcept
        {
            _JWJX__mix (_state._state, value);
        }

        void _JWJX__mix (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_pwm (JWJX_State& _state, float value) noexcept
        {
            _JWJX__pwm (_state._state, value);
        }

        void _JWJX__pwm (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_hpf (JWJX_State& _state, float value) noexcept
        {
            _JWJX__hpf (_state._state, value);
        }

        void _JWJX__hpf (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_cutoff (JWJX_State& _state, float value) noexcept
        {
            _JWJX__cutoff (_state._state, value);
        }

        void _JWJX__cutoff (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_reso (JWJX_State& _state, float value) noexcept
        {
            _JWJX__reso (_state._state, value);
        }

        void _JWJX__reso (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_fLfo (JWJX_State& _state, float value) noexcept
        {
            _JWJX__fLfo (_state._state, value);
        }

        void _JWJX__fLfo (_JWJX_State& _state, float value) noexcept
        {
            ParamsProcessor__fLfo (_state.paramsProcessor, value);
        }

        void ParamsProcessor__fLfo (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.fLfo = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_fEnv (JWJX_State& _state, float value) noexcept
        {
            _JWJX__fEnv (_state._state, value);
        }

        void _JWJX__fEnv (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_fEnvSel (JWJX_State& _state, float value) noexcept
        {
            _JWJX__fEnvSel (_state._state, value);
        }

        void _JWJX__fEnvSel (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_fKey (JWJX_State& _state, float value) noexcept
        {
            _JWJX__fKey (_state._state, value);
        }

        void _JWJX__fKey (_JWJX_State& _state, float value) noexcept
        {
            ParamsProcessor__fKey (_state.paramsProcessor, value);
        }

        void ParamsProcessor__fKey (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.fKey = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_dynamics (JWJX_State& _state, float value) noexcept
        {
            _JWJX__dynamics (_state._state, value);
        }

        void _JWJX__dynamics (_JWJX_State& _state, float value) noexcept
        {
            ParamsProcessor__dynamics (_state.paramsProcessor, value);
        }

        void ParamsProcessor__dynamics (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.dynamics = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_a1 (JWJX_State& _state, float value) noexcept
        {
            _JWJX__a1 (_state._state, value);
        }

        void _JWJX__a1 (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_d1 (JWJX_State& _state, float value) noexcept
        {
            _JWJX__d1 (_state._state, value);
        }

        void _JWJX__d1 (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_s1 (JWJX_State& _state, float value) noexcept
        {
            _JWJX__s1 (_state._state, value);
        }

        void _JWJX__s1 (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_r1 (JWJX_State& _state, float value) noexcept
        {
            _JWJX__r1 (_state._state, value);
        }

        void _JWJX__r1 (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_inv1 (JWJX_State& _state, float value) noexcept
        {
            _JWJX__inv1 (_state._state, value);
        }

        void _JWJX__inv1 (_JWJX_State& _state, float value) noexcept
        {
            ParamsProcessor__inv1 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__inv1 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.inv1 = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_a2 (JWJX_State& _state, float value) noexcept
        {
            _JWJX__a2 (_state._state, value);
        }

        void _JWJX__a2 (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_d2 (JWJX_State& _state, float value) noexcept
        {
            _JWJX__d2 (_state._state, value);
        }

        void _JWJX__d2 (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_s2 (JWJX_State& _state, float value) noexcept
        {
            _JWJX__s2 (_state._state, value);
        }

        void _JWJX__s2 (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_r2 (JWJX_State& _state, float value) noexcept
        {
            _JWJX__r2 (_state._state, value);
        }

        void _JWJX__r2 (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_inv2 (JWJX_State& _state, float value) noexcept
        {
            _JWJX__inv2 (_state._state, value);
        }

        void _JWJX__inv2 (_JWJX_State& _state, float value) noexcept
        {
            ParamsProcessor__inv2 (_state.paramsProcessor, value);
        }

        void ParamsProcessor__inv2 (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.inv2 = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_lfoRate (JWJX_State& _state, float value) noexcept
        {
            _JWJX__lfoRate (_state._state, value);
        }

        void _JWJX__lfoRate (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_lfoShape (JWJX_State& _state, float value) noexcept
        {
            _JWJX__lfoShape (_state._state, value);
        }

        void _JWJX__lfoShape (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_lfoDelay (JWJX_State& _state, float value) noexcept
        {
            _JWJX__lfoDelay (_state._state, value);
        }

        void _JWJX__lfoDelay (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_atVib (JWJX_State& _state, float value) noexcept
        {
            _JWJX__atVib (_state._state, value);
        }

        void _JWJX__atVib (_JWJX_State& _state, float value) noexcept
        {
            ParamsProcessor__atVib (_state.paramsProcessor, value);
        }

        void ParamsProcessor__atVib (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.atVib = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_atBri (JWJX_State& _state, float value) noexcept
        {
            _JWJX__atBri (_state._state, value);
        }

        void _JWJX__atBri (_JWJX_State& _state, float value) noexcept
        {
            ParamsProcessor__atBri (_state.paramsProcessor, value);
        }

        void ParamsProcessor__atBri (_ParamsProcessor_1_State& _state, float v) noexcept
        {
            _state.params.atBri = v;
            ParamsProcessor___writeEvent_paramsOut (_state, _state.params);
        }

        void _sendEvent_chorus (JWJX_State& _state, float value) noexcept
        {
            _JWJX__chorus (_state._state, value);
        }

        void _JWJX__chorus (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_volume (JWJX_State& _state, float value) noexcept
        {
            _JWJX__volume (_state._state, value);
        }

        void _JWJX__volume (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _sendEvent_tune (JWJX_State& _state, float value) noexcept
        {
            _JWJX__tune (_state._state, value);
        }

        void _JWJX__tune (_JWJX_State& _state, float value) noexcept
        {
        ...
        void _initialise (JWJX_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            _JWJX___initialise (_state._state, processorID, sessionID, frequency);
        }

        void _JWJX___initialise (_JWJX_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            int32_t  i;
            ...
            g__frequency = frequency;
            {
                ...
                for (;;)
                {
                    ...
                    JXVoice___initialise (_state.voices[i], processorID, sessionID, frequency);
                    {
                        ++i;
                    }
                }
        ...
        void JXVoice___initialise (_JXVoice_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
        ...
        void ParamsProcessor___initialise (_ParamsProcessor_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
            _state.params = plugincorp_Params { 0.5f, 0.0f, 0.5f, 0.0f, 0.0f, 0.0f, 0.5f, 0.0f, 0.5f, 0.52f, 0.0f, 0.5f, 0.5f, 0.5f, 0.0f, 0.6f, 0.25f, 0.0f, 0.45f, 0.0f, 0.35f, 0.5f, 0.05f, 0.45f, 0.4f, 0.35f, 0.0f, 0.06f, 0.5f, 0.75f, 0.4f, 0.0f, 0.35f, 0.0f, 0.2f, 0.3f, 0.25f, 0.6f, 0.8f, 0.5f };
        }
        ...
        {
            JXChorus___initialise (_state.chorus, processorID, sessionID, frequency);
            _initialise_3 (_state.out, processorID, sessionID, frequency);
        }

        void JXChorus___initialise (_JXChorus_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
            g__sessionID = sessionID;
            g__frequency = frequency;
            _state.mode = 0.6f;
        ...
        void _initialise_3 (_FXOut_1_State& _state, int32_t& processorID, int32_t sessionID, double frequency) noexcept
        {
        }

        void _advance (JWJX_State& _state, JWJX_IO& _io, int32_t _frames) noexcept
        {
            _JWJX_IO  ioCopy;
        ...
        void main (_JWJX_State& _state, _JWJX_IO& _io) noexcept
        {
            Array<_JXVoice_1_IO, 6>  voices_io;
            ...
            voices_io = Array<_JXVoice_1_IO, 6> {};
            ...
            }
            {
                ...
                for (;;)
                {
                    ...
                    JXVoice__main (_state.voices[i], voices_io[i]);
                    {
                        ++i;
                    }
                }
        ...
        void JXVoice__main (_JXVoice_1_State& _state, _JXVoice_1_IO& _io) noexcept
        {
            float  outL;
            float  outR;
            float  masterTune;
            ...
            _branch0_0:
            {
                for (;;)
                {
                    ...
                    if (_state.active)
                    {
                        ...
                        lfoHz = 0.1f + ((_state.params.lfoRate * _state.params.lfoRate) * 24.0f);
                        prevLfoPh = _state.lfoPh;
                        _state.lfoPh = (_state.lfoPh + (lfoHz * _state.invSr));
                        if (_state.lfoPh >= 1.0f)
                        {
                            _state.lfoPh = (_state.lfoPh - 1.0f);
                            _state.shVal = JXVoice__rnd (_state);
                        }
                        delayT = (_state.params.lfoDelay * _state.params.lfoDelay) * 2.5f;
                        lfoFade = (delayT < 0.001f) ? 1.0f : intrinsics::clamp (_state.noteT / delayT, 0.0f, 1.0f);
                        atVibAmt = g_at * _state.params.atVib;
                        lfo = JXVoice__lfoVal (_state, _state.lfoPh, _state.params.lfoShape) * (lfoFade + atVibAmt);
                        _state.e1 = JXVoice__advEnv (_state.e1Stage, _state.e1, _state.params.a1, _state.params.d1, _state.params.s1, _state.params.r1, _state.sr);
                        ...
                        _state.e2 = JXVoice__advEnv (_state.e2Stage, _state.e2, _state.params.a2, _state.params.d2, _state.params.s2, _state.params.r2, _state.sr);
                        ...
                        env1 = (_state.params.inv1 >= 0.5f) ? (1.0f - _state.e1) : _state.e1;
                        env2 = (_state.params.inv2 >= 0.5f) ? (1.0f - _state.e2) : _state.e2;
                        oct1 = intrinsics::floor ((_state.params.o1Range * 3.0f) + 0.5f) - 1.0f;
                        oct2 = intrinsics::floor ((_state.params.o2Range * 3.0f) + 0.5f) - 1.0f;
                        det1 = (_state.params.o1Tune - 0.5f) * 0.18f;
                        det2 = (_state.params.o2Tune - 0.5f) * 2.0f;
                        fine2 = (_state.params.o2Fine - 0.5f) * 0.18f;
                        o1Pe = (_state.params.o1EnvSel >= 0.5f) ? env2 : env1;
                        o2Pe = env1;
                        basePitchLfo = (lfo * _state.params.o1Lfo) * 0.06f;
                        o1PitchOct = (basePitchLfo + (((o1Pe - 0.0f) * _state.params.o1Env) * 2.0f)) + det1;
                        o2PitchOct = ((basePitchLfo * 0.7f) + det2) + fine2;
                        baseF = _state.freq * masterTune;
                        f1 = intrinsics::clamp (baseF * intrinsics::pow (2.0f, oct1 + o1PitchOct), 0.0f, _state.ny);
                        f2 = intrinsics::clamp (baseF * intrinsics::pow (2.0f, oct2 + o2PitchOct), 0.0f, _state.ny);
                        inc1 = f1 * _state.invSr;
                        inc2 = f2 * _state.invSr;
                        xm = static_cast<int32_t> (intrinsics::floor ((_state.params.xMod * 3.0f) + 0.5f));
                        depth = intrinsics::clamp (_state.params.xDepth, 0.0f, 1.0f);
                        pwEff = intrinsics::clamp (_state.params.pwm + (lfo * 0.1f), 0.05f, 0.95f);
                        w1 = static_cast<int32_t> (intrinsics::floor ((_state.params.o1Wave * 3.0f) + 0.5f));
                        w2 = static_cast<int32_t> (intrinsics::floor ((_state.params.o2Wave * 3.0f) + 0.5f));
                        prev1 = _state.ph1;
                        prev2 = _state.ph2;
                        ...
                        if ((xm == int32_t {1}) ? (_state.ph2 < prev2) : false)
                        {
                            _state.ph1 = (_state.ph1 * (1.0f - depth));
                        }
                        else
                        {
                            if ((xm == int32_t {2}) ? (_state.ph1 < prev1) : false)
                            {
                                _state.ph2 = (_state.ph2 * (1.0f - depth));
                            }
                        }
                        o1 = JXVoice__dco (_state, _state.ph1, inc1, w1, pwEff);
                        o2 = JXVoice__dco (_state, _state.ph2, inc2, w2, pwEff);
                        if (xm == int32_t {3})
                        {
                            m = o2 * depth;
                            o1 = ((o1 * (1.0f - depth)) + ((o1 * (1.0f + m)) * depth));
                        }
                        mixv = intrinsics::clamp (_state.params.mix, 0.0f, 1.0f);
                        sig = (o1 * (1.0f - mixv)) + (o2 * mixv);
                        sig = (sig * 0.7f);
                        yb = (sig - _state.dcx) + (0.9985f * _state.dcy);
                        _state.dcx = sig;
                        _state.dcy = yb;
                        sig = yb;
                        hpStep = static_cast<int32_t> (intrinsics::floor ((intrinsics::clamp (_state.params.hpf, 0.0f, 1.0f) * 3.0f) + 0.5f));
                        if (hpStep > int32_t {0})
                        {
                            hpHz = 40.0f * intrinsics::pow (2.0f, static_cast<float> (hpStep) * 1.6f);
                            hc = intrinsics::clamp (hpHz * _state.invSr, 0.0f, 0.45f);
                            _state.hpZ = (_state.hpZ + (hc * (sig - _state.hpZ)));
                            sig = (sig - _state.hpZ);
                        }
                        fSel = (_state.params.fEnvSel >= 0.5f) ? env2 : env1;
                        kbd = _state.params.fKey * JXVoice__log2f (_state.freq / g_refHz);
                        briAt = (g_at * _state.params.atBri) * 2.0f;
                        fcOct = ((((_state.params.cutoff * 9.0f) + ((_state.params.fEnv * fSel) * 6.0f)) + kbd) + ((lfo * _state.params.fLfo) * 4.0f)) + briAt;
                        fc = intrinsics::clamp (30.0f * intrinsics::pow (2.0f, fcOct), 25.0f, _state.ny);
                        kq = intrinsics::clamp (1.0f / (0.6f + ((_state.params.reso * _state.params.reso) * 6.0f)), 0.18f, 1.7f);
                        g = intrinsics::tan ((3.1415927f * fc) * _state.invSr);
                        a1c = 1.0f / (1.0f + (g * (g + kq)));
                        a2c = g * a1c;
                        a3c = g * a2c;
                        v3a = sig - _state.ic2a;
                        v1a = (a1c * _state.ic1a) + (a2c * v3a);
                        v2a = (_state.ic2a + (a2c * _state.ic1a)) + (a3c * v3a);
                        ...
                        lp = v2b;
                        velScale = (1.0f - _state.params.dynamics) + (_state.params.dynamics * _state.vel);
                        atLvl = 1.0f + (g_at * 0.15f);
                        amp = (((lp * env2) * velScale) * atLvl) * (0.45f + (_state.params.volume * 1.1f));
        ...
        float JXVoice__rnd (_JXVoice_1_State& _state) noexcept
        {
            float  s;
        ...
        float JXVoice__lfoVal (_JXVoice_1_State& _state, float ph, float shp) noexcept
        {
            float  si;
            float  sq;
            float  m;

            si = intrinsics::sin (ph * 6.2831855f);
            sq = (ph < 0.5f) ? 1.0f : -1.0f;
            if (shp < 0.5f)
            {
                m = shp * 2.0f;
                return (si * (1.0f - m)) + (sq * m);
            }
            return _state.shVal;
        }

        float JXVoice__advEnv (int32_t stage, float lvl, float a, float d, float s, float r, float sr) noexcept
        {
            float  ar;
            float  dr;
            float  rr;
            ...
            ar = 1.0f / (intrinsics::max (0.0008f, 0.004f + ((a * a) * 4.5f)) * sr);
            dr = 1.0f / (intrinsics::max (0.0008f, 0.005f + ((d * d) * 8.0f)) * sr);
            rr = 1.0f / (intrinsics::max (0.0008f, 0.005f + ((r * r) * 8.0f)) * sr);
            L = lvl;
            if (stage == int32_t {0})
            {
                L = (L + ar);
        ...
        float JXVoice__dco (_JXVoice_1_State& _state, float p, float inc, int32_t wv, float pwv) noexcept
        {
            float  p2;
            float  saw;
            float  saw2;
            ...
            if (wv == int32_t {0})
            {
                return ((2.0f * p) - 1.0f) - toi__dsp__polyBlep (p, inc);
            }
            if (wv == int32_t {1})
            {
                p2 = p + 0.5f;
            ...
            if (wv == int32_t {2})
            {
                w = intrinsics::clamp (0.25f + ((pwv - 0.5f) * 0.5f), 0.05f, 0.95f);
                p2_0 = p + w;
                if (p2_0 >= 1.0f)
                {
                    p2_0 = (p2_0 - 1.0f);
                }
                saw_0 = ((2.0f * p) - 1.0f) - toi__dsp__polyBlep (p, inc);
                saw2_0 = ((2.0f * p2_0) - 1.0f) - toi__dsp__polyBlep (p2_0, inc);
                return saw_0 - saw2_0;
            }
            return JXVoice__rnd (_state);
        }
        ...
        float JXVoice__log2f (float x) noexcept
        {
        ...
        void main_4 (_FXBus_1_State& _state, _FXBus_1_IO& _io) noexcept
        {
            ...
            JXChorus__main (_state.chorus, chorus_io);
            {
                out_io.audioIn = (out_io.audioIn + chorus_io.audioOut);
            }
            FXOut__main (_state.out, out_io);
            {
                _io.audioOut = (_io.audioOut + out_io.audioOut);
            }
        }

        void JXChorus__main (_JXChorus_1_State& _state, _FXBus_1_IO& _io) noexcept
        {
            float  wide;
            float  rate1;
            float  rate2;
            ...
            _branch0_0:
            {
                for (;;)
                {
                    ...
                    wide = intrinsics::clamp ((_state.mode - 0.5f) * 2.0f, 0.0f, 1.0f);
                    rate1 = 0.4f + (wide * 0.45f);
                    rate2 = 0.55f + (wide * 0.6f);
                    baseD = (0.0085f + (wide * 0.003f)) * _state.sr;
                    dep = (0.0022f + (wide * 0.0024f)) * _state.sr;
                    i1 = rate1 / _state.sr;
                    i2 = rate2 / _state.sr;
                    i3 = (rate1 * 0.83f) / _state.sr;
                    i4 = (rate2 * 0.9f) / _state.sr;
                    ...
                    wetL = (JXChorus__rdL (_state, baseD + (dep * m1)) + JXChorus__rdL (_state, (baseD * 1.35f) + (dep * m3))) * 0.5f;
                    wetR = (JXChorus__rdR (_state, baseD + (dep * m2)) + JXChorus__rdR (_state, (baseD * 1.4f) - (dep * m4))) * 0.5f;
                    _state.p1 = (_state.p1 + i1);
                    if (_state.p1 >= 1.0f)
                    {
                        _state.p1 = (_state.p1 - 1.0f);
                    }
                    _state.p2 = (_state.p2 + i2);
                    if (_state.p2 >= 1.0f)
                    {
                        _state.p2 = (_state.p2 - 1.0f);
                    }
                    _state.p3 = (_state.p3 + i3);
                    if (_state.p3 >= 1.0f)
                    {
                        _state.p3 = (_state.p3 - 1.0f);
                    }
                    _state.p4 = (_state.p4 + i4);
                    if (_state.p4 >= 1.0f)
                    {
                        _state.p4 = (_state.p4 - 1.0f);
                    }
                    std__intrinsics___wrapped_post_inc_8192 (_state.w);
                    amt = (_state.mode < 0.25f) ? 0.0f : (0.5f + (wide * 0.35f));
                    _temp[int32_t {0}] = ((_io.audioIn[int32_t {0}] * (1.0f - (amt * 0.4f))) + (wetL * amt));
                    _temp[int32_t {1}] = ((_io.audioIn[int32_t {1}] * (1.0f - (amt * 0.4f))) + (wetR * amt));
        ...
        float JXChorus__rdL (_JXChorus_1_State& _state, float d) noexcept
        {
            float  rp;
            int32_t  i0;
            int32_t  i1;
        ...
        float JXChorus__rdR (_JXChorus_1_State& _state, float d) noexcept
        {
            float  rp;
            int32_t  i0;
            int32_t  i1;
        ...
        void FXOut__main (_FXOut_1_State& _state, _FXBus_1_IO& _io) noexcept
        {
            Vector<float, 2>  _temp;

            for (;;)
            {
                _temp[int32_t {0}] = intrinsics::tanh (_io.audioIn[int32_t {0}] * 0.95f);
                _temp[int32_t {1}] = intrinsics::tanh (_io.audioIn[int32_t {1}] * 0.95f);
                _io.audioOut = (_io.audioOut + _temp);
                return;
            }
        }
        ...
        static JWJX_State& state_upcast_struct_JWJX_State_struct__FmHWN (_JWJX_State& child) { return *reinterpret_cast<JWJX_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (JWJX_State, _state))); }
        static _JWJX_State& state_upcast_struct_JWJX_State_struct_jECcjd (std_voices_std_voices_VoiceAllocator_specialised_lf0wne_1_State& child) { return *reinterpret_cast<_JWJX_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_JWJX_State, voiceAllocator))); }
        static _JWJX_State& state_upcast_struct_JWJX_State_struct_ZLmQTd (std_midi_MPEConverter_1_State& child) { return *reinterpret_cast<_JWJX_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_JWJX_State, noteTap))); }
        static _JWJX_State& state_upcast_struct_JWJX_State_struct_JLl2fd (_ParamsProcessor_1_State& child) { return *reinterpret_cast<_JWJX_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_JWJX_State, paramsProcessor))); }
        static _JWJX_State& state_upcast_struct_JWJX_State_struct_CxsOI (std_midi_MPEConverter_1_State& child) { return *reinterpret_cast<_JWJX_State*> (reinterpret_cast<char*> (std::addressof (child)) - (OFFSETOF (_JWJX_State, mpe))); }
        ...
        double g__frequency {};
        static constexpr float g_at { 0.0f };
...
struct JWJX
{
    using PerformerClass = performer::JWJX;
    static constexpr auto filename = "JX.cmajorpatch";
    ...
    static constexpr const char* cmaj_api_cmajpatchconnection_js =
        ...
        "    }\n"
        "}\n";
    static constexpr const char* JX_cmajorpatch =
        "{\n"
        "  \"CmajorVersion\": 1,\n"
        "  \"ID\": \"com.plugincorp.jwjx\",\n"
        "  \"version\": \"0.1\",\n"
        "  \"name\": \"JW-JX\",\n"
        "  \"description\": \"TOI JW-JX - 6-voice DCO polysynth with cross-mod clean-to-metallic morph by plugin corp\",\n"
        "  \"category\": \"instrument\",\n"
        "  \"manufacturer\": \"TOI\",\n"
        "  \"isInstrument\": true,\n"
    ...
    static constexpr std::array files =
    {
        ...
        File { "JX.cmajorpatch", std::string_view (JX_cmajorpatch, 398) },
...
{
    return new JWJXPlugin (std::make_shared<cmaj::Patch>());
}
